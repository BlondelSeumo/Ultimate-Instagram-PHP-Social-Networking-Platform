...
if(($login_user_name != '') && ($login_user_password != '') && ($login_user_name == $uname || $login_user_name == $uemail) && ($login_user_password == $pass)){
    ...
    $login_ok = true; 
	$getUserID = mysqli_query($db,"SELECT user_id,user_name, user_email,user_type FROM dot_users WHERE user_password = '$login_user_password' AND (user_name = '$login_user_name') AND user_status = '1'") or die(mysqli_error($db));
