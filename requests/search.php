...
if(isset($_POST['keys'])){
   ...
   if($searchKey != ''){
		...
		$GetHashTagsFromSearch = $Dot->Dot_GetHashTagsSearch($searchKey);
		if($keyRestuls || $GetHashTagsFromSearch){
           ...
           if($GetHashTagsFromSearch){
              foreach($GetHashTagsFromSearch as $sht){ 
				echo '
					<div class="result_user"> 
						<a href="'.$base_url.'hashtag/'.$searchKey.'">
							<div class="result_hash_s">#</div>
							<div class="result_user_name">#'.$searchKey.'</div> 
						</a>
					</div>
				';
			  }
