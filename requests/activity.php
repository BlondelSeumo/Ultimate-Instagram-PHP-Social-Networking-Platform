...
if (isset($_POST['f']) && in_array($_POST['f'], $Activitytype)) {
	...
	if ($activity == 'live') {
		if (isset($_POST['stream_name'])) {
			$streamingName = mysqli_real_escape_string($db, $_POST['stream_name']);
			$createStreaming = $Dot->Dot_CreateNeweStreaming($uid, $streamingName);
			$liveID = $createStreaming['live_id'];
			$getUsersForNotification = $Dot->Dot_GetAllUserFollowersForNotification($uid);
			if ($getUsersForNotification) {
				foreach ($getUsersForNotification as $liveNots) {
					$notificationDeviceID = $liveNots['device_id'];
					$notificationDeviceType = $liveNots['device_type'];
					$notificationDeviceKey = $liveNots['device_key'];
					if (!empty($notificationDeviceKey) && $oneSignalStatus == '1') {
						$likedYourPostTitle = $dataUserFullName . ' Started a live streams!';
						$likedPostNotificationBody = 'Click to join the live broadcast!';
						$url = $base_url . "live/" . $dataUsername;
						$Dot->Dot_OneSignalPushNotificationSend($likedPostNotificationBody, $likedYourPostTitle, $url, $notificationDeviceKey, $oneSignalApi, $oneSignalRestApi);
					}
				}
			}
			if ($amazonS3Status == 1) {$rect = $Dot->Dot_StartCloudRecording(1, $amazonS3Region, $amazonS3BucketName, $amazonS3Key, $amazonS3SecretKey, $streamingName, $uid, $liveID, $agoraAppID, $agoraCustomerID, $agoraCertificate);}
			$data = array(
				'post_id' => $liveID,
			);
	...
	/*Like Live Video*/
	if ($activity == 'likeLiveVideo') {
		if (isset($_POST['vd'])) {
			$liveLikedVideoID = mysqli_real_escape_string($db, $_POST['vd']);
			$insertLiveVideoLike = $Dot->Dot_InserLiveLikeVideo($uid, $liveLikedVideoID);
			if ($insertLiveVideoLike) {
				echo '1';
			}
		}
	}
					...
					echo '
					     <div class="carDetailsContainer">
							 ...
							 <div class="cardMenuContainer animwrap">
								<div class="carProfileLink caranim">
	...
	if ($activity == 'fastEmoji') {
		...
		if ($emoticonList) {
			...
			foreach ($emoticonList as $fast_answer) {
				...
				echo '<span class="fast_icon" id="fast_answer_with_emoji" style="background-image:url(' . $base_url . 'uploads/fastEmojis/' . $answerEmoji . ');" data-fastkey="' . $answerKey . '" data-id="' . $pid . '"  data-post="' . $answerPostType . '" data-type="fastK"></span>';
			}
		} else {
			echo '1';
		}
	...
	//Delete Post
	if ($activity == 'deletePostad') {
		if (isset($_POST['u']) && isset($_POST['this'])) {
			$deleteThisPost = mysqli_real_escape_string($db, $_POST['this']);
			$user = mysqli_real_escape_string($db, $_POST['u']);
			if ($pointSystemStatus == '1') {$DeletePoint = $Dot->Dot_RemovePoint($uid, 'post', $pointPost, $deleteThisPost);}
			$DeleteThisPostFromData = $Dot->Dot_DeletePostAdmn($uid, $deleteThisPost);
			if ($DeleteThisPostFromData) {
				echo '1';
			}
		}
	...
	if ($activity == 'personalInformation') {
		...
		if (in_array($_POST['relation'], $userRelationShip) && in_array($_POST['gen'], $genderArray)) {

			if ($userHeightFeature == '1') {
			...
			if ($userLifeStyleFeature == '1') {
				if (in_array($_POST['life'], $LifeStyleArray)) {
					$userLifeStyle = mysqli_real_escape_string($db, $_POST['life']);
				}
			} else {
				$userLifeStyle = '';
			}
			if ($userChildrenFeature == '1') {
				if (in_array($_POST['child'], $userChildrenArray)) {
					$userChildren = mysqli_real_escape_string($db, $_POST['child']);
				}
			} else {
				$userChildren = '';
			}
			if ($userSmokingFeature == '1') {
				if (in_array($_POST['smoke'], $userSmokeHabitArray)) {
					$userSmokeHabit = mysqli_real_escape_string($db, $_POST['smoke']);
				}
			} else {
				$userSmokeHabit = '';
			}
			if ($userDrinkingFeature == '1') {
				if (in_array($_POST['drink'], $userDrinkingHabitArray)) {
					$userDrinkingHabit = mysqli_real_escape_string($db, $_POST['drink']);
				}
			} else {
				$userDrinkingHabit = '';
			}
			if ($userBodyTypeFeature == '1') {
				if (in_array($_POST['body'], $userBodyTypeArray)) {
					$userBodyType = mysqli_real_escape_string($db, $_POST['body']);
				}
			} else {
				$userBodyType = '';
			}
			if ($userHairColorFeature == '1') {
				if (in_array($_POST['hair'], $userHairColorArray)) {
					$userHairColor = mysqli_real_escape_string($db, $_POST['hair']);
				}
			} else {
				$userHairColor = '';
			}
			if ($sexualityFeature == '1') {
				if (in_array($_POST['sex'], $SexualityArray)) {
					$userSexuality = mysqli_real_escape_string($db, $_POST['sex']);
				}
			} else {
				$userSexuality = '';
			}
			if ($userEyeColorFeature == '1') {
				if (in_array($_POST['eye'], $userEyeColorArray)) {
	...
	if ($activity == 'profileInformation') {
		if (isset($_POST['fn']) && isset($_POST['web']) && isset($_POST['ub']) && isset($_POST['lw']) && isset($_POST['unm'])) {
			...
			$newUserName = mysqli_real_escape_string($db, strtolower($_POST['unm']));
			if (strlen($newUserName) > 0) {
				if (preg_match('/[^A-Za-z0-9-_]/', $newUserName)) {
					echo '<div class="newWarning"> ' . $page_Lang['only_english_character'][$dataUserPageLanguage] . '</div>';
					exit();
				}
				if ($dataUsername != $newUserName) {
					$CheckUsername = $Dot->Dot_CheckUserNameExistINData($newUserName);
					if ($CheckUsername) {
						echo '<div class="newWarning"> ' . $page_Lang['username_already_in_use'][$dataUserPageLanguage] . '</div>';
						exit();
					}
				}
			} else {
				$newUserName = $dataUsername;
			}

			$saveNewProfileInformation = $Dot->Dot_SaveUserProfileInformation($uid, $newUserFullName, $newUserWebsite, $newUserBio, $newUserLikedWord, $newUserName);
	...
	if ($activity == 'getNewMessage') {
		include "../contents/popupMessageNotifications.php";
	}
	if ($activity == 'getNewNotification') {
		include "../contents/popupNotifications.php";
	}
		...
		if (isset($_POST['name'])) {
			...
			if ($searchResults) {
				foreach ($searchResults as $searchData) {
					...
					echo '
						 <!---->
						  <li class="_5l-3 _1ht1 getuse" style="display:inline-block;" data-id="' . $FinedeUserID . '" data-user="' . $FindedUserName . '" data-page="chat">
	...
	if ($activity == 'friendlistchat') {
		...
		if ($FriendsArray) {
		 ...
		 <div class="listHeaderFriend" id="closeChatUserList"> <h1 class="new_conversation_title"><span class="dinic">' . $Dot->Dot_SelectedMenuIcon('back_icon') . '</span>' . $page_Lang['new_conversation'][$dataUserPageLanguage] . '</h1></div>
		 <div class="cl_4u-m">
	...
	if ($activity == 'searchPeople') {
		include "../contents/popupSearchBox.php";
	}
	...
	if ($activity == 'deletestory') {
		if (isset($_POST['story'])) {
			...
			if ($storyid != '') {
				...
				if ($pointStoriesStatus == '1' && $pointSystemStatus == '1') {
					$Dot->Dot_RemoveStoryPoint($uid, $storyid);
				}
				...
				if ($Stickers) {
				   ...
				   ';
					foreach ($Stickers as $sticker) {
						...
						echo '<div class="sticker_body" style="background-image:url(' . $base_url . 'uploads/emoticons/F_Sticker/' . $stickerIMG . ');"  data-fastkey="' . $stickerKey . '" data-id="' . $postStickerID . '"  data-post="' . $postStickerType . '" data-type="jsticker"></div>';
					}
					echo '</div>';
				}
			}
		...
		if (isset($_POST['pmid']) && isset($_POST['post'])) {
			...
			if ($postStickerID) {
				...
				if ($dGifs) {
					echo '<div class="stickers_container" data-textarea="textarea' . $postStickerID . '">
				               <div class="stickers_header">
							        <div class="searchGiphyBox"><input type="text" class="postGif" data-type="commentGif" data-styp="' . $postStickerType . '" data-id="' . $postStickerID . '" placeholder="Search Gif"></div>
									<div class="closeGifBox"><svg xmlns="http://www.w3.org/2000/svg" x="0px" y="0px" width="30" height="30" viewBox="0 0 192 192" style=" fill:#000000;"><g fill="none" fill-rule="nonzero" stroke="none" stroke-width="1" stroke-linecap="butt" stroke-linejoin="miter" stroke-miterlimit="10" stroke-dasharray="" stroke-dashoffset="0" font-family="none" font-weight="none" font-size="none" text-anchor="none" style="mix-blend-mode: normal"><path d="M0,192v-192h192v192z" fill="none"></path><g fill="#8a99a4"><path d="M47.92188,39.92188c-3.25538,0.00085 -6.18567,1.97404 -7.41065,4.99016c-1.22498,3.01612 -0.50037,6.47372 1.83252,8.74421l42.34375,42.34375l-42.34375,42.34375c-2.0899,2.00654 -2.93176,4.98613 -2.2009,7.78965c0.73086,2.80352 2.92023,4.99289 5.72375,5.72375c2.80352,0.73086 5.78311,-0.111 7.78965,-2.20091l42.34375,-42.34375l42.34375,42.34375c2.00653,2.08993 4.98614,2.93181 7.78967,2.20095c2.80354,-0.73085 4.99292,-2.92024 5.72377,-5.72377c0.73085,-2.80354 -0.11102,-5.78314 -2.20095,-7.78967l-42.34375,-42.34375l42.34375,-42.34375c2.36608,-2.29993 3.07751,-5.81653 1.79148,-8.8553c-1.28603,-3.03877 -4.3057,-4.97634 -7.60397,-4.87907c-2.07839,0.06193 -4.05103,0.93056 -5.5,2.42188l-42.34375,42.34375l-42.34375,-42.34375c-1.50617,-1.54826 -3.57436,-2.42175 -5.73437,-2.42188z"></path></g></g></svg></div>
							   </div>
				   ...
				   ';
					foreach ($dGifs as $ggif) {
						...
						echo '
							<div class="gif_body"  data-fastkey="' . $dataGifKey . '" data-id="' . $postStickerID . '"  data-post="' . $postStickerType . '" data-type="jgif">
	...
	if ($activity == 'paypalid') {
		if (isset($_POST['client']) && isset($_POST['secret']) && isset($_POST['paymode']) && isset($_POST['minimumAmounth'])) {
			...
			$minimumCreditAmount = mysqli_real_escape_string($db, $_POST['minimumAmounth']);
			$SavePayPalSettings = $Dot->Dot_UpdatePayPalSettins($paypalMode, $paypalCliendtID, $paypalSecretID, $uid, $minimumCreditAmount);
...
<div class="app-customization-container">
    <div class="app-customization-wrapper">
        <div class="post-modal-middle">
            <div class="customize_profile_header"><?php echo $page_Lang['customize_profile'][$dataUserPageLanguage]; ?>
                <div class="closeDesign"><svg xmlns="http://www.w3.org/2000/svg" x="0px" y="0px" width="28" height="28"
                        ...
                        <g fill="none" fill-rule="nonzero" stroke="none" stroke-width="1" stroke-linecap="butt"
                            ...
                            <g fill="#8a99a4">
                                <path
                                    d="M47.92188,39.92188c-3.25538,0.00085 -6.18567,1.97404 -7.41065,4.99016c-1.22498,3.01612 -0.50037,6.47372 1.83252,8.74421l42.34375,42.34375l-42.34375,42.34375c-2.0899,2.00654 -2.93176,4.98613 -2.2009,7.78965c0.73086,2.80352 2.92023,4.99289 5.72375,5.72375c2.80352,0.73086 5.78311,-0.111 7.78965,-2.20091l42.34375,-42.34375l42.34375,42.34375c2.00653,2.08993 4.98614,2.93181 7.78967,2.20095c2.80354,-0.73085 4.99292,-2.92024 5.72377,-5.72377c0.73085,-2.80354 -0.11102,-5.78314 -2.20095,-7.78967l-42.34375,-42.34375l42.34375,-42.34375c2.36608,-2.29993 3.07751,-5.81653 1.79148,-8.8553c-1.28603,-3.03877 -4.3057,-4.97634 -7.60397,-4.87907c-2.07839,0.06193 -4.05103,0.93056 -5.5,2.42188l-42.34375,42.34375l-42.34375,-42.34375c-1.50617,-1.54826 -3.57436,-2.42175 -5.73437,-2.42188z">
                                </path>
                            </g>
                        </g>
                    </svg></div>
            ...
            <div class="customization_container">
                ...
                <div class="customizationWrapper">
                    <div class="customtit">
                        <div class="custom_utuicon"><svg xmlns="http://www.w3.org/2000/svg" x="0px" y="0px" width="28"
                                height="28" viewBox="0 0 192 192" style=" fill:#000000;">
                                <g fill="none" fill-rule="nonzero" stroke="none" stroke-width="1" stroke-linecap="butt"
                    ...
                    <div class="customtit">
                        <div class="custom_utuicon"><svg xmlns="http://www.w3.org/2000/svg" x="0px" y="0px" width="28"
                                ...
                                <g fill="none" fill-rule="nonzero" stroke="none" stroke-width="1" stroke-linecap="butt"
                                    ...
                                    <g fill="#8a99a4">
                                        <path
                                            d="M96,0c-10.52806,0 -19.2,8.67194 -19.2,19.2h-25.6c-10.52781,0 -19.2,8.67219 -19.2,19.2v115.2c0,10.52781 8.67219,19.2 19.2,19.2h89.6c10.52781,0 19.2,-8.67219 19.2,-19.2v-115.2c0,-10.52781 -8.67219,-19.2 -19.2,-19.2h-25.6c0,-10.52806 -8.67194,-19.2 -19.2,-19.2zM96,12.8c3.61043,0 6.4,2.78957 6.4,6.4c0,3.61043 -2.78957,6.4 -6.4,6.4c-3.61043,0 -6.4,-2.78957 -6.4,-6.4c0,-3.61043 2.78957,-6.4 6.4,-6.4zM51.2,32h25.6v6.4c0,3.5328 2.8672,6.4 6.4,6.4h25.6c3.5328,0 6.4,-2.8672 6.4,-6.4v-6.4h25.6c3.61619,0 6.4,2.78381 6.4,6.4v115.2c0,3.61619 -2.78381,6.4 -6.4,6.4h-89.6c-3.61619,0 -6.4,-2.78381 -6.4,-6.4v-115.2c0,-3.61619 2.78381,-6.4 6.4,-6.4zM96.0375,64c-2.66486,-0.01448 -5.05987,1.62368 -6.0125,4.1125l-22.0125,57.6c-0.81777,2.13701 -0.43317,4.54798 1.00888,6.32451c1.44205,1.77653 3.72243,2.64864 5.98192,2.28776c2.2595,-0.36089 4.15476,-1.89993 4.97169,-4.03726l3.3125,-8.6875h25.425l3.3125,8.6875c0.81694,2.13733 2.7122,3.67638 4.97169,4.03726c2.2595,0.36089 4.53987,-0.51123 5.98192,-2.28776c1.44205,-1.77653 1.82665,-4.18749 1.00888,-6.32451l-22.0125,-57.6c-0.94282,-2.46323 -3.30006,-4.09593 -5.9375,-4.1125zM96,88.325l7.825,20.475h-15.65z">
                                        </path>
                                    </g>
                                </g>
                            </svg></div><?php echo $page_Lang['text_colors_title'][$dataUserPageLanguage]; ?>
                    </div>
                    <div class="change_area_header">
                        <div class="selectedColorContainer">
                            <div class="selectedBgColor" id="txc" data-type="chtxcolor">
                                <?php echo $page_Lang['choose_profile_text_color'][$dataUserPageLanguage]; ?></div>
                        </div>
                    </div>
                </div>
                <!---->
                    ...
                    <div class="customtit">
                        <div class="custom_utuicon"><svg xmlns="http://www.w3.org/2000/svg" x="0px" y="0px" width="28"
                                ...
                                <g fill="none" fill-rule="nonzero" stroke="none" stroke-width="1" stroke-linecap="butt"
                                    ...
                                    <g fill="#8a99a4">
                                        <path
                                            d="M20,64c-1.024,0 -2.04812,0.39188 -2.82812,1.17188c-0.784,0.776 -1.17188,1.80412 -1.17188,2.82812v52c0,4.41828 3.58172,8 8,8h52c1.024,0 2.04412,-0.39587 2.82812,-1.17187c1.56,-1.56 1.56,-4.09626 0,-5.65626l-21.0625,-21.0625c11.26062,-9.98542 25.99488,-16.10938 42.23438,-16.10938c25.192,0 46.943,14.582 57.375,35.75c2.208,4.48 7.33413,6.67775 12.07813,5.09375c5.672,-1.888 8.55387,-8.36638 5.92187,-13.73438c-13.648,-27.88 -42.231,-47.10938 -75.375,-47.10938c-21.76404,0 -41.45944,8.40251 -56.34375,22l-20.82812,-20.82812c-0.78,-0.78 -1.80412,-1.17188 -2.82812,-1.17188z">
                                        </path>
                                    </g>
                                </g>
                            </svg></div>Use Default Custom Colors
                    </div>
                    <div class="change_area_header">
                        <div class="selectedColorContainer">
            ...
            <div class="post_box_footer">
                <div class="control left_btn">
                    <div class="close_post_box waves-effect waves-light btn blue-grey lighten-3" id="closePost">
                        <?php echo $page_Lang['cancel'][$dataUserPageLanguage]; ?></div>
                </div>
                <div class="control right_btn">
                    <div class="share_post_box waves-effect waves-light btn blue theCustoms" data-type="saveNewDesign"
                        data-pbgcolor="<?php echo $profileBackgroundColor; ?>"
                        data-pheadericon="<?php echo $profilebuttonColors; ?>"
                        data-ptcolor="<?php echo $profileTextColor; ?>" data-fonts="<?php echo $profileSelectedFont; ?>"
                        data-headercolor="<?php echo $profileHeaderSecondColors; ?>"
                        data-headericontwo="<?php echo $profileHeaderSecondicons; ?>">
                        <?php echo $page_Lang['save_changes'][$dataUserPageLanguage]; ?></div>
                </div>
            </div>
            <!---->
        </div>
...
<div class="social_share_buttons_container openSocial">
    <div class="shareonSocial">
        <div class="shareonSocialHeader"><?php echo $page_Lang['share_this_post_on'][$dataUserPageLanguage]; ?> <div
                class="close_icon_sv closethisBox"><svg xmlns="http://www.w3.org/2000/svg" x="0px" y="0px" width="24"
                    height="24" viewBox="0 0 192 192" style=" fill:#000000;">
                    <g fill="none" fill-rule="nonzero" stroke="none" stroke-width="1" stroke-linecap="butt"
                        stroke-linejoin="miter" stroke-miterlimit="10" stroke-dasharray="" stroke-dashoffset="0"
                        font-family="none" font-weight="none" font-size="none" text-anchor="none"
                        style="mix-blend-mode: normal">
        ...
        <div class="share_buttons_wrap">
            <?php if ($facebookShareStatus == '1') {?>
            <!---->
            <div class="socialShareButtonItem">
                <a id="facebook"
                    onclick="o_share_social('facebook', '<?php echo $base_url; ?>status.php?msgID=<?php echo $postNumber; ?>', '<?php echo $postNumber; ?>')"
                    title="facebook" rel="nofollow">
                    <div class="socialItem">
                        <div class="socialItem_icon"><svg xmlns="http://www.w3.org/2000/svg" x="0px" y="0px" width="40"
                                height="40" viewBox="0 0 192 192" style=" fill:#000000;">
                                <g fill="none" fill-rule="nonzero" stroke="none" stroke-width="1" stroke-linecap="butt"
                                    stroke-linejoin="miter" stroke-miterlimit="10" stroke-dasharray=""
                                    stroke-dashoffset="0" font-family="none" font-weight="none" font-size="none"
                                    text-anchor="none" style="mix-blend-mode: normal">
                                    ...
                                    <g id="Layer_1">
                                        ...
                                        <path
                                            d="M117.472,96h-13.472v48h-20v-48h-12v-16h12v-9.64c0.008,-14.032 5.836,-22.36 22.368,-22.36h13.632v16h-9.148c-6.436,0 -6.852,2.4 -6.852,6.892v9.108h16z"
                                            fill="#ffffff"></path>
                                    </g>
                                </g>
                            </svg></div>
            ...
            <!---->
            <?php }?>
            <?php if ($twitterShareStatus == '1') {?>
            <!---->
            <div class="socialShareButtonItem">
                <a id="twitter"
                    onclick="o_share_social('twitter', '<?php echo $base_url; ?>status.php?msgID=<?php echo $postNumber; ?>', '<?php echo $postNumber; ?>')"
            ...
            <!---->
            <?php }?>
            <?php if ($pinterestShareStatus == '1') {?>
            <!---->
            <div class="socialShareButtonItem">
                <a id="twitter"
                    onclick="o_share_social('pinterest', '<?php echo $base_url; ?>status.php?msgID=<?php echo $postNumber; ?>', '<?php echo $postNumber; ?>')"
                    ...
                    <div class="socialItem">
                        <div class="socialItem_icon"><svg xmlns="http://www.w3.org/2000/svg" x="0px" y="0px" width="40"
                                ...
                                <g fill="none" fill-rule="nonzero" stroke="none" stroke-width="1" stroke-linecap="butt"
                                    ...
                                    <path d="M0,192v-192h192v192z" fill="none"></path>
                                    <g id="Layer_1">
                                        <circle cx="24" cy="24" transform="scale(4,4)" r="20" fill="#d32f2f"></circle>
                                        <path
            ...
            <!---->
            <?php }?>
            <?php if ($tumblrShareStatus == '1') {?>
            <!---->
            <div class="socialShareButtonItem">
                <a id="twitter"
                    onclick="o_share_social('tumblr', '<?php echo $base_url; ?>status.php?msgID=<?php echo $postNumber; ?>', '<?php echo $postNumber; ?>')"
                    title="tumblr" rel="nofollow">
                    <div class="socialItem">
                        <div class="socialItem_icon"><svg xmlns="http://www.w3.org/2000/svg" x="0px" y="0px" width="40"
                                height="40" viewBox="0 0 192 192" style=" fill:#000000;">
                                <g fill="none" fill-rule="nonzero" stroke="none" stroke-width="1" stroke-linecap="butt"
            ...
            <!---->
            <?php }?>
            <?php if ($vkSocialShareStatus == '1') {?>
            <!---->
            <div class="socialShareButtonItem">
                <a id="twitter"
                    onclick="o_share_social('vkontakte', '<?php echo $base_url; ?>status.php?msgID=<?php echo $postNumber; ?>', '<?php echo $postNumber; ?>')"
            ...
            <!---->
            <?php }?>
            <?php if ($redditSocialShareStatus == '1') {?>
            <!---->
            <div class="socialShareButtonItem">
                <a id="twitter"
                    onclick="o_share_social('reddit', '<?php echo $base_url; ?>status.php?msgID=<?php echo $postNumber; ?>', '<?php echo $postNumber; ?>')"
                    title="reddit" rel="nofollow">
                    <div class="socialItem">
                        <div class="socialItem_icon"><svg xmlns="http://www.w3.org/2000/svg" x="0px" y="0px" width="40"
                                height="40" viewBox="0 0 192 192" style=" fill:#000000;">
                                <g fill="none" fill-rule="nonzero" stroke="none" stroke-width="1" stroke-linecap="butt"
                                    ...
                                    <g>
                                        <g id="surface1">
                                            <path
                                                d="M48.76562,78.21875c-7.75,-6.96875 -19.15625,-6.90625 -25.45312,0.125c-6.29688,7.01562 -5.20312,20.07812 3.70312,26.51562z"
                                                fill="#ffffff"></path>
                                            <path
                                                d="M143.23438,78.21875c7.75,-6.96875 19.15625,-6.90625 25.45312,0.125c6.29688,7.01562 5.20312,20.07812 -3.70312,26.51562z"
                                                fill="#ffffff"></path>
                                            <path
                                                d="M167.28125,41.90625c0,7.71875 -6.26563,14 -14,14c-7.73438,0 -14,-6.28125 -14,-14c0,-7.73438 6.26562,-14 14,-14c7.73437,0 14,6.26562 14,14z"
                                                fill="#ffffff"></path>
                                            <path
                                                d="M168.6875,114.4375c0,-28.59375 -32.39062,-51.78125 -72.34375,-51.78125c-39.95313,0 -72.34375,23.1875 -72.34375,51.78125c0,28.60938 32.39062,51.79688 72.34375,51.79688c39.95312,0 72.34375,-23.1875 72.34375,-51.79688z"
                                                fill="#ffffff"></path>
                                            <path
                                                d="M133,105.5625c0,6.375 -5.17188,11.53125 -11.54688,11.53125c-6.35937,0 -11.53125,-5.15625 -11.53125,-11.53125c0,-6.375 5.17188,-11.53125 11.53125,-11.53125c6.375,0 11.54688,5.15625 11.54688,11.53125z"
                                                fill="#d84315"></path>
                                            <path
                                                d="M82.07812,105.5625c0,6.375 -5.17187,11.53125 -11.53125,11.53125c-6.375,0 -11.54688,-5.15625 -11.54688,-11.53125c0,-6.375 5.17188,-11.53125 11.54688,-11.53125c6.35938,0 11.53125,5.15625 11.53125,11.53125z"
                                                fill="#d84315"></path>
                                            <path
                                                d="M96.01562,139.60938c-13.01562,0 -24.5625,-2.98438 -32.01562,-7.60938c4.09375,8.17188 16.78125,16 32.01562,16c15.20313,0 27.89063,-7.82812 31.98438,-16c-7.42188,4.625 -18.98438,7.60938 -31.98438,7.60938z"
                                                fill="#37474f"></path>
                                            <path
                                                d="M167.3125,108.10938l-4.67188,-6.48438c3.32813,-2.40625 5.48438,-6.23438 5.95313,-10.5c0.42187,-3.92188 -0.625,-7.60938 -2.89063,-10.125c-2.28125,-2.54688 -5.5625,-3.95312 -9.21875,-3.96875c-3.71875,0.04687 -7.57812,1.45312 -10.59375,4.15625l-5.34375,-5.95312c4.5,-4.03125 10.1875,-6.09375 15.96875,-6.20312c5.95313,0.03125 11.32813,2.375 15.14063,6.625c3.76562,4.20312 5.54687,10.15625 4.89062,16.32812c-0.70312,6.51562 -4.0625,12.40625 -9.23438,16.125z"
                                                fill="#37474f"></path>
                                            <path
                                                d="M24.67188,108.10938c-5.15625,-3.73438 -8.51562,-9.60938 -9.21875,-16.125c-0.67187,-6.17188 1.10938,-12.125 4.89063,-16.32812c3.79687,-4.23438 9.1875,-6.59375 15.125,-6.625c0.04687,0 0.07812,0 0.10937,0c5.76563,0 11.40625,2.20312 15.85938,6.21875l-5.34375,5.95312c-3,-2.71875 -6.75,-4.03125 -10.57813,-4.17187c-3.65625,0.01562 -6.9375,1.42187 -9.21875,3.98437c-2.25,2.5 -3.3125,6.1875 -2.89062,10.10938c0.46875,4.26562 2.625,8.09375 5.95312,10.5z"
                                                fill="#37474f"></path>
                                            <path
                                                d="M100,67.35938h-8c0,-11.54688 0,-42.1875 19.92188,-42.1875c8.60938,0 12.76562,4.84375 15.79688,8.375c2.51562,2.9375 3.84375,4.34375 6.46875,4.34375h5.48438v8h-5.48438c-6.42188,0 -9.8125,-3.95312 -12.54688,-7.14062c-2.67187,-3.125 -4.78125,-5.59375 -9.73437,-5.59375c-8,0.01562 -11.90625,11.20312 -11.90625,34.20312z"
                                                fill="#37474f"></path>
                                            <path
                                                d="M96.34375,67.79688c37.6875,0 68.34375,20.92188 68.34375,46.64062c0,25.73438 -30.65625,46.65625 -68.34375,46.65625c-37.6875,0 -68.34375,-20.92188 -68.34375,-46.65625c0,-25.71875 30.65625,-46.64062 68.34375,-46.64062M96.34375,59.79688c-42.17188,0 -76.34375,24.46875 -76.34375,54.64062c0,30.1875 34.1875,54.65625 76.34375,54.65625c42.15625,0 76.34375,-24.46875 76.34375,-54.65625c0,-30.17188 -34.1875,-54.64062 -76.34375,-54.64062z"
                                                fill="#37474f"></path>
                                            <path
                                                d="M153.28125,31.90625c5.51562,0 10,4.48437 10,10c0,5.5 -4.48438,10 -10,10c-5.51563,0 -10,-4.5 -10,-10c0,-5.51563 4.48437,-10 10,-10M153.28125,23.90625c-9.9375,0 -18,8.04687 -18,18c0,9.9375 8.0625,18 18,18c9.9375,0 18,-8.0625 18,-18c0,-9.95313 -8.04688,-18 -18,-18z"
                                                fill="#37474f"></path>
                                        </g>
                                    </g>
                                </g>
            ...
            <!---->
            <?php }?>
            <?php if ($linkedinSocialShareStatus == '1') {?>
            <!---->
            <div class="socialShareButtonItem">
                <a id="twitter"
                    onclick="o_share_social('linkedin', '<?php echo $base_url; ?>status.php?msgID=<?php echo $postNumber; ?>', '<?php echo $postNumber; ?>')"
                        ...
                        <div class="socialItem_icon"><svg xmlns="http://www.w3.org/2000/svg" x="0px" y="0px" width="40"
                                ...
                                <g fill="none" fill-rule="nonzero" stroke="none" stroke-width="1" stroke-linecap="butt"
                                    ...
                                    <g id="Layer_1">
                                        ...
                                        <rect x="14" y="19" transform="scale(4,4)" width="4" height="15" fill="#ffffff">
                                        </rect>
                                        <path
                                            d="M63.952,68h-0.088c-4.776,0 -7.864,-3.56 -7.864,-8.004c0,-4.54 3.184,-7.996 8.044,-7.996c4.868,0 7.864,3.456 7.956,7.996c0,4.444 -3.088,8.004 -8.048,8.004z"
                                            fill="#ffffff"></path>
                                        <path
                                            d="M140,98c0,-12.152 -9.848,-22 -22,-22c-7.448,0 -14.02,3.712 -18,9.376v-9.376h-16v60h16v-32c0,-6.628 5.372,-12 12,-12c6.628,0 12,5.372 12,12v32h16c0,0 0,-36.316 0,-38z"
                                            fill="#ffffff"></path>
                                    </g>
                                </g>
                            </svg></div>
            ...
            <!---->
            <?php }?>
            <?php if ($whatsappSocialShareStatus == '1') {?>
            <!---->
            <div class="socialShareButtonItem">
                <a id="twitter"
                    onclick="o_share_social('whatsapp', '<?php echo $base_url; ?>status.php?msgID=<?php echo $postNumber; ?>', '<?php echo $postNumber; ?>')"
            ...
            <!---->
            <?php }?>
            <?php if ($viberSocialShareStatus == '1') {?>
            <!---->
            <div class="socialShareButtonItem">
                <a id="twitter"
                    onclick="o_share_social('viber', '<?php echo $base_url; ?>status.php?msgID=<?php echo $postNumber; ?>', '<?php echo $postNumber; ?>')"
                    title="viber" rel="nofollow">
                    <div class="socialItem">
                        <div class="socialItem_icon"><svg xmlns="http://www.w3.org/2000/svg" x="0px" y="0px" width="40"
                                height="40" viewBox="0 0 192 192" style=" fill:#000000;">
                                <g fill="none" fill-rule="nonzero" stroke="none" stroke-width="1" stroke-linecap="butt"
            ...
            <!---->
            <?php }?>
            <?php if ($diggSocialShareStatus == '1') {?>
            <!---->
            <div class="socialShareButtonItem">
                <a id="twitter"
                    onclick="o_share_social('digg', '<?php echo $base_url; ?>status.php?msgID=<?php echo $postNumber; ?>', '<?php echo $postNumber; ?>')"
                    title="digg" rel="nofollow">
                    <div class="socialItem">
                        <div class="socialItem_icon"><svg xmlns="http://www.w3.org/2000/svg" x="0px" y="0px" width="40"
                                height="40" viewBox="0 0 192 192" style=" fill:#000000;">
                                <g fill="none" fill-rule="nonzero" stroke="none" stroke-width="1" stroke-linecap="butt"
            ...
            <!---->
            <?php }?>
            <?php if ($deliciousSocialShareStatus == '1') {?>
            <!---->
            <div class="socialShareButtonItem">
                <a id="twitter"
                    onclick="o_share_social('delicious', '<?php echo $base_url; ?>status.php?msgID=<?php echo $postNumber; ?>', '<?php echo $postNumber; ?>')"
                    title="delicious" rel="nofollow">
                    <div class="socialItem">
                        <div class="socialItem_icon"><svg xmlns="http://www.w3.org/2000/svg" x="0px" y="0px" width="40"
                                height="40" viewBox="0 0 192 192" style=" fill:#000000;">
                                <g fill="none" fill-rule="nonzero" stroke="none" stroke-width="1" stroke-linecap="butt"
                                    ...
                                    <g>
                                        <g id="surface1">
                                            <path
                                                d="M24,40c0,-8.79688 7.20312,-16 16,-16h112c8.79688,0 16,7.20312 16,16v112c0,8.79688 -7.20312,16 -16,16h-112c-8.79688,0 -16,-7.20312 -16,-16z"
                                                fill="#eceff1"></path>
                                            <path d="M152,24h-56v72h72v-56c0,-8.79688 -7.20312,-16 -16,-16z"
                                                fill="#448aff"></path>
                                            <path d="M24,152c0,8.79688 7.20312,16 16,16h56v-72h-72z" fill="#37474f">
                                            </path>
                                            <path d="M96,96v72h56c8.79688,0 16,-7.20312 16,-16v-56z" fill="#b0bec5">
                                            </path>
                                        </g>
                                    </g>
                                </g>
            ...
            <!---->
            <?php }?>
            <?php if ($evernoteSocialShareStatus == '1') {?>
            <!---->
            <div class="socialShareButtonItem">
                <a id="twitter"
                    onclick="o_share_social('evernote', '<?php echo $base_url; ?>status.php?msgID=<?php echo $postNumber; ?>', '<?php echo $postNumber; ?>')"
                    title="evernote" rel="nofollow">
                    <div class="socialItem">
                        <div class="socialItem_icon"><svg xmlns="http://www.w3.org/2000/svg" x="0px" y="0px" width="40"
                                height="40" viewBox="0 0 192 192" style=" fill:#000000;">
                                <g fill="none" fill-rule="nonzero" stroke="none" stroke-width="1" stroke-linecap="butt"
            ...
            <!---->
            <?php }?>
            <?php if ($yahooSocialShareStatus == '1') {?>
            <!---->
            <div class="socialShareButtonItem">
                <a id="twitter"
                    onclick="o_share_social('yahoo', '<?php echo $base_url; ?>status.php?msgID=<?php echo $postNumber; ?>', '<?php echo $postNumber; ?>')"
                        ...
                        <div class="socialItem_icon"><svg xmlns="http://www.w3.org/2000/svg" x="0px" y="0px" width="40"
                                ...
                                <g fill="none" fill-rule="nonzero" stroke="none" stroke-width="1" stroke-linecap="butt"
                                    ...
                                    <g fill="#673ab7">
                                        <g id="surface1">
                                            <path
                                                d="M36,20c0,0 5.59375,1.20312 10,1.20312c4.40625,0 10,-1.20312 10,-1.20312l40,68l40.40625,-68c0,0 4.39062,1.59375 9.59375,1.59375c5.59375,0 10,-1.59375 10,-1.59375l-52,88l1.59375,64c0,0 -5.59375,-1.59375 -9.59375,-1.59375c-4,0 -10,1.59375 -10,1.59375l2,-64z">
                                            </path>
                                        </g>
                                    </g>
                                </g>
            ...
            <!---->
            <?php }?>
            <?php if ($gmailSocialShareStatus == '1') {?>
            <!---->
            <div class="socialShareButtonItem">
                <a id="twitter"
                    onclick="o_share_social('gmail', '<?php echo $base_url; ?>status.php?msgID=<?php echo $postNumber; ?>', '<?php echo $postNumber; ?>')"
                    title="gmail" rel="nofollow">
                    <div class="socialItem">
                        <div class="socialItem_icon"><svg xmlns="http://www.w3.org/2000/svg" x="0px" y="0px" width="40"
                                height="40" viewBox="0 0 192 192" style=" fill:#000000;">
                                <g fill="none" fill-rule="nonzero" stroke="none" stroke-width="1" stroke-linecap="butt"
                                    ...
                                    <g>
                                        <g id="surface1">
                                            <path
                                                d="M22,162h148c7.73438,0 14,-6.26562 14,-14v-104c0,-7.73438 -6.26562,-14 -14,-14h-148c-7.73438,0 -14,6.26562 -14,14v104c0,7.73438 6.26562,14 14,14z"
                                                fill="#e0e0e0"></path>
                                            <path
                                                d="M104,162h66c7.73438,0 14,-6.26562 14,-14v-104c0,-7.73438 -6.26562,-14 -14,-14h-148c-7.73438,0 -14,6.26562 -14,14z"
                                                fill="#d9d9d9"></path>
                                            <path d="M26.98438,162h143.01562c7.73438,0 14,-6.26562 14,-14v-102z"
                                                fill="#eeeeee"></path>
                                            <path
                                                d="M102.98438,162h67.01562c7.73438,0 14,-6.26562 14,-14v-102l-108.92188,80.46875z"
                                                fill="#e0e0e0"></path>
                                            <path
                                                d="M170,38h-148c-7.73438,0 -14,-1.73438 -14,6v104c0,7.73438 6.26562,14 14,14h6v-114h136v114h6c7.73438,0 14,-6.26562 14,-14v-104c0,-7.73438 -6.26562,-6 -14,-6z"
                                                fill="#ca3737"></path>
                                            <path
                                                d="M170,30h-148c-7.73438,0 -14,6.14062 -14,14c0,4.82812 6.07812,9.03125 6.07812,9.03125l81.92188,58l81.92188,-58c0,0 6.07812,-4.20313 6.07812,-9.03125c0,-7.85938 -6.26562,-14 -14,-14z"
                                                fill="#f5f5f5"></path>
                                            <path
                                                d="M172.98438,30.32812l-76.98438,53.67188l-76.98438,-53.67188c-6.29687,1.34375 -11.01562,6.85938 -11.01562,13.67188c0,4.82812 6.07812,9.03125 6.07812,9.03125l81.92188,58l81.92188,-58c0,0 6.07812,-4.20313 6.07812,-9.03125c0,-6.8125 -4.71875,-12.32812 -11.01562,-13.67188z"
                                                fill="#e84f4b"></path>
                                        </g>
                                    </g>
                                </g>
            ...
            <!---->
            <?php }?>
            <?php if ($copyPostStatus == '1') {?>
            <!---->
            <div class="socialShareButtonItem">
                <div class="socialItem"
                    onclick="M.toast({html: '<?php echo $page_Lang['link_copied'][$dataUserPageLanguage]; ?>'})">
                    <div class="socialItem_icon copyUrl" data-clipboard-text="<?php echo $sslugSeo; ?>"><svg
                            ...
                            <g fill="none" fill-rule="nonzero" stroke="none" stroke-width="1" stroke-linecap="butt"
                                ...
                                <g fill="#34495e">
                                    <path
                                        d="M32,16c-8.84,0 -16,7.16 -16,16v112h16v-112h112v-16zM64,48c-8.84,0 -16,7.16 -16,16v96c0,8.84 7.16,16 16,16h96c8.84,0 16,-7.16 16,-16v-96c0,-8.84 -7.16,-16 -16,-16zM64,64h96v96h-96z">
                                    </path>
                                </g>
                            </g>
                        </svg></div>
...
<div class="social_share_buttons_container openSocial">
    <div class="shareonSocial">
        <div class="shareonSocialHeader">
            <?php echo $page_Lang['choose_profile_background_color'][$dataUserPageLanguage]; ?> <div
                class="close_icon_sv closethisBox"><svg xmlns="http://www.w3.org/2000/svg" x="0px" y="0px" width="24"
                    height="24" viewBox="0 0 192 192" style=" fill:#000000;">
                    <g fill="none" fill-rule="nonzero" stroke="none" stroke-width="1" stroke-linecap="butt"
                        stroke-linejoin="miter" stroke-miterlimit="10" stroke-dasharray="" stroke-dashoffset="0"
...
$colors = $Dot->Dot_DataColors();
    ...
    <div class="selectedFooter">
        <div class="control right_btn">
            <div class="finishSelectC share_post_box waves-effect waves-light btn blue">
                <?php echo $page_Lang['end_color_selection'][$dataUserPageLanguage]; ?></div>
        </div>
    </div>
</div>
<?php }
...
<div class="social_share_buttons_container openSocial">
    <div class="shareonSocial">
        <div class="shareonSocialHeader"><?php echo $page_Lang['choose_profile_icon_color'][$dataUserPageLanguage]; ?>
            <div class="close_icon_sv closethisBox"><svg xmlns="http://www.w3.org/2000/svg" x="0px" y="0px" width="24"
                    height="24" viewBox="0 0 192 192" style=" fill:#000000;">
                    <g fill="none" fill-rule="nonzero" stroke="none" stroke-width="1" stroke-linecap="butt"
                        stroke-linejoin="miter" stroke-miterlimit="10" stroke-dasharray="" stroke-dashoffset="0"
                        font-family="none" font-weight="none" font-size="none" text-anchor="none"
...
<div class="social_share_buttons_container openSocial">
    <div class="shareonSocial">
        <div class="shareonSocialHeader"><?php echo $page_Lang['choose_profile_text_color'][$dataUserPageLanguage]; ?>
            <div class="close_icon_sv closethisBox"><svg xmlns="http://www.w3.org/2000/svg" x="0px" y="0px" width="24"
                    height="24" viewBox="0 0 192 192" style=" fill:#000000;">
                    <g fill="none" fill-rule="nonzero" stroke="none" stroke-width="1" stroke-linecap="butt"
                        stroke-linejoin="miter" stroke-miterlimit="10" stroke-dasharray="" stroke-dashoffset="0"
...
<div class="social_share_buttons_container openSocial">
    <div class="shareonSocial">
        <div class="shareonSocialHeader"><?php echo $page_Lang['choose_profile_font_style'][$dataUserPageLanguage]; ?>
            <div class="close_icon_sv closethisBox"><svg xmlns="http://www.w3.org/2000/svg" x="0px" y="0px" width="24"
                    height="24" viewBox="0 0 192 192" style=" fill:#000000;">
                    <g fill="none" fill-rule="nonzero" stroke="none" stroke-width="1" stroke-linecap="butt"
                        stroke-linejoin="miter" stroke-miterlimit="10" stroke-dasharray="" stroke-dashoffset="0"
...
// Re Share Post
	...
	if ($activity == 'choosepament') {
		...
		if (isset($_POST['pd']) && isset($_POST['u']) && isset($_POST['ty']) && isset($_POST['amnt'])) {
			...
			$amountsHere = $Dot->Dot_GetProductAmounts($postID, $postOwnerID);
			$thePaymentDonateAmount = $amountsHere['product_normal_price'];
			if ($thePaymentDonateAmount) {
				$theUcardID = mysqli_real_escape_string($db, $_POST['c']);
				$paymentDonateAmount = preg_replace('/[.,]/', '', $thePaymentDonateAmount);
			} else {
				$dntAmount = mysqli_real_escape_string($db, $_POST['amnt']);
				$paymentDonateAmount = preg_replace('/[.,]/', '', $dntAmount);
			}

		}
		//if(!empty($theUcardID)){$userDataCardID = $theUcardID;}else{$userDataCardID = '';}
		$userDataCardID = isset($theUcardID) ? $theUcardID : NULL;
...
//if $val is actually a float
		...
		if ($paymentPostType == 'pm') {
			...
			$paymentAmountPro = preg_replace('/[.,]/', '', $theNewPrice['price_amounth']);
			$originalAmount = $paymentAmountPro;
			$paypalPaymentExchangeRate = 'https://currency-1015.appspot.com/?from=' . $theMainCurrency . '&to=' . $paypalCurrency . '';
			...
			$paypalCalculate = '<div class="donate_exchange_rate"><span class="derate_icon">' . $Dot->Dot_SelectedMenuIcon('down_right') . '</span><div class="derate">' . $currencys[$paypalCurrency] . '' . $paypalAmount . '</div></div>';

			$iyziCoPaymentExchangeRate = 'https://currency-1015.appspot.com/?from=' . $theMainCurrency . '&to=' . $iyziCoCurrency . '';
			...
			$iyziCoCalculate = '<div class="donate_exchange_rate"><span class="derate_icon">' . $Dot->Dot_SelectedMenuIcon('down_right') . '</span><div class="derate">' . $currencys[$iyziCoCurrency] . '' . $iyziCoAmount . '</div></div>';

			$bitPayPaymentExchangeRate = 'https://currency-1015.appspot.com/?from=' . $theMainCurrency . '&to=' . $bitPayCurrency . '';
			...
			$bitPayCalculate = '<div class="donate_exchange_rate"><span class="derate_icon">' . $Dot->Dot_SelectedMenuIcon('down_right') . '</span><div class="derate">' . $currencys[$bitPayCurrency] . '' . $bitPayAmount . '</div></div>';

			$autHorizeNetPaymentExchangeRate = 'https://currency-1015.appspot.com/?from=' . $theMainCurrency . '&to=' . $authorizeNetCurrency . '';
			...
			$autHorizeNetCalculate = '<div class="donate_exchange_rate"><span class="derate_icon">' . $Dot->Dot_SelectedMenuIcon('down_right') . '</span><div class="derate">' . $currencys[$authorizeNetCurrency] . '' . $autHorizeNetAmount . '</div></div>';

			$paystackPaymentExchangeRate = 'https://currency-1015.appspot.com/?from=' . $theMainCurrency . '&to=' . $paystackCurrency . '';
			...
			$paystackCalculate = '<div class="donate_exchange_rate"><span class="derate_icon">' . $Dot->Dot_SelectedMenuIcon('down_right') . '</span><div class="derate">' . $currencys[$paystackCurrency] . '' . $paystackAmount . '</div></div>';

			$stripePaymentExchangeRate = 'https://currency-1015.appspot.com/?from=' . $theMainCurrency . '&to=' . $stripeCurrency . '';
			...
			$stripeCalculate = '<div class="donate_exchange_rate"><span class="derate_icon">' . $Dot->Dot_SelectedMenuIcon('down_right') . '</span><div class="derate">' . $currencys[$stripeCurrency] . '' . $stripeAmount . '</div></div>';

			$razorPayPaymentExchangeRate = 'https://currency-1015.appspot.com/?from=' . $theMainCurrency . '&to=' . $razorPayCurrency . '';
			...
			$razorPayCalculate = '<div class="donate_exchange_rate"><span class="derate_icon">' . $Dot->Dot_SelectedMenuIcon('down_right') . '</span><div class="derate">' . $currencys[$razorPayCurrency] . '' . $razorPayAmount . '</div></div>';
			...
			$productCurrency = isset($getPostCurrency['product_currency']) ? $getPostCurrency['product_currency'] : $theMainCurrency;
			...
			if ($getProductDiscount) {
				...
				$theCurrentPrice = $data_differencePrice;
				$theOldOne = '<div class="cart_price_old"><s>' . $Dot->Dot_restyle_text($getProductDiscount) . '</s></div>';
				$originalAmount = $theCurrentPrice;
			} else {
				$data_dis_Percentage = '';
				$theCurrentPrice = $getProductNormalPrice;
				$theOldOne = '';
				$originalAmount = $theCurrentPrice;
			}
			/****/
			$paypalPaymentExchangeRate = 'https://currency-1015.appspot.com/?from=' . $productCurrency . '&to=' . $paypalCurrency . '';
			...
			$iyziCoPaymentExchangeRate = 'https://currency-1015.appspot.com/?from=' . $productCurrency . '&to=' . $iyziCoCurrency . '';
			...
			$bitPayPaymentExchangeRate = 'https://currency-1015.appspot.com/?from=' . $productCurrency . '&to=' . $bitPayCurrency . '';
			...
			$autHorizeNetPaymentExchangeRate = 'https://currency-1015.appspot.com/?from=' . $productCurrency . '&to=' . $authorizeNetCurrency . '';
			...
			$paystackPaymentExchangeRate = 'https://currency-1015.appspot.com/?from=' . $productCurrency . '&to=' . $paystackCurrency . '';
			...
			$stripePaymentExchangeRate = 'https://currency-1015.appspot.com/?from=' . $productCurrency . '&to=' . $stripeCurrency . '';
			...
			$razorPayPaymentExchangeRate = 'https://currency-1015.appspot.com/?from=' . $productCurrency . '&to=' . $razorPayCurrency . '';
			...
			$razorPayCalculate = '<div class="donate_exchange_rate"><span class="derate_icon">' . $Dot->Dot_SelectedMenuIcon('down_right') . '</span><div class="derate">' . $currencys[$razorPayCurrency] . '' . $razorPayAmount . '</div></div>';
		} else if ($paymentPostType == 'donate') {
			if (!empty($paymentDonateAmount)) {
				...
				$originalAmount = $paymentDonateAmount;
		...
		} else if ($paymentPostType == 'liveDonate') {
			if (!empty($paymentDonateAmount)) {
				$getProductName = 'Donate Live Streaming';
				$getProductDescription = 'I paid for something that I want to live to user.';
				$originalAmount = $paymentDonateAmount;
...
//if $val is actually a float
		...
		$earningDolarThisMonthSum = $Dot->Dot_TotalEarningThisMonthSumAll($uid);

		$sendTotal = truncate($earningDolarThisMonthSum, 2);
		$data = array(
			'total_e' => $currencys[$theMainCurrency] . $Dot->Dot_UpdateTotalEarning($uid, $sendTotal),
		);
		$result = json_encode($data);
		echo preg_replace('/,\s*"[^"]+":null|"[^"]+":null,?/', '', $result);
	}
	...
	if ($activity == 'paypalwithdrawal') {
		if (isset($_POST['paypalEmail']) && isset($_POST['amountPaypal'])) {
			...
			if ($amountPayPal < $minimumWithdrawalAmount) {
			...
			if (!empty($amountPayPal) && !empty($paypalEmail) && $amountPayPal >= $minimumWithdrawalAmount && is_numeric($amountPayPal) && filter_var($paypalEmail, FILTER_VALIDATE_EMAIL) && $amountPayPal <= $dataUserTotalEarnings) {
	...
	if ($activity == 'convertpointtodolar') {
		$dolarEarningFromPoints = number_format($AllTotalPoints, 2, '.', '.') / $pointEqualDollar;
		...
		if ($calculatedPointToDolar) {
			$data = array(
				...
				'dlSum' => $currencys[$theMainCurrency] . $calculatedPointToDolar,
			);
			$result = json_encode($data);
			echo preg_replace('/,\s*"[^"]+":null|"[^"]+":null,?/', '', $result);
		}
	...
	if ($activity == 'transferbalance') {
		if ($dataPontEarnings >= $minimumWithdrawalAmount) {
	...
	if ($activity == 'userfirstinfo') {
		if (isset($_POST['u'])) {
			...
			$Dot->Dot_UpdateUserFStatus($uid, $thisUser);
	...
	// Clear Old Filter Serach
	if ($activity == 'clearfilterusers') {
		$Dot->Dot_ResetOldSearch($uid);
	}
	// Show Filtered Users
	if ($activity == 'srcfltyp') {
		if (isset($_POST['miAge']) && isset($_POST['maxAge']) && isset($_POST['dist']) && isset($_POST['gen']) && isset($_POST['onof']) && isset($_POST['av']) && isset($_POST['relstat']) && isset($_POST['sexlity']) && isset($_POST['heght']) && isset($_POST['weght']) && isset($_POST['lfstyl']) && isset($_POST['chldrn']) && isset($_POST['smkhbt']) && isset($_POST['drnkhbt']) && isset($_POST['hrclr']) && isset($_POST['eyclr']) && isset($_POST['bdyType'])) {
			...
			if ($checkAgeForPro == 1) {
				$miniAge = mysqli_real_escape_string($db, $_POST['miAge']);
				$maxiAge = mysqli_real_escape_string($db, $_POST['maxAge']);
			} else {
				$miniAge = NULL;
				$maxiAge = NULL;
			}
...
<div class="user_c">
    <div class="user_w no<?php echo $userID; ?>" style="background-image:url('<?php echo $dataDistUserAvatar; ?>');">
        <div class="user_flw_btn_in">
            <div class="friend_user icons friend_user_sug<?php echo $userID; ?> iconFollow" id=""
                data-type="<?php echo $followType; ?>" data-id="<?php echo $userID; ?>" data-get="friendSend"
                data-rel="<?php echo $followType; ?>" data-page="suggestion"></div>
        </div>
        <div class="user_in">
            <div class="user_inf">
                <div class="see_profile hvr-underline-from-left sep" data-type="userfirstinfo"
                    data-id="<?php echo $userID; ?>">
                    <div class="sp_i"><?php echo $Dot->Dot_SelectedMenuIcon('see_profle'); ?></div>
                    <?php echo $page_Lang['see_user_profile_in_popup'][$dataUserPageLanguage]; ?>
                </div>
                <div class="user_in_nm truncate">
                    <?php echo $userDistFullName . ',' . $distUserAge . ' ' . $verifiedBadge; ?></div>
                <div class="user_in_dist"><?php echo floor($dd); ?>
                    <?php echo $page_Lang['km_away'][$dataUserPageLanguage]; ?></div>
            </div>
            <span class="user_inf_btns">
                <div class="to_item_u right_to_color getuse" id="chatBox" data-id="<?php echo $userID; ?>"
                    data-user="<?php echo $distUsername; ?>" data-page="normal" data-type="chat">
                    <div class="to_item_icon"><?php echo $Dot->Dot_SelectedMenuIcon('messenger'); ?></div>
                </div>
                <div class="to_item_u tii makeFavourite mf_<?php echo $userID; ?>" id="<?php echo $userID; ?>"
                    data-f="<?php echo $favType; ?>">
                    <div class="to_item_icon" id="f_v_<?php echo $userID; ?>"><?php echo $favoIcon; ?></div>
                </div>
            </span>
        </div>
    </div>
...
/*Send Live Comment*/
	if ($activity == 'sendLiveCom') {
		if (isset($_POST['livec']) && isset($_POST['lcomment'])) {
			$liveID = mysqli_real_escape_string($db, $_POST['livec']);
			$liveComment = mysqli_real_escape_string($db, $_POST['lcomment']);
			$theLiveComment = htmlcode($liveComment);
			if (!empty($liveComment)) {

				$insertLiveComment = $Dot->Dot_InsertiveComment($uid, $liveID, $theLiveComment);

				$liveNewCommentID = $insertLiveComment['live_comment_id'];
				$liveNewCommentUidFk = $insertLiveComment['live_comment_uid_fk'];
				$liveNewComment = $insertLiveComment['live_video_comment'];
				$liveNewCommentUserName = $insertLiveComment['user_name'];
				$liveNewCommentUserFullName = $insertLiveComment['user_fullname'];
				$liveNewCommentVerifiedStatus = $insertLiveComment['verified_user'];
				$liveNewCommentStartStatus = $insertLiveComment['start_live'];
				$cVerifiedBadge = '';
				...
				if ($liveNewCommentID) {
					echo '
			   <div class="gElp9 eo2As cUq_' . $liveNewCommentID . '" id="' . $liveNewCommentID . '">
			  <div class="commentBody_w">
				<!--Commented User Avatar STARTED-->
				<div class="comment_avatar" id="1">
				  <a href="' . $base_url . 'profile/' . $liveNewCommentUserName . '"><img class="_6q-tv" src="' . $liveNewCommentUserAvatar . '" alt="' . $liveNewCommentUserFullName . '"></a>
				</div>
				<!--Commented User Avatar FINISHED-->
				<!--Comment and Username STARTED-->
				<div class="o-MQd simg" id="ed_com_' . $liveNewCommentID . '">
				  <a class="FPmhX" title="' . $liveNewCommentUserFullName . '" href="' . $base_url . 'profile/' . $liveNewCommentUserName . '">' . $liveNewCommentUserFullName . '</a>
				  ' . $cVerifiedBadge . '
				  <div class="commentitem_" data-linkify="this" data-linkify-target="_target">' . sanitize_output($liveNewComment) . '</div>
				  <div class="uv"> </div>
				</div>
	...
	if ($activity == 'disableLive') {
		if (isset($_POST['idc'])) {
			$liveID = mysqli_real_escape_string($db, $_POST['idc']);
			$disableLiveVideo = $Dot->Dot_UpdateLiveStatus($uid, $liveID);
		}
	}
/*Event Delete Request*/
	if ($activity == 'nogoing') {
		if (isset($_POST['e_e'])) {
			$evi = mysqli_real_escape_string($db, $_POST['e_e']);
			$deleteEventRequest = $Dot->Dot_DeleteEventRequest($uid, $evi);
			if ($deleteEventRequest) {
				echo '1';
			} else {
				echo 'Something went wrong';
			}
...
/*Donate Form OPEN*/
	if ($activity == 'donateLiveFrm') {
		if (isset($_POST['donateThis']) && isset($_POST['donateThisu'])) {
			$donateThisPost = mysqli_real_escape_string($db, $_POST['donateThis']);
			$donateThisUser = mysqli_real_escape_string($db, $_POST['donateThisu']);
			$checkAllExist = $Dot->Dot_CheckAllisOkForLive($uid, $donateThisPost, $donateThisUser);
			if ($checkAllExist) {
				include "../contents/html_donate_live_form.php";
			}
		}
	}
/*Live Streaming Comment Status Update*/
	if ($activity == 'securityLiveComment') {
		$privateEnumNumber = array("0", "1");
		if (in_array($_POST['notit'], $privateEnumNumber) && isset($_POST['liveStream'])) {
			$theCLValue = mysqli_real_escape_string($db, $_POST['notit']);
			$liveStramID = mysqli_real_escape_string($db, $_POST['liveStream']);
			$updateLiveStreamCommentStatus = $Dot->Dot_UpdateLiveStreamingCommentStatus($uid, $liveStramID, $theCLValue);
			if ($updateLiveStreamCommentStatus) {
				if ($theCLValue == '1') {
					$lvCStatus = '1';
					$commentsstd = $page_Lang['live_comments_opened'][$dataUserPageLanguage];
				} else {
					$lvCStatus = '0';
					$commentsstd = $page_Lang['live_comments_closed'][$dataUserPageLanguage];
				}
				$data = array(
					'status' => $lvCStatus,
					'note' => $commentsstd,
...
/*Live Streaming Like Status Update*/

	if ($activity == 'securityLiveLike') {
		$privateEnumNumber = array("0", "1");
		if (in_array($_POST['notit'], $privateEnumNumber) && isset($_POST['liveStream'])) {
			$theCLValue = mysqli_real_escape_string($db, $_POST['notit']);
			$liveStramID = mysqli_real_escape_string($db, $_POST['liveStream']);
			$updateLiveStreamCommentStatus = $Dot->Dot_UpdateLiveStreamingLikeStatus($uid, $liveStramID, $theCLValue);
			if ($updateLiveStreamCommentStatus) {
				if ($theCLValue == '1') {
					$lvCStatus = '1';
					$likesstd = $page_Lang['live_likes_opened'][$dataUserPageLanguage];
				} else {
					$lvCStatus = '0';
					$likesstd = $page_Lang['live_likes_closed'][$dataUserPageLanguage];
				}
				$data = array(
					'status' => $lvCStatus,
					'note' => $likesstd,
	...
	if ($activity == 'securityDonatiion') {
		$privateEnumNumber = array("0", "1");
		if (in_array($_POST['notit'], $privateEnumNumber) && isset($_POST['liveStream'])) {
			$theCLValue = mysqli_real_escape_string($db, $_POST['notit']);
			$liveStramID = mysqli_real_escape_string($db, $_POST['liveStream']);
			$updateLiveStreamCommentStatus = $Dot->Dot_UpdateLiveStreamingDonationStatus($uid, $liveStramID, $theCLValue);
			if ($updateLiveStreamCommentStatus) {
				if ($theCLValue == '1') {
					$lvCStatus = '1';
					$donatestd = $page_Lang['donation_opened'][$dataUserPageLanguage];
				} else {
					$lvCStatus = '0';
					$donatestd = $page_Lang['donation_closed'][$dataUserPageLanguage];
				}
				$data = array(
					'status' => $lvCStatus,
					'note' => $donatestd,
	...
	if ($activity == 'livfilt') {
		if (isset($_POST['fil']) && isset($_POST['liveStream'])) {
			$liveVideooFilter = mysqli_real_escape_string($db, $_POST['fil']);
			$livVid = mysqli_real_escape_string($db, $_POST['liveStream']);
			$updateLiveVideoFilter = $Dot->Dot_UpdateLiveVideoFilter($uid, $liveVideooFilter, $livVid);
		}
	}
	if ($activity == 'productedit') {
		if (isset($_POST['title']) && isset($_POST['detail']) && isset($_POST['categories']) && isset($_POST['price']) && isset($_POST['place']) && isset($_POST['tumbs']) && isset($_POST['newCurren']) && isset($_POST['pr'])) {
			$newProductTitle = mysqli_real_escape_string($db, $_POST['title']);
			$newProductDetail = mysqli_real_escape_string($db, $_POST['detail']);
			$newProductCategory = mysqli_real_escape_string($db, $_POST['categories']);
			$newProductPrice = mysqli_real_escape_string($db, $_POST['price']);
			$newProductPlace = mysqli_real_escape_string($db, $_POST['place']);
			$newProductTumbs = mysqli_real_escape_string($db, $_POST['tumbs']);
			$newProductCurrency = mysqli_real_escape_string($db, $_POST['newCurren']);
			$productID = mysqli_real_escape_string($db, $_POST['pr']);
			/*****************/
			function random_code($length) {
				return substr(base_convert(sha1(uniqid(mt_rand())), 16, 36), 0, $length);
			}
			$productDescriptionHtml = htmlcode($newProductDetail);
			if (!empty($newProductTitle)) {
				$slug = 'p_' . random_code(8) . '_' . url_slugies($newProductTitle);
			} elseif (!empty($newProductDetail)) {
				$slug = 'p_' . random_code(8) . '_' . url_slugies($newProductDetail);
			} elseif (empty($newProductTitle) && empty($newProductDetail)) {
				$slug = 'p_' . random_code(8);
			}
			if (!is_numeric($newProductPrice) && empty($newProductPrice)) {
				$theResult = 'Ürün fiyatı geçersiz';
				$theStatus = '1';
			}
			if (empty($newProductTumbs) || empty($newProductTitle) || empty($newProductDetail) || empty($newProductCategory) || empty($newProductPrice) || empty($newProductPlace) || empty($newProductCurrency)) {
				$theResult = 'Tüm alanları doldurun.';
				$theStatus = '2';
			}
			if ($theStatus !== '1' || $theStatus !== '1') {
				$updateMyProductDetails = $Dot->Dot_UpdateMyProductDetails($uid, $newProductTitle, $newProductDetail, $newProductCategory, $newProductPrice, $newProductPlace, $newProductTumbs, $newProductCurrency, $productID, $slug);
				if ($updateMyProductDetails) {
					$done = 'yes';
				} else {
					$done = 'no';
				}
	...
	if ($activity == 'commentGif') {
		if (isset($_POST['gifKeyCo']) && isset($_POST['cp']) && isset($_POST['styp'])) {
			$gifSearchName = mysqli_real_escape_string($db, $_POST['gifKeyCo']);
			$gid = mysqli_real_escape_string($db, $_POST['cp']);
			$type = mysqli_real_escape_string($db, $_POST['styp']);
			if (!empty($gifSearchName)) {
				$file = file_get_contents('https://api.giphy.com/v1/gifs/search?q=' . $gifSearchName . '&api_key=' . $giphyapiKey . '');
				$json = json_decode($file);
				$count = count($json->data);
				if ($count == '0') {
					echo '<div class="no-gif-found">' . $page_Lang['no_gif_founded'][$dataUserPageLanguage] . '</div>';
				...
				for ($i = 0; $i < $count; $i++) {
					$giphyImageUrl = @$json->data[$i]->images->fixed_height->url;
					echo '
					<div class="gif_body" data-id="' . $gid . '" data-fastkey="' . $giphyImageUrl . '" data-post="' . $type . '" data-type="jgif">
