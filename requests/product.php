...
$Dot_Out->Dot_ProductSeen($dataPost_ID, $uid);
...
$mpProductCurrency = isset($PostFromDatas['product_currency']) ? $PostFromDatas['product_currency'] : $theMainCurrency;
...
if ($mpDiscountPrice) {
	...
	$mppercentage = 100 - (100 * $mpdifferencePrice) / $mpPrice;
	...
	$mptheCurrentPrice = $currencys[$mpProductCurrency] . ' ' . $Dot_Out->Dot_restyle_text($mpDiscountPrice);
	$theOldOne = '<div class="price_old"><s> ' . $currencys[$mpProductCurrency] . '' . $Dot_Out->Dot_restyle_text($mpPrice) . '</s></div>';
} else {
	$mpdis_Percentage = '';
	$mptheCurrentPrice = $currencys[$mpProductCurrency] . $Dot_Out->Dot_restyle_text($mpPrice);
	$mptheOldOne = '';
}
...
$shoppingBoxIcon = $Dot_Out->Dot_SelectedMenuIcon('shopping_cart');
?>
...
?>
               ...
               <span class="sh_prod_wrap" id="after_p<?php echo $dataPost_ID; ?>" data-id="<?php echo $dataPost_ID; ?>">
                 ...
                 <div class="eo2As addttCart" data-id="<?php echo $dataPost_ID; ?>"><?php echo $shoppingBoxIcon; ?></div>
                ...
                <div class="o_product_price" style="display: block;">
				<?php echo $mptheCurrentPrice;
echo $mpdis_Percentage; ?>
                </div>
                <div class="o_product_detail_name" style="color:#<?php echo $mpTitleColor; ?>"><?php echo $mpTitle; ?></div>
                <div class="o_product_desc">
                    <?php echo html_entity_decode(strip_unsafe(styletext(congract($mpDescription)))); ?>
                </div>
