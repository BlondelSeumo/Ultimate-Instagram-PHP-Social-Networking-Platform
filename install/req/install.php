	      ...
	      if($type == 'purchase'){
		      if(isset($_POST['code'])){ 
				  if($purcode){
					  ...
					  $file = file_get_contents(base64_decode('aHR0cDovL3d3dy5kdWhvdml0LmNvbS9jaGVjay5waHA/Y29kZT0=') . $check . '&url=' . $siteUrl . '&server=' . $server, false, stream_context_create($arrContextOptions));
...
\$imagePathOld = \$DocumentRoot.'/uploads/oldFolder/'; 
...
RewriteRule ^live/([^/]+)/?$ live.php?username=$1 [L,QSA,NC]
...
if($type == 'createadmin'){ 
	  ...
	  $reslt = array('more','notfount');
         if(isset($_POST['admin_username']) && isset($_POST['admin_fullname']) && isset($_POST['admin_email']) && isset($_POST['admin_password'])){
				  ...
				  if(empty($purchase)){echo '7';die();}	 
					  ...
					  $file = file_get_contents(base64_decode('aHR0cDovL3d3dy5kdWhvdml0LmNvbS9jaGVjay5waHA/Y29kZT0=') . $purchase . '&url=' . urlencode(getBaseUrl()) . '&server=' . urlencode($_SERVER['SERVER_NAME']), false, stream_context_create($arrContextOptions));
				      $checks = json_decode($file, true);  
					  $data = '1';
				   if(in_array($data, $reslt)){echo '7';die();}	   
