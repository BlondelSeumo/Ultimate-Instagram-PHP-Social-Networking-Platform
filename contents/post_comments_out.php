...
$UserCommentedAvatar = $Dot_Out->Dot_UserAvatar($UserCommentUID, $base_url);
/*Check User Liked the Uniq Comment Before*/ 
$CheckUserLikedTheCommendBefore = $Dot_Out->Dot_CheckCommentLiked($uid,$UserCommentID);
...
if($CheckUserLikedTheCommendBefore) {
 ...
 $cVerifiedBadge ='';
$CheckUserVerifiedFromData = $Dot_Out->Dot_CheckUserVerified($UserCommentUID);
if($CheckUserVerifiedFromData){
   $cVerifiedBadge = $Dot->Dot_SelectedMenuIcon('verification_icon');
}
/*Show hide Total Comment Like*/
$TotalUniqCommentLike = $Dot_Out->Dot_UniqCommentLikeCount($UserCommentID);
...
$emoji = $Dot_Out->Dot_ReplaceEmoji($UserCommentText, $base_url);
$fastEmoji = '';
if($emoji){
...
$upostComment = $Dot_Out->GetTheMentions($UserCommentText,$base_url);
$sticker = $Dot_Out->Dot_ReplaceSticker($UserSticker, $base_url);
$stiEmoji ='';
if($sticker){
...
$cGif = $Dot_Out->Dot_ReplaceGif($UserGif, $base_url);
$giEmoji ='';
if($cGif){
   foreach($cGif as $sti){
       $giKey = $sti['emoji_key'];
...
<div class="gElp9 eo2As hov cUq_<?php echo $UserCommentID;?>" id="cUq_<?php echo $UserCommentID;?>" <?php echo $commented;?>>
  ...
  <div class="comment_avatar show_card" id="<?php echo $UserCommentUID;?>" data-user="<?php echo $UserCommentedUserName;?>" data-type="userCard">
    <a href="<?php echo $base_url.'profile/'.$UserCommentedUserName;?>"><img class="_6q-tv" src="<?php echo $UserCommentedAvatar;?>" alt="<?php echo $UserCommentedFullName;?> profil resmi"></a> 
  </div>
  ...
  <div class="o-MQd simg" id="ed_com_<?php echo $UserCommentID;?>" data-comment="<?php echo htmlcode($UserCommentText);?>" data-this="<?php echo $UserCommentID;?>">  
     <a class="FPmhX" title="<?php echo $UserCommentedFullName;?>" href="<?php echo $base_url.'profile/'.$UserCommentedUserName;?>"><?php echo $UserCommentedFullName;?><?php echo $cVerifiedBadge;?></a> 
     <div class="commentitem_" data-linkify="this" data-linkify-target="_target">
     <?php 
		    if($emoji){
			    echo $fastEmoji;
			}else{
			   echo strip_unsafe(styletext($upostComment)); 
			} 
			if($UserSticker){
		       echo $stiEmoji;
			}
     ...
     <?php 
	     if($recordedVoice){
		        $getRecord = $Dot_Out->Dot_GetUploadedAudioID($recordedVoice);
