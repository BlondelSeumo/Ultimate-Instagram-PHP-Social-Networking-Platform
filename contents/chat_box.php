...
if($ChatUserOnlineOffline > $loggedTime) {
  ...
  $VideoAudioButtons = '
  <li>
  <div class="fitAllChat o_others_op" onclick="video_call_manage(0, '.$toUserID.', 0);">
    '.$Dot->Dot_SelectedMenuIcon('audio_call_icon').'
  </div>
  </li>
  <li>
  <div class="o_others_op fitAllChat" onclick="video_call_manage(0,'.$toUserID.','.$data_uid.');">
    '.$Dot->Dot_SelectedMenuIcon('video_call_icon').'
  </div>
  </li>'; 
} else { 
    $online='<span class="_2v6o timeago" title="'.$getLastTime.'"></span>';
	$VideoAudioButtons = '<li><div class="fitAllChat o_others_op">'.$Dot->Dot_SelectedMenuIcon('no_audio_call').'</div></li><li><div class="fitAllChat o_others_op">'.$Dot->Dot_SelectedMenuIcon('no_video_call').'</div></li>';
...
<div class="_1t2u2" id="getMessage" data-message="getMessage" data-type="view" data-user="<?php echo $ChatUserName;?>" data-userid="<?php echo $toUserID;?>">
  ...
  <ul class="_fl2_openMenu">
    ...
    <ul class="_fl2" style="padding:0px;">
      <li> 
        <div class="fitAllChat o_others_op icons_tree aboutChatUser" id="hiddeIt">
          <?php echo $Dot->Dot_SelectedMenuIcon('report_icon');?>
        </div>
      </li>
              ...
              <span class="rightBoxList">
                  ...
                  <div class="currentUserInformation">
                       <span class="currentUserInformationWrap">
                            ...
                            <img src="<?php echo $ChatUserAvatar;?>" width="120" height="120" alt="" class="img_avatar">
                            </div>
                            <div class="_2jnt">
...
<div class="profile_life_info_form">
  ...
  <div class="info_form_items">
    ...
    <div class="info_item_box">
      ...
      <div class="info_item_answer"> 
        <?php if ($ChatUserHeight && $userHeightFeature == '1') {echo $ChatUserHeight . 'cm';}?>
        <?php if ($ChatUserWeight && $userWeightFeature == '1') {echo ',' . $ChatUserWeight . 'kg';}?>
        <?php if ($ChatUserBodyType !== 'unspecified' && $userBodyTypeFeature == '1') {echo ',' . $page_Lang[$ChatUserBodyType][$dataUserPageLanguage];}?>
        <?php if ($ChatUserHairColor !== 'unspecified' && $userHairColorFeature == '1') {echo ',' . $page_Lang[$ChatUserHairColor][$dataUserPageLanguage];}?>
        <?php if ($ChatUserEyeColor !== 'unspecified' && $userEyeColorFeature == '1') {echo $page_Lang[$ChatUserEyeColor][$dataUserPageLanguage];}?> 
        </div>
    </div>
      <?php }?>     
  ...
  <?php if($ChatUserJobTitle || $ChatUserUniversitySchool){?>
  <h1 class="info_title">
  ...
  <div class="info_form_items">
    <div class="business_scholl_university">
      <div class="info_item_box _item_answer">
        <?php if($ChatUserJobTitle){echo $ChatUserJobTitle.',';} echo $ChatUserCompanyName;?> 
      </div>
      
      <?php if($ChatUserUniversitySchool){?>
        <div class="info_item_box _item_answer">
    ...
    <?php  
	   ...
	   if($GetAllUserInterstedItems){
			foreach($GetAllUserInterstedItems as $GetUserInterest){
				...
				$typesArray = array(
				   'type_eating' => "icons_tree_bin eatingicon",
				   'type_music' => "icons_tree_bin musicicon",
				   'type_film_tv' => "icons_tree_bin filmicon",
				   'type_travel' => "icons_tree_bin travelicon",
				   'type_expertise' => "icons_tree_bin businessicon"
				);
				echo '
				   <span class="intr js-intr js-intr-ids-'.$InterestedIDdata.'" id="'.$UserInterestedTypeValue.'"><span class="'.$typesArray[$UserInterestedType].'"></span><span class="intr-txt">'.$page_Lang[$UserInterestedTypeValue][$dataUserPageLanguage].'</span></span>
				'; 
			}
		} else {
      ...
      <a class="icons_tree coreSpriteDesktopSendMenu">
         <span class="Szr5JChC dnone">
            ...
            <div class="sendcbuttons icons_tree vidsendicon"> 
              <?php echo $Dot->Dot_SelectedMenuIcon('chat_svg_video_icon');?>
            ...
            <?php echo $Dot->Dot_SelectedMenuIcon('file_svg_icon');?>
            ...
            <div class="sendcbuttons timesendicon icons_tree"><?php echo $Dot->Dot_SelectedMenuIcon('self_destruction_icon');?></div>
         </span>
      </a></div> 
    </div>
    <div class="_4rv4">
      <div class="XrOeyCh sendmymes"><a class="icons_tree coreSpriteDesktopSendActivity"><?php echo $Dot->Dot_SelectedMenuIcon('fast_send_mesage_icon');?></a></div> 
    </div>
  </div>
</div>
</div>
