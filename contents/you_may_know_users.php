...
if ($SuggestedUserList) {
	...
	foreach ($SuggestedUserList as $sugUser) {
		...
		$sUserVerified = $sugUser['verified_user'];
		...
		$sUverifiedBadge = '';
		if ($sUserVerified == '1') {
			$sUverifiedBadge = $Dot->Dot_SelectedMenuIcon('verification_icon');
		}
		echo '
		 <!--Suggested User STARTED-->
			<div class="suggestedUserBody" id="sgu6">
			  ...
			  <div class="suggestedNameFollower show-user-pro" data-pro="' . $sUid . '" data-proid="' . $sUsername . '">
				<div class="suggestedName"><a href="' . $base_url . 'profile/' . $sUsername . '">' . $sFullName . $sUverifiedBadge . '</a></div>
