...
<span class="ex_text_post_container" id="<?php echo $dataPostID;?>" data-last="<?php echo $dataPostID;?>" data-id="<?php echo $dataPostID;?>" data-ui="<?php echo $dataPostUID;?>">
     <span class="ex_post_wrp ex_post_bor_right"> 
          <?php 
		  if($dataPostWaterMarkID){
				  echo '<div class="post_text_info_watermark_ex post_info'.$dataPostID.'" style="background-image:url('.$base_url.'uploads/watermarkbg/'.$waterMarkImage.'); color:#'.$colorWaterMark.';" id="post_info'.$dataPostID.'" data-linkify="this" data-linkify-target="_target" data-trl="'.$postTextDetails.'">'.strip_unsafe(styletext($postTextDetails)).'</div>';
			  }
		 ?> 
     </span>
