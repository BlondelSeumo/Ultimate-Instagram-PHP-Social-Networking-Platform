...
$dataMarketProductCurrency = isset($PostFromData['product_currency']) ? $PostFromData['product_currency'] : $theMainCurrency;
...
if ($dataMarketProductDiscountPrice) {
	...
	$theCurrentPrice = $currencys[$dataMarketProductCurrency] . ' ' . $Dot->Dot_restyle_text($dataMarketProductDiscountPrice);
	$theOldOne = '<div class="price_old"><s> ' . $currencys[$dataMarketProductCurrency] . '' . $Dot->Dot_restyle_text($dataMarketProductDiscountPrice) . '</s></div>';
} else {
	$data_dis_Percentage = '';
	$theCurrentPrice = $currencys[$dataMarketProductCurrency] . $Dot->Dot_restyle_text($dataMarketProductPrice);
	$theOldOne = '';
}
$userisPro = '';
...
$editProduct = '';
if ($dataPostUID == $uid) {
	$postDeleteButton = '<div class="post_menu_item  del_post" data-post="' . $dataPostID . '" data-u="' . $dataPostCreatedUserName . '" data-type="deletePost"><span class="postMenuicon">' . $Dot->Dot_SelectedMenuIcon('delete_icon') . '</span> ' . $page_Lang['delete_post'][$dataUserPageLanguage] . '</div>';
	if ($postPageType != 'market') {
		$editWhoCanSee = '<div class="post_menu_item  whns" data-id="' . $dataPostID . '"  data-type="whcnse"><span class="postMenuicon dnb">' . $Dot->Dot_SelectedMenuIcon('who_can_see_this_post') . '</span> ' . $page_Lang['change_who_can_see'][$dataUserPageLanguage] . '</div>';
		$disableCommentbutton = '<div class="post_menu_item disable_comment  "  id="cm-ed' . $dataPostID . '" data-cmid="' . $dataPostID . '" data-type="' . $commentDisabledEnabled . '" data-uid="' . $dataPostUID . '"><span class="postMenuicon">' . $Dot->Dot_SelectedMenuIcon('disable_comment_button') . '</span> <span class="cmc' . $dataPostID . '"> ' . $commentDisableTextStatus . '</span></div> ';
		$editPostbutton = '<div class="post_menu_item  edit_post" data-post="' . $dataPostID . '"  data-type="editPost"><span class="postMenuicon">' . $Dot->Dot_SelectedMenuIcon('edit_icon') . '</span> ' . $page_Lang['edit_post_'][$dataUserPageLanguage] . '</div>';
	}
	if ($dataMarketProductPrice) {
		$boostAds = '<div class="post_menu_item  boost_post" data-boost="' . $dataPostID . '"  data-type="boost_product"><span class="postMenuicon">' . $Dot->Dot_SelectedMenuIcon('boost_icon') . '</span> ' . $page_Lang['boost_product'][$dataUserPageLanguage] . '</div>';
		$editProduct = '<a href="' . $base_url . 'product-edit/' . $dataPostID . '"><div class="post_menu_item "><span class="postMenuicon">' . $Dot->Dot_SelectedMenuIcon('edit_icon') . '</span> ' . $page_Lang['edit_product'][$dataUserPageLanguage] . '</div></a>';
	}

}
if ($dataUserType == 1) {
	$postDeleteButton = '<div class="post_menu_item  del_post" data-post="' . $dataPostID . '" data-u="' . $dataPostCreatedUserName . '" data-type="deletePostad"><span class="postMenuicon">' . $Dot->Dot_SelectedMenuIcon('delete_icon') . '</span> ' . $page_Lang['delete_post'][$dataUserPageLanguage] . '</div>';
}
...
if ($dataUserVerified == '1') {
	$verifiedBadge = $Dot->Dot_SelectedMenuIcon('verification_icon');
}
...
if ($dataPostWaterMarkID) {
	...
	$waterMarkImage = isset($patternDetails['watermark_image']) ? $patternDetails['watermark_image'] : NULL;
...
if ($dataPostType == 'p_event') {
	...
	$date2 = new DateTime($post_event_start);
	$interval = $date1->diff($date2);
	$days = $interval->days;
	$post_event_day_left = strtotime($post_event_end_day_number) - strtotime($post_event_start_day_number);
...
if ($dataPostUID != $uid && $donationFeatureStatus != '0' && empty($dataSharedPost)) {
	$postDonateButton = '<div class="post_menu_item  mamount" data-donateid="' . $dataPostID . '"  data-type="donateFrm" data-u="' . $dataPostUID . '"><span class="postMenuicon dnb">' . $Dot->Dot_SelectedMenuIcon('donate_icon') . '</span> ' . $page_Lang['donate_this_post'][$dataUserPageLanguage] . '</div>';
...
echo $userisPro; ?>
    ...
    <!--User Name Started-->
    <div class="o-UQd">
    	<a class="nJAzx" title="<?php echo $dataPostCreatedUserFullName; ?>" href="<?php echo $base_url . 'profile/' . $dataPostCreatedUserName; ?>"><?php echo $dataPostCreatedUserFullName; ?><?php echo $verifiedBadge; ?></a><span style="color:#A8A8A8;font-size:13px;font-weight:500;text-transform: none;">@<?php echo $dataPostCreatedUserName; ?></span><?php echo $sharedFrom; ?></div>
  ...
  <div class="global_post_info_container">
    <span class="fast_icon_answer ic<?php echo $dataPostID; ?>" id="ic<?php echo $dataPostID; ?>"></span>
     <?php
echo $feelingStatus;
if ($dataPostTitleText || $dataPostTitleTextDetails || $postLatitude || $postLongitude) {
	...
	if ($dataPostWaterMarkID) {
		echo '<div class="post_text_info_watermark post_info' . $dataPostID . '" style="background-image:url(' . $base_url . 'uploads/watermarkbg/' . $waterMarkImage . '); color:#' . $colorWaterMark . ';" id="post_info' . $dataPostID . '" data-linkify="this" data-linkify-target="_target" data-trl="' . strip_unsafe(styletext($dataPostTitleTextDetails)) . '">
          ' . html_entity_decode(strip_unsafe(styletext(congract($postTextDetails)))) . '</div>';
	} else if ($postTextDetails) {
		echo '<div class="post_text_info post_info' . $dataPostID . ' ' . $longText . '" id="post_info' . $dataPostID . '" data-linkify="this" data-linkify-target="_target" data-trl="' . strip_unsafe(styletext($dataPostTitleTextDetails)) . '">' . html_entity_decode(strip_unsafe(styletext(congract($postTextDetails)))) . ' ' . $showMoreBtn . '</div>';
	}
...
if ($dataBeforeAfterImages) {
	...
	if ($amazons3UploadStatus == '1') {
		$before = 'https://' . $amazons3UploadBucket . '.s3.' . $amazons3UploadRegion . '.amazonaws.com/images/' . $last1;
		$after = 'https://' . $amazons3UploadBucket . '.s3.' . $amazons3UploadRegion . '.amazonaws.com/images/' . $last2;
	} else {
...
if ($dataPostWhichImage) {
	...
	foreach ($w as $wi) {
		...
		if ($whichImagesFromData) {
			...
			if ($amazons3UploadStatus == '1') {
				$final_image = 'https://' . $amazons3UploadBucket . '.s3.' . $amazons3UploadRegion . '.amazonaws.com/images/' . $whichImagesFromData['uploaded_image'];
			} else {
...
if ($dataMarketProductPrice) {
	...
	echo '
			    <div class="product_more" style="background-color:#' . $color . ';">' . $shoppingBoxIcon . '
					...
					<div class="adtoCardText">' . $page_Lang['add_product_to_cart'][$dataUserPageLanguage] . '</div>
	...
	if ($site) {
		echo '<div class="url_link_img" data-ytkey="' . $dataPostID . '">
				    <div class="url_link_mini_link">' . $dataPostCreatedLinkMiniUrl . '</div>
					    ...
					    ' . $Dot->Dot_SelectedMenuIcon('playVideoBtn') . '
				  </div>
					</div>
					';
	} else {
...
if ($dataPostCreatedVideoID) {
	...
	if ($amazons3UploadStatus == '1') {
		$videoS3Url = 'https://' . $amazons3UploadBucket . '.s3.' . $amazons3UploadRegion . '.amazonaws.com/video/' . $VideoData['video_path'] . '.' . $getVideoExtension;
		$tumbImage = 'https://' . $amazons3UploadBucket . '.s3.' . $amazons3UploadRegion . '.amazonaws.com/video/' . $VideoData['video_path'] . '.png';
		if ($Dot->Dot_CheckImageExistIn($tumbImage)) {
			$tumbImage = $tumbImage;
		} else {
			$tumbImage = $base_url . 'uploads/video/safe_video.png';
		}
	} else {
		$videoS3Url = $base_url . 'uploads/video/' . $VideoData['video_path'] . '.' . $getVideoExtension;
		$tumbImage = $base_url . 'uploads/video/' . $VideoData['video_path'] . '.png';
		if ($Dot->Dot_CheckImageExistIn($tumbImage)) {
			$tumbImage = $tumbImage;
		} else {
			$tumbImage = $base_url . 'uploads/video/safe_video.png';
	...
	echo '
			   ...
			   <!--Media Player STARTED-->
					<div class="media-wrapper videoPlayr" id="videoPlayer' . $dataPostID . '">
						<video class="vidchaVideo mejs__player el_' . $dataPostID . '" poster="' . $tumbImage . '" id="player' . $dataPostID . '" data-id="' . $dataPostID . '" width="100%" height="360"style="max-width:100%;" src="' . $videoS3Url . '" type="video/mp4" controls preload="none"></video>
					</div>
			   <!--Media Player FINISHED-->
			   ' . $startMute . '
			</div>

			<script>fluidPlayer("player' . $dataPostID . '");</script>
			   ';
}
if ($dataPostCreatedAudioID) {
	...
	if ($amazons3UploadStatus == '1') {
		$audoUrl = 'https://' . $amazons3UploadBucket . '.s3.' . $amazons3UploadRegion . '.amazonaws.com/audio/' . $audioPath;
	} else {
		$audoUrl = $base_url . "uploads/auido/" . $audioPath;
	}
	echo '
				<!--Media Player STARTED-->
			   <div class="media-wrapper">
				   <audio class="player_aud" preload="none" controls style="max-width:100%;">
					  <source src="' . $audoUrl . '" type="audio/mp3">
				   </audio>
			   </div>
			   <!--Media Player FINISHED-->
				';
...
if ($dataPostType == 'p_avatar') {
	...
	if ($amazons3UploadStatus == '1') {
		$avatarUploadedUrl = 'https://' . $amazons3UploadBucket . '.s3.' . $amazons3UploadRegion . '.amazonaws.com/avatar/' . $userAvatarPath;
	} else {
		$avatarUploadedUrl = $base_url . 'uploads/avatar/' . $userAvatarPath;
	}
	echo '
				   <div class="item">
				      <img src="' . $avatarUploadedUrl . '"  class="a0uk">
...
if ($dataPostType == 'p_cover') {
	...
	$userCoverUrl = $uploadedAvatar['image_path'];
	if ($amazons3UploadStatus == '1') {
		$CoverUploadedUrl = 'https://' . $amazons3UploadBucket . '.s3.' . $amazons3UploadRegion . '.amazonaws.com/cover/' . $userCoverUrl;
	} else {
		$CoverUploadedUrl = $base_url . 'uploads/cover/' . $userCoverUrl;
	}
	echo '
				   <div class="item">
				      <img src="' . $CoverUploadedUrl . '"  class="a0uk">
...
if ($checkPostAlreadyExist) {
	...
	if ($dataPostWaterMarkID) {
		if ($sharedPostDesc) {$wtStyle = '<div class="post_text_info_watermark post_info' . $dataPostID . '" style="background-image:url(' . $base_url . 'uploads/watermarkbg/' . $waterMarkImage . '); color:#' . $colorWaterMark . ';" id="post_info' . $dataPostID . '" data-linkify="this" data-linkify-target="_target" data-trl="' . $sharedPostDesc . '">' . strip_unsafe(styletext($sharedPostDesc)) . '</div>';} else { $wtStyle = '';}
	} else {
		if ($sharedPostDesc) {
	...
	if ($sTitle || $wtStyle) {
		echo '<div class="post_text_container_shared">';
		if ($sTitle) {
			echo $sTitle;
		}
		if ($wtStyle) {
...
<?php if ($dataPostType != 'p_product') {
    ...
    <div class="here<?php echo $dataPostID; ?>">
      ...
      <?php }?>
      <?php if ($dataPostType != 'p_which' && $dataPostType != 'p_bfaf') {?>
      ...
      <?php }?>
      <?php $os = array("p_which", "p_event");if (!in_array($dataPostType, $os) && $socialShareFeatureStatus != '0') {?>
...
$GetLikedUsers = $Dot->Dot_ShowPostLikedUserAvatar($dataPostID);
    ...
    <span class="dislikeCount dis<?php echo $dataPostID; ?>" style="<?php echo $dislikeStyle; ?>">
       <span class="dis_<?php echo $dataPostID; ?>"><?php echo $totalyPostUnLiked; ?></span> <?php echo $page_Lang['dislike_post'][$dataUserPageLanguage]; ?>
    </span>
    <?php }?>
  </div>
  <!--Comment Sum STARTED-->
...
<!--Comments FINISHED-->
<?php if ($dataCommentDisableStatus == '0' || $dataPostUID == $uid) {?>
<!--Comment This POST STARTED-->
<div class="sH9wk _JgwE  eo2As tag_<?php echo $dataPostID; ?>" style="display:none;">
...
<div class="commentFastButtons h_body<?php echo $dataPostID; ?>" style="display:none;">
   ...
   <div class="recordVoice recordButton" id="record" data-id="<?php echo $dataPostID; ?>">
        <div class="record_btn<?php echo $dataPostID; ?>" style="display:block;"><?php echo $Dot->Dot_SelectedMenuIcon('record'); ?></div>
        <div class="finish_record_btn<?php echo $dataPostID; ?>" id="save" data-vid="<?php echo $dataPostID; ?>" style="display:none;">
         <?php echo $Dot->Dot_SelectedMenuIcon('recording'); ?>
        </div>
   </div>

   <div class="gifs show_this" data-position="top" data-tooltip="Send Gif"data-type="getGifs" data-id="<?php echo $dataPostID; ?>"  data-post="<?php echo $dataPostType; ?>">
      <?php echo $Dot->Dot_SelectedMenuIcon('giphygifs'); ?>
   </div>

<div class="stickers  show_this" data-position="top" data-tooltip="<?php echo $page_Lang['post_a_sticker'][$dataUserPageLanguage]; ?>"data-type="getStickers" data-id="<?php echo $dataPostID; ?>"  data-post="<?php echo $dataPostType; ?>">
   <?php echo $Dot->Dot_SelectedMenuIcon('stickercom'); ?>
</div>

<div class="fast_answericons show_this" data-id="<?php echo $dataPostID; ?>" data-position="top" data-tooltip="<?php echo $page_Lang['fast_emoji_answer'][$dataUserPageLanguage]; ?>" data-type="fastEmoji"  data-post="<?php echo $dataPostType; ?>">
   <?php echo $Dot->Dot_SelectedMenuIcon('fastcom'); ?>
</div>
</div>
