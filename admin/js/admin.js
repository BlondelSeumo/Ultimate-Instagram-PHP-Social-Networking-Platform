// JavaScript Document 
 ...
 //Active page link from settings page
  ...
  $(".selecDefaultLang , .selectTheOption , .selectTheColor , .afterscroll  , .selecDefaultFeeling, .selectTheOptionDayWeekMonthYear , .changeCurrency, .defCurr, .selectDefaultTheme , .sendEmailTo, .regions , .selectCityName").livequery(function() { 
	$(this).formSelect();
  });
  ...
  $("body").on("click", ".save_websitesettings", function(){
	...
	var ffmpeg = $("#tffmpeg").val();
	var datawebdetails = 'f='+type+'&wn='+webname+'&ww='+webkeywords+'&wi='+webinfo+'&wt='+webtitle+'&fffmpeg='+ffmpeg;
  ...
  $("body").on("click", ".delete_this_user", function(){
   ...
   $(document).bind( "mouseup touchend", function(e){
	...
	$("body").on("click", ".change_langeage_keys", function(){
		 ...
		 var new_word = $("#"+id).val();
		 var dataLangChange = 'f='+type+'&this_lng='+language+'&lang_id='+language_id+'&word='+encodeURIComponent(new_word)+'&thisky='+language_key;
	     $.ajax({
			type: 'POST',
			url: requestUrl + 'admin/requests/request', 
			data: dataLangChange,
  ...
  $("body").on("click",".paypalsetting", function(){
	   ...
	   var creditAmount = $("#miminumcrediamount").val();
	   var data = 'f='+type+'&client='+cliend+'&secret='+secret+'&paymode='+paymode+'&minimumAmounth='+creditAmount;
	   $.ajax({
	     type: 'POST',
		 url: requestUrl + 'requests/activity',
		 data: data,
...
// Change Feature Open/Close
 ...
 $("body").on("click", ".tlslss", function(){
  ...
  $("body").on("click", ".savesSmtp", function(){
	  ...
	  var tmial = $("#tmail").val();
	  var data = 'f='+type+'&smtphost='+encodeURIComponent(smtpHost)+'&smtpusername='+encodeURIComponent(smtpUsername)+'&smtppassword='+encodeURIComponent(smtppassword)+'&smtpport='+smtpport+'&tstml='+encodeURIComponent(tmial);
	  $.ajax({
		  type: "POST",
		  data: data,
		  url: requestUrl + "admin/requests/request",
...
//Change Main Language
	$("body").on('click', ".payment_comission_fee", function() { 
	    var type = 'paymenfee'; 
		var nFee = $("#paymenfee").val();
		var minWithdraw = $("#miminumwithdrawlamount").val();
		var data = 'f='+type+'&paymfee='+nFee+'&minwith='+minWithdraw;
		$.ajax({
				type: 'POST',
				url: requestUrl + "admin/requests/request",
				data: data,
				cache: false,
				beforeSend: function() { $("#set_newfeee").append('<span class="preloadCom"><span class="progress"><span class="indeterminate"></span></span></span>'); },
				success: function(response) {  
   ...
   // Show Ordered Post Details
   $("body").on("click",".ordDetails", function(){
	  var type = $(this).attr("data-type"); 
	  var orderID = $(this).attr("data-or");
	  var productID = $(this).attr("data-id");
	  var buyer = $(this).attr("data-ui"); 
	  var data = 'f='+type + '&ordid='+orderID+'&prodID='+productID+'&buyerID='+buyer;
	  $.ajax({
			type: 'POST',
			url: requestUrl + "admin/requests/request",
			data: data,
			cache: false,
			beforeSend: function() { $(".sam_"+buyer).append('<span class="preloadCom"><span class="progress"><span class="indeterminate"></span></span></span>'); },
			success: function(response) {  
   ...
   $("body").on("click", ".deleteOrder", function(){
      var userID = $(this).attr("id");//Get Comment id
	  var thePopUpType = $(this).attr("data-type"); 
	  var popUpComment = 'popUp='+thePopUpType+'&anid='+userID;
	  $.ajax({
		  type: "POST",
		  url: requestUrl + 'requests/popups',
		  data: popUpComment,
  ...
  /*Delete Order*/
  $("body").on("click", ".delete_this_order", function(){
      var orderID = $(this).attr("id");//Get Comment id
	  var thePopUpType = $(this).attr("data-type");  
	  var data = 'f='+thePopUpType+'&orderid='+orderID;
	  $.ajax({
		  type: "POST",
		  url: requestUrl + 'admin/requests/request',
		  data: data,
     ...
     /*Delete Order PopUp*/
   $("body").on("click", ".deleteOrderFromCart", function(){
      var userID = $(this).attr("id");//Get Comment id
	  var thePopUpType = $(this).attr("data-type"); 
	  var popUpComment = 'popUp='+thePopUpType+'&anid='+userID;
	  $.ajax({
    ...
    /*Delete Order*/
  $("body").on("click", ".delete_this_order_from_cart", function(){
      var orderID = $(this).attr("id");//Get Comment id
	  var thePopUpType = $(this).attr("data-type");  
	  var data = 'f='+thePopUpType+'&crtid='+orderID;
	  $.ajax({
		  type: "POST",
		  url: requestUrl + 'admin/requests/request',
		  data: data,
 ...
 // Smtp Mail
 $("body").on("click", ".liveStreamWith", function(){
       var type = 'liveWidth';
	   var val = $(this).attr("data-val");
	   var data = 'f='+type+'&liveStreamw='+val;
	   $.ajax({
		  type: "POST",
		  data: data,
		  url: requestUrl + "admin/requests/request",
		  cache: false,
		  beforeSend: function() {$("#lvwdth").append('<span class="preloadCom"><span class="progress"><span class="indeterminate"></span></span></span>');},
		  success: function(response) {
			if (val == "1") { 
			     $(".usingtype").remove();
			     setTimeout(function() {
  ...
  $("body").on("change",".defCurr", function(){
	var type = $(this).attr("data-type"); 
	var currency = $(this).val();  
	var data = 'f='+type+'&crncy='+currency;
	$.ajax({
