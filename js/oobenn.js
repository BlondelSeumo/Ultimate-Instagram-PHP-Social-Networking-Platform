...
$(document).ready(function() {
    $("body").on("click", ".post_modal_wral_close_co", function() {
        $(".mnup").addClass('postButtonItemOut');
        setTimeout(function() {
    ...
    AutoTiseTextArea();
    $(".timeago , .postCreatedTime , .comti , .profileRegisteredTime , .ltime , .announceTime , .reqTime").livequery(function() {
        $(this).timeago();
    });
    ...
    $("body").on("focus", ".addComment", function() {
        var ID = $(this).attr("id");
        $(".h_body" + ID).css({ 'position': 'relative', 'width': '100%' });
    });
    //See Other Comments
    ...
    $("body").on("click", ".save_profile_info", function() {
        ...
        var u_usrnme = $("#register_username").val();
        var dataProfileInfo = 'f=' + type + '&fn=' + ful_name + '&web=' + webAd + '&ub=' + encodeURIComponent(u_bio) + '&lw=' + encodeURIComponent(l_word) + '&unm=' + encodeURIComponent(u_usrnme);
        //Check url is valid
    ...
    $('body').on('change', '#change', function(e) {
        ...
        $("#avatarForm").ajaxForm({
            ...
            success: function(response) {
                ...
                $(".profile_avatar_image_container").css('background-image', 'url(' + response + ')');
                $(".primgh").css('background-image', 'url(' + response + ')');
    ...
    $('body').on('change', '#changeCover', function(e) {
        ...
        $("#coverForm").ajaxForm({
            ...
            success: function() {
                ...
                var getImaageURL = $(".CoverImage").css("background-image").replace(/^url\(['"](.+)['"]\)/, '$1');
                setTimeout(function() {
                    $(".profile_coverBgGradient").css('background-image', 'url(' + getImaageURL + ')');
                }, 600);
            },
            error: function() {
    ...
    function showMoreData() {
        ...
        if (scrollLoad && $(window).scrollTop() + window.innerHeight >= document.body.scrollHeight) {
            ...
            if (PageType == 'more_friends') { var ID = $('#morePostType').children('.UserCardContainer').last().attr('data-last'); var profile = $("#morePostType").attr("data-profile"); }
            ...
            if (PageType == 'more_product') { var ID = $('#moreProduct').children('.zMhqu').last().attr('data-last'); var productType = $('#morePostType').attr('data-product'); }
            if ($('#preloadMore').length === 0) {
                var dataString = "lastmid=" + ID + '&f=' + PageType + '&p=' + profile + '&tag=' + hashtag + '&story=' + story + '&event=' + eventID + '&e_ty=' + eventType + '&theme=' + tem + '&c=' + productType;
                $.ajax({
                    type: "POST",
                    url: requestUrl + 'requests/activity',
                    data: dataString,
                    ...
                    success: function(response) {
                        if (response) {
                            ...
                            $("#preloadMore").remove();
                            if (PageType == 'more_feeds' || PageType == 'more_explore_vid' || PageType == 'more_profilePosts' || PageType == 'more_favourites') {
                                //AutoVideoPlaying();
                            }
                        }
                        if (response.trim() == '') {
    ...
    $(document).bind("mouseup touchend", function(e) {
        ...
        var postBoxes = $(".newNotificationTest");
        if (!postBoxes.is(e.target) && postBoxes.has(e.target).length === 0) {
            $(".newNotificationTest").remove();
        }
                    ...
                    $.ajax({
                        ...
                        success: function(response) {
                            if (response) {
                                ...
                                $(".menlist" + ID).hide().html('');
                            }
                        }
                    });
                }
            } else {
                $(".mentionUserList").hide().html('');
            }
        }, 500);
    });
    ...
    $("body").on("click", ".gifs", function() {
        ...
        if (childNodes.length) {
            ...
            if (width >= 600) {
                $(".u_p" + ID).append('<div class="pop pop_sticker" style="position:absolute; right:20px; bottom:50px;padding-top:0px;"></div>');
            }
            if (width <= 600) {
                $(".u_p" + ID).after('<div class="popMobile pop_sticker"></div>');
            }
    ...
    function gifData() {
        var type = 'commentGif';
        var box = $('.postGif').val();
        var post_StickerID = $('.postGif').attr("data-id");
        var styp = $('.postGif').attr("data-styp");
        contentbox = $.trim(box);
        var dataString = 'f=' + type + '&gifKeyCo=' + box + '&cp=' + post_StickerID + '&styp=' + styp;
        if (contentbox !== '') {
            $.ajax({
                type: "POST",
                url: requestUrl + "requests/activity",
    ...
    $("body").on("click", ".movg", function() {
        ...
        $.ajax({
            ...
            data: data,
            beforeSend: function() {
                $(".newnotifications_").prepend("<div class='prog-colm-which panel-flex crnm'><div class='prog-bar-colm'><div class='progressmi'><span class='indeterminatespost'></span></div></div></div>");
            },
            success: function(response) {
                if (response == 1) {
                    $("#iev" + eventID).fadeOut();
    ...
    $("body").on("click", ".share_product", function() {
        ...
        var productCurrency = $('.currency').val();
        ...
        var dataNewImage = 'curncy=' + productCurrency + '&ppric=' + encodeURIComponent(productPrice) + '&pti=' + encodeURIComponent(productTitle) + '&prid=' + encodeURIComponent(productDescription) + '&prCat=' + productCategory + '&uploads=' + UploadedImageValues + '&mpro=' + type + '&mstatus=' + productMessageStatus + '&place=' + productPlace;
        $.ajax({
            type: 'POST',
            url: requestUrl + 'requests/event',
            data: dataNewImage,
    ...
    $("body").on("click", ".donate_post", function() {
        ...
        var donateAmount = '';
        var data = 'f=' + f + '&pd=' + postID + '&u=' + post_o_i + '&ty=' + data_type + '&amnt=' + donateAmount + '&c=' + cid;
        $.ajax({
            type: 'POST',
            url: requestUrl + 'requests/activity',
            data: data,
    ...
    /*Donate Amount Box*/
    $("body").on("click", ".dlimam", function() {
        var type = 'donateLiveFrm';
        var postID = $("#live_post_id").val();
        var post_o_i = $(this).attr('data-u');
        var data_type = $(this).attr('data-type');
        var data = 'f=' + type + '&donateThis=' + postID + '&donateThisu=' + post_o_i + '&ty=' + data_type;
        $.ajax({
    ...
    /*Send Live Video Comment*/
    $("body").on("click", ".sendLiveComment", function() {
        if ($('#live_post_id').val()) {
            var type = 'sendLiveCom';
            var live = $("#live_post_id").val();
            var liveCom = $(".live_comment_m").val();
            var data = 'f=' + type + '&livec=' + live + '&lcomment=' + encodeURIComponent(liveCom);
            $.ajax({
                type: 'POST',
                url: requestUrl + 'requests/activity',
                data: data,
                beforeSend: function() {},
                success: function(response) {
                    $(".all_live_comments").append(response);
                    $(".live_comment_m").val('');
                }
            });
        }
    });
    $('.live_comment_m').bind('keydown', function(e) {
        var key = e.which || e.keyCode || 0;
        if (key == 13) {
            if ($('#live_post_id').val()) {
                var type = 'sendLiveCom';
    ...
    $("body").on("click", ".inLeft", function() {
        if (!$(".leftSticky , .rightSticky").hasClass("inMob")) {
            $(".leftSticky , .rightSticky").addClass("inMob");
            $('.leftSidebarHide').show();
            $('body').addClass('disableScroll');
        } else {
            $(".leftSticky , .rightSticky").removeClass("inMob");
            $('.leftSidebarHide').css({ 'display': '' });
            $('body').removeClass('disableScroll');
        }
    });
    /*Like Live Video */
    $("body").on("click", ".sendLiveLike", function() {
        var type = "likeLiveVideo";
        var lvID = $("#live_post_id").val();
