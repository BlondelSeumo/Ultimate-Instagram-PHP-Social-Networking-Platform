...
$pagesDefaultLanguage = isset($duhovit['default_lang']) ? $duhovit['default_lang'] : 'english';
...
$dataCountry = isset($data['country']) ? $data['country'] : NULL;
$dataCountryPhone = $data['country'];
/*if(!$dataCountry){
  $res = file_get_contents('https://www.iplocate.io/api/lookup/' . $uip);
  $res = json_decode($res);
  $userCountrycode = isset($res->country_code) ? $res->country_code : NULL;
  $userLatitude = isset($res->latitude) ? $res->latitude : NULL;
  $userLongitude = isset($res->longitude) ? $res->longitude : NULL;
  if($userCountrycode){
	$Dot->Dot_UpdateUserCountry($uid, $userCountrycode, $uip, $userLatitude, $userLongitude);
  } 
}*/
$LiveVideos = $Dot->Dot_FriendsLiveVideoStreams($uid); 
$oliveStyle = '';
if($LiveVideos){
  $oliveStyle = 'allLivVideosPlus';
}
...
$dataUsercanShareText = $data['can_post_text'];
$dataUsercanShareImage = $data['can_post_image'];
$dataUsercanShareVideo = $data['can_post_video'];
$dataUsercanShareLink = $data['can_post_link'];
$dataUsercanShareAudio = $data['can_post_audio'];
$dataUsercanShareFilter = $data['can_post_filters'];
$dataUsercanShareGiphy = $data['can_post_giphy'];
$dataUsercanShareLocation = $data['can_post_location'];
$dataUsercanShareWatermark = $data['can_post_watermark'];
$dataUsercanShareWhichImage = $data['can_post_which'];
$dataUsercanShareProduct = $data['can_post_product'];
$dataUsercanShareBeforeAfter = $data['can_post_bfaf'];
...
if ($dataVerifiedUserStatus == 1) {
	$thisisVerifiedMember = $Dot->Dot_SelectedMenuIcon('verification_icon');
}
...
$Dot->Dot_UpdateLiveStatusIfExist($uid);
...
$defaultTestEmail = isset($duhovit['test_email']) ? $duhovit['test_email'] : NULL;
$watermarkStatus = $duhovit['watermark_status'];
...
$earningsDolarLiveBroadCastingThisMonth = $Dot->Dot_TotalLiveEarningThisMonthDolar($uid);
...
$earningDolarThisMonthMarket = $Dot->Dot_TotalEarningThisMonthDolarMarket($uid);
//PayPal
...
//Stripe
$stripePaymentModeData = $duhovit['stripe_payment_mode'];
if ($stripePaymentModeData == 1) {
	$stripePaymentMode = true;
} else {
...
$donationFeatureStatus = $duhovit['donation_feature'];
//Point Sum
...
/*Social Share buttons Status*/

$socialShareFeatureStatus = $duhovit['social_share_status'];
$facebookShareStatus = $duhovit['s_facebook'];
$twitterShareStatus = $duhovit['s_twitter'];
$tumblrShareStatus = $duhovit['s_tumblr'];
$pinterestShareStatus = $duhovit['s_pinteres'];
$vkSocialShareStatus = $duhovit['s_vk'];
$diggSocialShareStatus = $duhovit['s_digg'];
$linkedinSocialShareStatus = $duhovit['s_linkedin'];
$redditSocialShareStatus = $duhovit['s_reddit'];
$whatsappSocialShareStatus = $duhovit['s_whatsapp'];
$viberSocialShareStatus = $duhovit['s_viber'];
$deliciousSocialShareStatus = $duhovit['s_delicious'];
$evernoteSocialShareStatus = $duhovit['s_evernote'];
$yahooSocialShareStatus = $duhovit['s_yahoo'];
$gmailSocialShareStatus = $duhovit['s_gmail'];
$copyPostStatus = $duhovit['s_copy'];

$agoreStatus = $duhovit['agora_status'];
$agoraAppID = $duhovit['agora_app_id'];
$agoraCustomerID = $duhovit['agora_customer_id'];
$agoraCertificate = $duhovit['agora_certificate'];

$liveVideoStatus = $duhovit['live_video_status'];
$saveLiveVideos = $duhovit['save_live_videos'];
$saveAmazonLiveVideos = $duhovit['save_live_videos_on_amazon'];
$liveStreamingVideoWithStatus = $duhovit['streaming_video_with'];

$amazonS3Status = $duhovit['amazone_s3_2'];
$amazonS3BucketName = $duhovit['bucket_name_2'];
$amazonS3Key = $duhovit['amazone_s3_key_2'];
$amazonS3SecretKey = $duhovit['amazone_s3_s_key_2'];
$amazonS3Region = $duhovit['region_2'];

$amazons3UploadStatus = $duhovit['s3_status'];
$amazons3UploadSecretKey = $duhovit['s3_secret_key'];
$amazons3UploadKey = $duhovit['s3_key'];
$amazons3UploadBucket = $duhovit['s3_bucket'];
$amazons3UploadRegion = $duhovit['s3_region']; 
 
$theMainCurrency = $duhovit['default_currency'];
$currencys = array(
	"USD" => '$',
	"AUD" => '$',
	"EUR" => '€',
...
$minimumWithdrawalAmount = $duhovit['min_withdrawal_amount'];
...
$websiteFFMPEGPath = $duhovit['ffmpeg_path'];
$sexualityFeature = $duhovit['ed_sexuality'];
$userHeightFeature = $duhovit['ed_height'];
$userWeightFeature = $duhovit['ed_weight'];
$userLifeStyleFeature = $duhovit['ed_lifestyle'];
$userChildrenFeature = $duhovit['ed_children'];
$userSmokingFeature = $duhovit['ed_smoking'];
$userDrinkingFeature = $duhovit['ed_drinking'];
$userBodyTypeFeature = $duhovit['ed_body_type'];
$userHairColorFeature = $duhovit['ed_hair_color'];
$userEyeColorFeature = $duhovit['ed_eye_color'];
$styleMode = 'style';
$day_night_icon = 'night';
$DayNigtMode = isset($page_Lang['night_mode'][$dataUserPageLanguage]) ? $page_Lang['night_mode'][$dataUserPageLanguage] : $page_Lang['night_mode']['english'];
if ($dataPageStyleMode == 2) {
	$styleMode = 'night_style';
	$DayNigtMode = isset($page_Lang['day_mode'][$dataUserPageLanguage]) ? $page_Lang['day_mode'][$dataUserPageLanguage] : $page_Lang['day_mode']['english'];
	$day_night_icon = 'day';
}
...
$dolarEarningFromPoints = '';
if (!empty($AllTotalPoints)) {
...
if ($CheckUserDeleteStatus == '3') {
	$deleteKey = $Dot->Dot_GetDeleteKey($uid);
	$blocked = $yourAccountDeleted . '?delete=' . $deleteKey;}
