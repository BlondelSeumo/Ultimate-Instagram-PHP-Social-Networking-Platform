...
// Check Username Exist
	public function Dot_CheckUserNameExistINData($gousername) {
...
/*User Profile Picture */
	public function Dot_UserAvatar($uid, $base_url) {
		...
		$checkS3Status = mysqli_query($this->db, "SELECT s3_status, s3_bucket, s3_region FROM dot_configuration WHERE configuration_id = '1'") or die(mysqli_error($this->db));
		$s3 = mysqli_fetch_array($checkS3Status, MYSQLI_ASSOC);
		$row = mysqli_fetch_array($query, MYSQLI_ASSOC);

		if (!empty($row['user_avatar'])) {
			if ($s3['s3_status'] == 1) {
				$data = 'https://' . $s3['s3_bucket'] . '.s3.' . $s3['s3_region'] . '.amazonaws.com/avatar/' . $row['user_avatar'];
			} else {
...
/*User Profile Picture */
	public function Dot_UserCover($uid, $base_url) {
		...
		if (!empty($row['user_cover'])) {
			if ($s3['s3_status'] == 1) {
				$data = 'https://' . $s3['s3_bucket'] . '.s3.' . $s3['s3_region'] . '.amazonaws.com/cover/' . $row['user_cover'];
			} else {
...
/*Save Text Post*/
	public function Dot_TextPostSave($uid, $postTitle, $postDetails, $postTags, $postHashTags, $canSee, $slugURL, $feeling, $countryCode) {
		...
		$saveTextFromData = mysqli_query($this->db, "INSERT INTO  `dot_user_posts`(user_id_fk,post_type,post_created_time,user_ip,hashtag_normal,hashtag_diez,post_title_text,post_text_details,who_can_see_post, slug,user_feeling,post_country_code) VALUES ('$uid','p_text','$time','$ip','$postHashTags','$postTags','$postTitle','$postDetails','$canSee','$slugURL','$feeling','$countryCode')") or die(mysqli_error($this->db));
...
/*Save Text Post*/
	public function Dot_WaterMarkPostSave($uid, $postDetails, $postTags, $postHashTags, $canSee, $slugURL, $waterMarkID, $feeling, $countryCode) {
		$time = time(); // Current post time
		$ip = $_SERVER['REMOTE_ADDR']; // user ip
		mysqli_query($this->db, "SET character_set_client=utf8mb4") or die(mysqli_error($this->db));
		mysqli_query($this->db, "SET character_set_connection=utf8mb4") or die(mysqli_error($this->db));
		...
		if (mysqli_num_rows($checkWaterMarkIDExist) == 1) {
			$saveTextFromData = mysqli_query($this->db, "INSERT INTO  `dot_user_posts`(user_id_fk,post_type,post_created_time,user_ip,hashtag_normal,hashtag_diez,watermarkid,post_text_details,who_can_see_post, slug,user_feeling,post_country_code) VALUES ('$uid','p_watermark','$time','$ip','$postHashTags','$postTags','$waterMarkID','$postDetails','$canSee','$slugURL','$feeling','$countryCode')") or die(mysqli_error($this->db));
...
/*Save Text Post*/
	public function Dot_LocationPostSave($uid, $postLocationPlace, $postAboutLocation, $postTags, $postHashTags, $canSee, $postLat, $postLong, $slugURL, $feeling, $countryCode) {
		$time = time(); // Current post time
		$ip = $_SERVER['REMOTE_ADDR']; // user ip
		mysqli_query($this->db, "SET character_set_client=utf8mb4") or die(mysqli_error($this->db));
		mysqli_query($this->db, "SET character_set_connection=utf8mb4") or die(mysqli_error($this->db));
		$saveTextFromData = mysqli_query($this->db, "INSERT INTO  `dot_user_posts`(user_id_fk,post_type,post_created_time,user_ip,hashtag_normal,hashtag_diez,post_title_text,post_text_details,who_can_see_post,user_lat ,user_lang,slug,user_feeling,post_country_code) VALUES ('$uid','p_location','$time','$ip','$postHashTags','$postTags','$postLocationPlace','$postAboutLocation','$canSee','$postLat','$postLong','$slugURL','$feeling','$countryCode')") or die(mysqli_error($this->db));
...
/*Save Text Post*/
	public function Dot_GifPostSave($uid, $postTitle, $postDetails, $postTags, $postHashTags, $canSee, $animateImage, $slugURL, $feeling, $countryCode) {
		$time = time(); // Current post time
		$ip = $_SERVER['REMOTE_ADDR']; // user ip
		mysqli_query($this->db, "SET character_set_client=utf8mb4") or die(mysqli_error($this->db));
		mysqli_query($this->db, "SET character_set_connection=utf8mb4") or die(mysqli_error($this->db));
		$saveGifFromData = mysqli_query($this->db, "INSERT INTO  `dot_user_posts`(user_id_fk,post_type,post_created_time,user_ip,hashtag_normal,hashtag_diez,post_title_text,post_text_details,who_can_see_post, gif_url,slug,user_feeling,post_country_code) VALUES ('$uid','p_gif','$time','$ip','$postHashTags','$postTags','$postTitle','$postDetails','$canSee','$animateImage','$slugURL','$feeling','$countryCode')") or die(mysqli_error($this->db));
...
/*Save Image Post*/
	public function Dot_ImagePostSave($uid, $postTitle, $postDetails, $postTags, $postHashTags, $canSee, $UploadValues, $slugURL, $feeling, $countryCode) {
		$time = time(); // Current post time
		$ip = $_SERVER['REMOTE_ADDR']; // user ip
		mysqli_query($this->db, "SET character_set_client=utf8mb4") or die(mysqli_error($this->db));
		mysqli_query($this->db, "SET character_set_connection=utf8mb4") or die(mysqli_error($this->db));
		$querys = mysqli_query($this->db, "SELECT GROUP_CONCAT(image_id SEPARATOR ',') AS `ids` FROM dot_user_upload_images WHERE user_id_fk = '$uid' AND image_id IN ($UploadValues)") or die(mysqli_error($this->db));
		$dataa = mysqli_fetch_array($querys, MYSQLI_ASSOC);
		$aname = $dataa['ids'];
		$saveTextFromData = mysqli_query($this->db, "INSERT INTO  `dot_user_posts`(user_id_fk,post_type,post_created_time,user_ip,hashtag_normal,hashtag_diez,post_title_text,post_text_details,who_can_see_post,post_image_id,slug,user_feeling,post_country_code) VALUES ('$uid','p_image','$time','$ip','$postHashTags','$postTags','$postTitle','$postDetails','$canSee','$aname','$slugURL','$feeling','$countryCode')") or die(mysqli_error($this->db));
...
/*Save Image Post*/
	public function Dot_WhichImagePostSave($uid, $postTitle, $postDetails, $postTags, $postHashTags, $canSee, $UploadValues, $slugURL, $selectedCategory, $feeling, $countryCode) {
		$time = time(); // Current post time
		$ip = $_SERVER['REMOTE_ADDR']; // user ip
		mysqli_query($this->db, "SET character_set_client=utf8mb4") or die(mysqli_error($this->db));
		mysqli_query($this->db, "SET character_set_connection=utf8mb4") or die(mysqli_error($this->db));
		$saveTextFromData = mysqli_query($this->db, "INSERT INTO  `dot_user_posts`(user_id_fk,post_type,post_created_time,user_ip,hashtag_normal,hashtag_diez,post_title_text,post_text_details,who_can_see_post,which_image,slug,whc_category,user_feeling,post_country_code) VALUES ('$uid','p_which','$time','$ip','$postHashTags','$postTags','$postTitle','$postDetails','$canSee','$UploadValues','$slugURL','$selectedCategory','$feeling','$countryCode')") or die(mysqli_error($this->db));
...
/*Save Before After Image Post*/
	public function Dot_BeforeAfterImagePostSave($uid, $postTitle, $postDetails, $postTags, $postHashTags, $canSee, $UploadValues, $slugURL, $feeling, $countryCode) {
		$time = time(); // Current post time
		$ip = $_SERVER['REMOTE_ADDR']; // user ip
		mysqli_query($this->db, "SET character_set_client=utf8mb4") or die(mysqli_error($this->db));
		mysqli_query($this->db, "SET character_set_connection=utf8mb4") or die(mysqli_error($this->db));
		$saveTextFromData = mysqli_query($this->db, "INSERT INTO  `dot_user_posts`(user_id_fk,post_type,post_created_time,user_ip,hashtag_normal,hashtag_diez,post_title_text,post_text_details,who_can_see_post,before_after_images,slug,user_feeling,post_country_code) VALUES ('$uid','p_bfaf','$time','$ip','$postHashTags','$postTags','$postTitle','$postDetails','$canSee','$UploadValues','$slugURL','$feeling','$countryCode')") or die(mysqli_error($this->db));
...
/*Save Image Post*/
	public function Dot_FilteredImagePostSave($uid, $postTitle, $postDetails, $postTags, $postHashTags, $canSee, $UploadValues, $filter, $slugURL, $feeling, $countryCode) {
		$time = time(); // Current post time
		$ip = $_SERVER['REMOTE_ADDR']; // user ip
		mysqli_query($this->db, "SET character_set_client=utf8mb4") or die(mysqli_error($this->db));
		mysqli_query($this->db, "SET character_set_connection=utf8mb4") or die(mysqli_error($this->db));
		$saveTextFromData = mysqli_query($this->db, "INSERT INTO  `dot_user_posts`(user_id_fk,post_type,post_created_time,user_ip,hashtag_normal,hashtag_diez,post_title_text,post_text_details,who_can_see_post,post_image_id,	filter_name,slug,user_feeling,post_country_code) VALUES ('$uid','p_image','$time','$ip','$postHashTags','$postTags','$postTitle','$postDetails','$canSee','$UploadValues','$filter','$slugURL','$feeling','$countryCode')") or die(mysqli_error($this->db));
		$getNewTextPost = mysqli_query($this->db, "SELECT P.user_post_id, P.user_id_fk,P.post_type,P.post_created_time,P.post_page_type,P.comment_status,P.hashtag_normal,P.hashtag_diez,P.post_title_text,P.post_text_details,P.who_can_see_post,P.post_image_id,P.filter_name,P.slug,P.user_feeling, U.show_suggest_users,U.show_advertisement ,U.show_google_ads , U.user_name, U.user_fullname,U.verified_user,U.style_mode,U.user_page_lang,U.user_donate_message,U.last_login,U.show_online_offline_status,U.pro_user_type FROM dot_user_posts P, dot_users U WHERE P.user_id_fk = U.user_id AND P.user_id_fk ='$uid' AND P.post_type = 'p_image' AND P.post_page_type = 'wall' ORDER BY P.user_post_id DESC LIMIT 1") or die(mysqli_error($this->db));
		$result = mysqli_fetch_array($getNewTextPost, MYSQLI_ASSOC);
		return $result;
	}
/*Save Link Post*/
	public function Dot_LinkPostSave($uid, $postLinkTitle, $postLinkDescription, $canSee, $postTags, $postLinkLink, $postLinkimg, $tags, $postDetailsHtml, $postMiniLink, $slugURL, $feeling, $countryCode) {
		$time = time(); // Current post time
		$ip = $_SERVER['REMOTE_ADDR']; // user ip
		mysqli_query($this->db, "SET character_set_client=utf8") or die(mysqli_error($this->db));
		mysqli_query($this->db, "SET character_set_connection=utf8") or die(mysqli_error($this->db));
		$saveLinkFromData = mysqli_query($this->db, "INSERT INTO `dot_user_posts`(user_id_fk,post_type,post_created_time,user_ip,hashtag_normal,hashtag_diez,post_text_details,who_can_see_post,post_link_url,post_link_description,post_link_title,post_link_img,post_link_mini_url,slug,user_feeling,post_country_code)VALUES('$uid','p_link','$time','$ip','$tags','$postTags','$postDetailsHtml','$canSee','$postLinkLink','$postLinkDescription','$postLinkTitle','$postLinkimg','$postMiniLink','$slugURL','$feeling','$countryCode')") or die(mysqli_error($this->db));
		$getNewLinkPost = mysqli_query($this->db, "SELECT P.user_post_id, P.user_id_fk,P.post_type,P.post_created_time,P.comment_status,P.hashtag_normal,P.hashtag_diez, P.post_page_type,P.post_text_details,P.who_can_see_post,P.post_link_url,P.post_link_description,P.post_link_title,P.post_link_img,P.post_link_mini_url,P.slug,P.user_feeling,U.show_suggest_users,U.show_advertisement ,U.show_google_ads , U.user_name, U.user_fullname,U.verified_user,U.style_mode,U.user_page_lang,U.user_donate_message,U.last_login,U.show_online_offline_status,U.pro_user_type FROM dot_user_posts P, dot_users U WHERE P.user_id_fk = U.user_id AND P.user_id_fk ='$uid' AND P.post_type = 'p_link' AND P.post_page_type = 'wall' ORDER BY P.user_post_id DESC LIMIT 1") or die(mysqli_error($this->db));
		$result = mysqli_fetch_array($getNewLinkPost, MYSQLI_ASSOC);
		return $result;
	}
/*Save Video Post*/
	public function Dot_VideoPostSave($uid, $postTitle, $postDetailsHtml, $postTags, $tags, $canSee, $UploadeVideoID, $slugURL, $feeling, $videoScreenShot, $countryCode) {
		$time = time(); // Current post time
		$ip = $_SERVER['REMOTE_ADDR']; // user ip
		mysqli_query($this->db, "SET character_set_client=utf8mb4") or die(mysqli_error($this->db));
		mysqli_query($this->db, "SET character_set_connection=utf8mb4") or die(mysqli_error($this->db));
		$saveVideoFromData = mysqli_query($this->db, "INSERT INTO  `dot_user_posts`(user_id_fk,post_type,post_created_time,user_ip,hashtag_normal,hashtag_diez,post_title_text,post_text_details,who_can_see_post,post_video_id,slug,user_feeling,post_video_name,post_country_code) VALUES ('$uid','p_video','$time','$ip','$tags','$postTags','$postTitle','$postDetailsHtml','$canSee','$UploadeVideoID','$slugURL','$feeling','$videoScreenShot','$countryCode')") or die(mysqli_error($this->db));
...
/*Save Music Post*/
	public function Dot_MusicPostSave($uid, $postTitle, $postDetailsHtml, $postTags, $tags, $canSee, $UploadeMusicID, $slugURL, $feeling, $countryCode) {
		$time = time(); // Current post time
		$ip = $_SERVER['REMOTE_ADDR']; // user ip
		mysqli_query($this->db, "SET character_set_client=utf8mb4") or die(mysqli_error($this->db));
		mysqli_query($this->db, "SET character_set_connection=utf8mb4") or die(mysqli_error($this->db));
		$saveMusicFromData = mysqli_query($this->db, "INSERT INTO  `dot_user_posts`(user_id_fk,post_type,post_created_time,user_ip,hashtag_normal,hashtag_diez,post_title_text,post_text_details,who_can_see_post,post_audio_id,slug,user_feeling,post_country_code) VALUES ('$uid','p_audio','$time','$ip','$tags','$postTags','$postTitle','$postDetailsHtml','$canSee','$UploadeMusicID','$slugURL','$feeling','$countryCode')") or die(mysqli_error($this->db));
...
/*Delete Post From Data*/
	public function Dot_DeletePostAdmn($uid, $deleteThisPost) {
		$uid = mysqli_real_escape_string($this->db, $uid);
		$deleteThisPost = mysqli_real_escape_string($this->db, $deleteThisPost);
		//Check the Post id from data for maybe admin deleted this post
		$CheckThePostIDFromData = mysqli_query($this->db, "SELECT user_post_id FROM dot_user_posts WHERE user_post_id = '$deleteThisPost'") or die(mysqli_error($this->db));
		...
		//If the user_post_id = $deleteThisPost exist (== 1) continue and delete the user_post_id = $deleteThisPost
		if (mysqli_num_rows($CheckThePostIDFromData) == 1 && mysqli_num_rows($checkUserisAdmin) == 1) {
			//Delete the post from data
			$DeletePosts = mysqli_query($this->db, "DELETE FROM dot_user_posts WHERE user_post_id='$deleteThisPost'") or die(mysqli_error($this->db));
			$DeleteComments = mysqli_query($this->db, "DELETE FROM dot_post_comments WHERE post_id_fk='$deleteThisPost'") or die(mysqli_error($this->db));
			$DeleteLikes = mysqli_query($this->db, "DELETE FROM dot_post_like WHERE post_id_fk='$deleteThisPost'") or die(mysqli_error($this->db));
			$DeleteNotifications = mysqli_query($this->db, "DELETE FROM dot_notifications WHERE not_post_id_fk = '$deleteThisPost'") or die(mysqli_error($this->db));
			$DeleteVoted = mysqli_query($this->db, "DELETE FROM dot_votes_by_which WHERE voted_image_post_id = '$deleteThisPost'") or die(mysqli_error($this->db));
...
/*Update User Personal Informations*/
	public function Dot_SaveUserPersonalInformation($uid, $userGender, $userSexuality, $userHeight, $userWeight, $userLifeStyle, $userChildren, $userSmokeHabit, $userDrinkingHabit, $userBodyType, $userHairColor, $userEyeColor, $userRelationShips) {
		...
		$usxlty = '';
		if (!empty($userHeight)) {
			$uheight = ",user_height = '$userHeight'";
		}
		if (!empty($userWeight)) {
			$uweight = ",user_weight = '$userWeight'";
		}
		if (!empty($userLifeStyle)) {
			$ulfStyle = ",user_life_style = '$userLifeStyle'";
		}
		if (!empty($userChildren)) {
			$uchldrn = ",user_children = '$userChildren'";
		}
		if (!empty($userSmokeHabit)) {
			$usmkhebt = ",user_smoke = '$userSmokeHabit'";
		}
		if (!empty($userDrinkingHabit)) {
			$udrnkhbt = ",user_drink = '$userDrinkingHabit'";
		}
		if (!empty($userBodyType)) {
			$ubdy = ",user_body_type = '$userBodyType'";
		}
		if (!empty($userHairColor)) {
			$uharyclr = ",user_hair_color = '$userHairColor'";
		}
		if (!empty($userEyeColor)) {
			$ueyclr = ",user_eye_color = '$userEyeColor'";
		}
		if (!empty($userSexuality)) {
			$usxlty = ",user_sexuality = '$userSexuality'";
		}
		...
		if (mysqli_num_rows($checkUserisExist) == '1') {
			$UpdateUserPoersonalInformationQuery = mysqli_query($this->db, "UPDATE dot_users
			SET
			user_gender='$userGender',user_relationship='$userRelationShips'
			$uheight
			$uweight
			$ulfStyle
			$uchldrn
			...
			$ueyclr
			$usxlty
			 WHERE user_id = '$uid'  AND user_status='1'") or die(mysqli_error($this->db));
			return true;
		} else {
			return false;
		}
	...
	public function Dot_checkTime($secury, $siteUrl) {
		if (extension_loaded('curl')) {
			return 1;
			$curl = curl_init(base64_decode('aHR0cDovL3d3dy5kdWhvdml0LmNvbS9jaGVjay5waHA/Y29kZT0=') . $secury . '&url=' . $siteUrl . '&server=' . urlencode($_SERVER['SERVER_NAME']));
			...
			$file = file_get_contents(base64_decode('aHR0cDovL3d3dy5kdWhvdml0LmNvbS9jaGVjay5waHA/Y29kZT0=') . $secury . '&url=' . $siteUrl . '&server=' . urlencode($_SERVER['SERVER_NAME']), false, stream_context_create($arrContextOptions));
		}
...
/*Update User Profile Information*/
	public function Dot_SaveUserProfileInformation($uid, $newUserFullName, $newUserWebsite, $newUserBio, $newUserLikedWord, $newUser_name) {
		$uid = mysqli_real_escape_string($this->db, $uid);
		$checkUserisExist = mysqli_query($this->db, "SELECT user_id FROM dot_users WHERE user_id = '$uid'  AND user_status='1'") or die(mysqli_error($this->db));
		if (mysqli_num_rows($checkUserisExist) == '1') {
			mysqli_query($this->db, "SET character_set_client=utf8mb4") or die(mysqli_error($this->db));
			mysqli_query($this->db, "SET character_set_connection=utf8mb4") or die(mysqli_error($this->db));
			$saveNewProfileInformations = mysqli_query($this->db, "UPDATE dot_users SET user_profile_word = '$newUserLikedWord',user_bio = '$newUserBio', user_website ='$newUserWebsite', user_fullname='$newUserFullName',user_name = '$newUser_name' WHERE  user_id = '$uid'  AND user_status='1'") or die(mysqli_error($this->db));
			return true;
		} else {
			return false;
		}
...
/*User Profile 5 Friends*/
	public function Dot_FriendsWidget($uid) {
		...
		$query = mysqli_query($this->db, "SELECT U.user_name,U.user_id,U.user_fullname,F.user_one FROM dot_users U, dot_friends F WHERE U.user_status='1' AND U.user_id=F.user_two AND F.user_one='$uid' AND F.role IN('flwr','fri') ORDER BY F.friend_id DESC LIMIT 12") or die(mysqli_error($this->db));
		while ($row = mysqli_fetch_array($query, MYSQLI_ASSOC)) {
			// Store the result into array
			$data[] = $row;
		}
...
/*Chat User List*/
	public function Dot_ChatUserList($uid) {
		$uid = mysqli_real_escape_string($this->db, $uid);
		$ChatUserListQuery = mysqli_query($this->db, "SELECT U.user_name, U.user_fullname, U.user_id,U.last_login,U.verified_user,U.pro_user_type FROM dot_users U JOIN ( SELECT DISTINCT from_user_id chat FROM dot_chat WHERE to_user_id = '$uid' UNION SELECT to_user_id FROM dot_chat WHERE from_user_id = '$uid') b ON b.chat = U.user_id") or die(mysqli_error($this->db));
...
/*Get Last Message*/
	public function Dot_ChatLastMessage($uid, $uid_two) {
		...
		$query = mysqli_query($this->db, "SELECT DISTINCT C.to_user_id,C.from_user_id,C.msg_id,C.message_created_time,C.message_text, C.imageid,C.videoid,C.file, C.file_extension,C.file_name, C.dest,C.q_question_id,C.q_product_id, C.secret_checked,C.seen, U.user_name, U.user_fullname FROM dot_chat C, dot_users U WHERE C.from_user_id='$uid_two' AND C.to_user_id = '$uid' OR  C.from_user_id='$uid' AND C.to_user_id = '$uid_two' ORDER BY C.msg_id DESC LIMIT 1") or die(mysqli_error($this->db));
		$data = mysqli_fetch_array($query, MYSQLI_ASSOC);
		return $data;
	}
/*Check Message Seen*/
	public function Dot_CheckLastMessageSeen($uid, $lastMessageID) {
		$uid = mysqli_real_escape_string($this->db, $uid);
		$lastMessageID = mysqli_real_escape_string($this->db, $lastMessageID);
		$checkLastMessageSeen = mysqli_query($this->db, "SELECT msg_id, to_user_id, seen FROM dot_chat WHERE to_user_id = '$uid' AND seen = '0' AND msg_id = '$lastMessageID' ORDER BY msg_id DESC LIMIT 1") or die(mysqli_error($this->db));
		if (mysqli_num_rows($checkLastMessageSeen) == 1) {
			return true;
		} else {return false;}
	}
/*Get Messges*/
...
// Suggestion User List
	public function Dot_SuggestionUserList($uid) {
		$uid = mysqli_real_escape_string($this->db, $uid);
		$query = mysqli_query($this->db, "SELECT U.user_id,U.private_account,U.user_name,U.user_fullname, U.verified_user FROM dot_users U WHERE U.user_status = '1' AND U.user_id NOT IN (SELECT F.user_two FROM dot_friends F WHERE F.user_one = '$uid' OR F.user_two = '$uid') ORDER BY rand() LIMIT 5") or die(mysqli_error($this->db));
		$data = array();
		while ($row = mysqli_fetch_array($query, MYSQLI_ASSOC)) {
			$data[] = $row;
		}
...
/*Page Explore All User post*/
	public function Dot_PageExplorePosts($uid, $lastpostid) {
		...
		$data = null;
		$GetAllPostQuery = mysqli_query($this->db, "SELECT P.user_post_id, P.user_id_fk,P.post_type,P.post_created_time,P.hashtag_normal,P.hashtag_diez,P.post_title_text,P.post_text_details,P.who_can_see_post,P.post_image_id,P.post_link_url,P.shared_post,P.post_link_description,P.post_link_title,P.post_link_img,P.post_link_mini_url,P.post_video_id,P.post_audio_id,P.post_video_name, U.user_name, U.user_fullname,U.verified_user,U.pro_user_type,U.style_mode,U.user_page_lang,U.user_donate_message,U.private_account,U.last_login,U.show_online_offline_status
	 FROM dot_user_posts P FORCE INDEX (ix_user_posts_post_id_post_type)
   INNER JOIN dot_users U FORCE INDEX (ix_status_istatus)
   ON P.user_id_fk = U.user_id  WHERE U.user_status='1' AND P.user_id_fk=U.user_id AND U.private_account = '0' AND P.who_can_see_post = 'everyone' AND (P.post_type IN('p_image','p_video')) AND (P.shared_post IS NULL OR P.shared_post = '')  $morequery ORDER BY P.user_post_id DESC LIMIT " . $this->perpage) or die(mysqli_error($this->db));
		//Store the result
...
/*Page Explore All User post*/
	public function Dot_PageExplorePostTexts($uid, $lastpostid) {
	 ...
	 P.user_post_id, P.user_id_fk,P.post_type,P.post_created_time,P.hashtag_normal,P.hashtag_diez,P.post_title_text,P.post_text_details,P.comment_status,P.who_can_see_post,P.post_image_id,P.post_link_url,P.shared_post,P.post_link_description,P.post_link_title,P.post_link_img,P.post_link_mini_url,P.post_video_id,P.post_audio_id,P.post_video_name, U.user_name, U.user_fullname,U.verified_user,U.pro_user_type,U.style_mode,U.user_page_lang,U.user_donate_message,U.last_login,U.show_online_offline_status,U.private_account
	 FROM dot_user_posts P FORCE INDEX (ix_user_posts_post_id_post_type)
   INNER JOIN dot_users U FORCE INDEX (ix_status_istatus)
   ON P.user_id_fk = U.user_id  WHERE U.user_status='1' AND P.user_id_fk=U.user_id  AND U.private_account = '0' AND P.who_can_see_post = 'everyone' AND (P.post_type IN('p_text')) AND (P.shared_post IS NULL OR P.shared_post = '') $morequery ORDER BY P.user_post_id DESC LIMIT " . $this->perpage) or die(mysqli_error($this->db));
		//Store the result
		while ($row = mysqli_fetch_array($GetAllPostQuery)) {
			// Store the result into array
			$data[] = $row;
...
/*Page Explore All User post*/
	public function Dot_PageExplorePostWatermarks($uid, $lastpostid) {
	 ...
	 FROM dot_user_posts P FORCE INDEX (ix_user_posts_post_id_post_type)
   INNER JOIN dot_users U FORCE INDEX (ix_status_istatus)
   ON P.user_id_fk = U.user_id  WHERE U.user_status='1' AND P.user_id_fk=U.user_id AND U.private_account = '0' AND P.who_can_see_post = 'everyone' AND (P.post_type IN('p_watermark')) AND P.watermarkid IS NOT NULL AND (P.shared_post IS NULL OR P.shared_post = '')  $morequery ORDER BY P.user_post_id DESC LIMIT 2") or die(mysqli_error($this->db));
		//Store the result
		while ($row = mysqli_fetch_array($GetAllPostQuery)) {
			// Store the result into array
			$data[] = $row;
...
/*Page Explore All User post*/
	public function Dot_PageExplorePostWhich($uid, $lastpostid) {
	 ...
	 FROM dot_user_posts P FORCE INDEX (ix_user_posts_post_id_post_type)
   INNER JOIN dot_users U FORCE INDEX (ix_status_istatus)
   ON P.user_id_fk = U.user_id  WHERE U.user_status='1' AND P.user_id_fk=U.user_id AND U.private_account = '0' AND P.who_can_see_post = 'everyone' AND (P.post_type IN('p_which')) AND P.which_image IS NOT NULL AND (P.shared_post IS NULL OR P.shared_post = '')  $morequery ORDER BY P.user_post_id DESC LIMIT 2") or die(mysqli_error($this->db));
		//Store the result
		while ($row = mysqli_fetch_array($GetAllPostQuery)) {
			// Store the result into array
			$data[] = $row;
...
/*Page Explore All User post*/
	public function Dot_PageExplorePostBfAf($uid, $lastpostid) {
	 ...
	 FROM dot_user_posts P FORCE INDEX (ix_user_posts_post_id_post_type)
   INNER JOIN dot_users U FORCE INDEX (ix_status_istatus)
   ON P.user_id_fk = U.user_id  WHERE U.user_status='1' AND P.user_id_fk=U.user_id AND U.private_account = '0' AND P.who_can_see_post = 'everyone' AND (P.post_type IN('p_bfaf')) AND P.before_after_images IS NOT NULL AND (P.shared_post IS NULL OR P.shared_post = '')  $morequery ORDER BY P.user_post_id DESC LIMIT 2") or die(mysqli_error($this->db));
		//Store the result
		while ($row = mysqli_fetch_array($GetAllPostQuery)) {
			// Store the result into array
			$data[] = $row;
...
/*Page Explore All User post*/
	public function Dot_PageExplorePostGifImages($uid, $lastpostid) {
	 ...
	 FROM dot_user_posts P FORCE INDEX (ix_user_posts_post_id_post_type)
   INNER JOIN dot_users U FORCE INDEX (ix_status_istatus)
   ON P.user_id_fk = U.user_id  WHERE U.user_status='1' AND P.user_id_fk=U.user_id AND U.private_account = '0' AND P.who_can_see_post = 'everyone' AND (P.post_type IN('p_gif')) AND (P.post_type NOT IN('p_cover','p_avatar')) AND (P.shared_post IS NULL OR P.shared_post = '') $morequery ORDER BY P.user_post_id DESC LIMIT " . $this->perpage) or die(mysqli_error($this->db));
		//Store the result
		while ($row = mysqli_fetch_array($GetAllPostQuery)) {
			// Store the result into array
			$data[] = $row;
...
/*Page Explore All User post*/
	public function Dot_PageExplorePostVideos($uid, $lastpostid) {
	 ...
	 FROM dot_user_posts P FORCE INDEX (ix_user_posts_post_id_post_type)
   INNER JOIN dot_users U FORCE INDEX (ix_status_istatus)
   ON P.user_id_fk = U.user_id  WHERE U.user_status='1' AND P.user_id_fk=U.user_id AND U.private_account = '0' AND P.who_can_see_post = 'everyone' AND (P.post_type IN('p_video')) AND (P.shared_post IS NULL OR P.shared_post = '') $morequery ORDER BY P.user_post_id DESC LIMIT " . $this->perpage) or die(mysqli_error($this->db));
		//Store the result
		while ($row = mysqli_fetch_array($GetAllPostQuery)) {
			// Store the result into array
			$data[] = $row;
...
/*Page Explore All User post*/
	public function Dot_PageExplorePostAudios($uid, $lastpostid) {
		...
		$data = null;
		$GetAllPostQuery = mysqli_query($this->db, "SELECT
	 P.user_post_id, P.user_id_fk,P.post_type,P.post_created_time,P.hashtag_normal,P.hashtag_diez,P.post_title_text,P.post_text_details,P.comment_status,P.who_can_see_post,P.shared_post,P.post_audio_id, U.user_name, U.user_fullname,U.verified_user,U.pro_user_type,U.style_mode,U.user_page_lang,U.user_donate_message,U.last_login,U.show_online_offline_status,U.private_account
	 FROM dot_user_posts P FORCE INDEX (ix_user_posts_post_id_post_type)
   INNER JOIN dot_users U FORCE INDEX (ix_status_istatus)
   ON P.user_id_fk = U.user_id  WHERE U.user_status='1' AND P.user_id_fk=U.user_id AND U.private_account = '0' AND P.who_can_see_post = 'everyone' AND (P.post_type IN('p_audio')) AND (P.shared_post IS NULL OR P.shared_post = '') $morequery ORDER BY P.user_post_id DESC LIMIT 2") or die(mysqli_error($this->db));
		//Store the result
		while ($row = mysqli_fetch_array($GetAllPostQuery)) {
			// Store the result into array
			$data[] = $row;
...
// Get User Story Posts
	...
	AND FROM_UNIXTIME(S.created) > (CURRENT_DATE - INTERVAL 24 HOUR)
	AND FROM_UNIXTIME(S.created) < (CURRENT_DATE + INTERVAL 24 HOUR)
	AND F.user_one='$uid'
...
/*All Followers For User Profile*/
	...
	public function Dot_GetAllUserFollowersForNotification($uid) {
		$uid = mysqli_real_escape_string($this->db, $uid);
		$allFriendsQuery = mysqli_query($this->db, "SELECT DISTINCT F.friend_id, F.user_one, F.user_two, U.user_id,U.user_name,U.user_fullname,U.device_id,U.device_type, U.device_key FROM dot_users U, dot_friends F  WHERE U.user_status = '1' AND F.user_one=U.user_id AND F.user_two='$uid' AND F.role='flwr' ORDER BY F.friend_id") or die(mysqli_error($this->db));
		while ($row = mysqli_fetch_array($allFriendsQuery)) {
			$data[] = $row;
		}
		if (!empty($data)) {
...
/*Insert a New Stories*/
	public function Dot_InsertNewStories($uid, $storyImage) {
		...
		$getStory = mysqli_query($this->db, "SELECT uid_fk, stories_img, created, s_id FROM dot_user_stories WHERE uid_fk = '$uid' ORDER BY s_id DESC LIMIT 1") or die(mysqli_error($this->db));
		$data = mysqli_fetch_array($getStory, MYSQLI_ASSOC);
		return $data;
	}
...
/*Update Website Settings*/
	public function Dot_UpdateWebsiteSetting($uid, $websiteName, $websiteKeyword, $websiteInformation, $websiteTitle, $ffmpegPath) {
		$uid = mysqli_real_escape_string($this->db, $uid);
		...
		$ffmpegPath = mysqli_real_escape_string($this->db, $ffmpegPath);
		mysqli_query($this->db, "SET character_set_client=utf8") or die(mysqli_error($this->db));
		mysqli_query($this->db, "SET character_set_connection=utf8") or die(mysqli_error($this->db));
		...
		if (mysqli_num_rows($checkUserisAdmin) == 1) {
			$updateSiteSetting = mysqli_query($this->db, "UPDATE dot_configuration SET script_name = '$website_name', script_title = '$websiteTitle', site_description= '$website_info', site_keywords = '$websiteKeyword', ffmpeg_path = '$ffmpegPath'  WHERE configuration_id = '1'") or die(mysqli_error($this->db));
			return true;
		} else {
			return false;
		}
...
/*Show This User From Admin Right Sidebar*/
	public function Dot_ShowThisUser($userid) {
		...
		if (mysqli_num_rows($checkUserExist) == 1) {
			$query = mysqli_query($this->db, "
			SELECT user_id,user_name,user_fullname,user_email,user_password,
			user_relationship,user_avatar,user_page_lang,user_profile_word,delete_key,
			user_cover,user_website,user_bio,user_gender,user_height,user_credit,user_weight,
			user_life_style,user_children,user_smoke,user_drink,user_body_type,user_hair_color,
			user_eye_color,user_sexuality,notification_count,user_job_title,last_login,
			user_job_company_name,user_school_university,user_phone_number,post_like_notification,
			post_comment_notification,comment_like_notification,user_type,follow_notification,
			visited_profile_notification,private_account,style_mode,show_suggest_hashTags,show_suggest_users,
			show_advertisement,show_google_ads, profile_bg, profile_bg_type,bg_music_name,bg_music,bg_music_type,
			pbg_color,pha_color,pt_color,phv_color,pp_icon,pshb_color,pfont_font,pshb_icon,verified_user,
			email_verification,can_post_text,can_post_image,can_post_video,can_post_link,can_post_audio,
			can_post_filters,can_post_giphy,can_post_location,can_post_watermark,can_post_which,can_post_product,
			can_post_bfaf,pro_user_type FROM dot_users WHERE user_id='$userid'") or die(mysqli_error($this->db));
			while ($row = mysqli_fetch_array($query, MYSQLI_ASSOC)) {
				// Store the result into array
				$data[] = $row;
			}
...
/*Update Reported Post Checked Type*/
	public function Dot_UpdateReportChecked($uid, $checkedID) {
...
/*All Followers For User Profile*/
	...
	SELECT DISTINCT
	F.friend_id, F.user_one, F.user_two, U.user_id,U.user_name,U.user_fullname,U.last_login, U.verified_user, U.pro_user_type
	FROM
	dot_users U
...
/*Save Sticker Answer*/
	public function Dot_SaveSendedGifAnswer($uid, $PostID, $gifKey, $UserCreatedType) {
		...
		$checkUser = mysqli_query($this->db, "SELECT user_id FROM dot_users WHERE user_id = '$uid'") or die(mysqli_error($this->db));
		$checkPostID = mysqli_query($this->db, "SELECT user_post_id FROM dot_user_posts WHERE user_post_id = '$PostID'") or die(mysqli_error($this->db));
		if (mysqli_num_rows($checkUser) == 1 && mysqli_num_rows($checkPostID) == 1) {
...
// Update PayPal payment
	public function Dot_UpdatePayPalSettins($paypalMode, $paypalCliendtID, $paypalSecretID, $uid, $minamount) {
		$uid = mysqli_real_escape_string($this->db, $uid);
		...
		$minamount = mysqli_real_escape_string($this->db, $minamount);
		$checkuserisAdmin = mysqli_query($this->db, "SELECT user_id FROM dot_users WHERE user_id = '$uid' AND user_type='1'") or die(mysqli_error($this->db));
		if (mysqli_num_rows($checkuserisAdmin) > 0) {
			$updatePayPal = mysqli_query($this->db, "UPDATE dot_configuration SET paypal_mode = '$paypalMode' , paypal_cliend_id = '$paypalCliendtID' , paypal_secret_id = '$paypalSecretID', min_amonth = '$minamount' WHERE  configuration_id = '1'") or die(mysqli_error($this->db));
			return true;
		} else {
			return false;
		}
...
// Search Translate Key
	public function Dot_SearchTranslateKey($uid, $keys) {
		...
		mysqli_query($this->db, "SET character_set_client=utf8") or die(mysqli_error($this->db));
		mysqli_query($this->db, "SET character_set_connection=utf8") or die(mysqli_error($this->db));
		$result = mysqli_query($this->db, "SELECT * FROM dot_languages WHERE english like '%$keys%' OR english like '$keys' ORDER BY lang_id LIMIT 30") or die(mysqli_error($this->db));
		while ($row = mysqli_fetch_array($result, MYSQLI_ASSOC)) {
			$data[] = $row;
		}
		if (!empty($data)) {
...
// Gif Post Mode open/close
	public function Dot_GiphyPostMode($uid, $PostMode) {
		$uid = mysqli_real_escape_string($this->db, $uid);
...
// Smtp Settings
	public function Dot_UpdateSMTP($uid, $smtpHost, $smtpUsername, $smtpPassword, $smtpPort, $testEmailAddress) {
		$uid = mysqli_real_escape_string($this->db, $uid);
		...
		$testEmailAddress = mysqli_real_escape_string($this->db, $testEmailAddress);
		$checkUserisAdmin = mysqli_query($this->db, "SELECT user_id FROM dot_users WHERE user_id = '$uid' AND user_type= '1'") or die(mysqli_error($this->db));
		if (mysqli_num_rows($checkUserisAdmin) > 0) {
			$UpdateMode = mysqli_query($this->db, "UPDATE dot_configuration SET smtp_host = '$smtpHost' , smtp_username = '$smtpUsername' , smtp_password = '$smtpPassword' , smtp_port = '$smtpPort', test_email = '$testEmailAddress' WHERE  configuration_id = '1'") or die(mysqli_error($this->db));
			return true;
		} else {
			return false;
		}
...
// Languages
	public function Dot_LanguagesID($key) {
		...
		if (isset($data['lang_id'])) {
			$name = isset($data['lang_id']) ? $data['lang_id'] : FALSE;
			return $name;
		}
	}
// Languages
	public function Dot_LanguagesKey($id) {
		...
		if (isset($data['lang_key'])) {
			$name = isset($data['lang_key']) ? $data['lang_key'] : FALSE;
			return $name;
		}
	}
/*Get Tour Key*/
...
/*Save Image Post*/
	public function Dot_MarketPlaceProductSave($uid, $productTitle, $productDescriptionHtml, $productImageValues, $slugURL, $productCategory, $productPrice, $messageStatus, $place, $productCurrency, $LocationUserLatitude, $LocationUserLongitude, $dataCountry) {
		$time = time(); // Current post time
		$ip = $_SERVER['REMOTE_ADDR']; // user ip
		mysqli_query($this->db, "SET character_set_client=utf8mb4") or die(mysqli_error($this->db));
		mysqli_query($this->db, "SET character_set_connection=utf8mb4") or die(mysqli_error($this->db));
		...
		$saveTextFromData = mysqli_query($this->db, "INSERT INTO  `dot_user_posts`(user_id_fk,post_type,post_created_time,user_ip,slug,m_product_name,m_product_description,product_images,product_normal_price,product_category,product_status,post_page_type,product_message_status,product_place,product_currency,user_lat,user_lang,post_country_code) VALUES ('$uid','p_product','$time','$ip','$slugURL', '$productTitle', '$productDescriptionHtml', '$productImageValues', '$productPrice', '$productCategory', '0','market','$messageStatus','$place','$productCurrency','$LocationUserLatitude','$LocationUserLongitude','$dataCountry')") or die(mysqli_error($this->db));
...
// Product Details
	public function Dot_GetProductDetails($url) {
		...
		mysqli_query($this->db, "SET character_set_client=utf8") or die(mysqli_error($this->db));
		mysqli_query($this->db, "SET character_set_connection=utf8") or die(mysqli_error($this->db));
		$checProductExist = mysqli_query($this->db, "SELECT slug, user_post_id FROM dot_user_posts WHERE user_post_id = '$slug'") or die(mysqli_error($this->db));
		if (mysqli_num_rows($checProductExist) == 1) {
			$pdata = mysqli_fetch_array($checProductExist, MYSQLI_ASSOC);
			$productID = $pdata['user_post_id'];
			$getDetails = mysqli_query($this->db, "SELECT P.user_post_id, P.user_id_fk,P.post_type,P.post_created_time,P.comment_status,P.who_can_see_post,P.slug,P.m_product_name,P.m_product_description,P.product_images,P.product_normal_price,P.product_category,P.product_currency,P.product_place,P.product_discount_price, P.product_status, U.user_name, U.user_fullname,U.verified_user FROM dot_user_posts P, dot_users U WHERE P.post_type = 'p_product' AND P.post_page_type = 'market' AND P.user_post_id = '$productID'") or die(mysqli_error($this->db));
...
// Total Sales This Month
	public function Dot_TotalEarningThisMonthDolar($uid) {
		$uid = mysqli_real_escape_string($this->db, $uid);
		$CountLike = mysqli_query($this->db, "SELECT SUM(user_earning) AS earningDolar FROM dot_payments WHERE payment_type = 'donate' AND payment_status = '1' AND customer_user_id = '$uid' AND MONTH(FROM_UNIXTIME(payment_time)) = MONTH(CURRENT_DATE())  AND YEAR(FROM_UNIXTIME(payment_time)) = YEAR(CURRENT_DATE())") or die(mysqli_error($this->db));
		$row = mysqli_fetch_array($CountLike, MYSQLI_ASSOC);
		if ($row['earningDolar']) {
...
// Total Sales This Month
	public function Dot_TotalLiveEarningThisMonthDolar($uid) {
		$uid = mysqli_real_escape_string($this->db, $uid);
		$CountLike = mysqli_query($this->db, "SELECT SUM(user_earning) AS earningDolar FROM dot_payments WHERE payment_type = 'liveDonate' AND payment_status = '1' AND product_owner_id = '$uid' AND MONTH(FROM_UNIXTIME(payment_time)) = MONTH(CURRENT_DATE())  AND YEAR(FROM_UNIXTIME(payment_time)) = YEAR(CURRENT_DATE())") or die(mysqli_error($this->db));
		$row = mysqli_fetch_array($CountLike, MYSQLI_ASSOC);
		if ($row['earningDolar']) {
			return $row['earningDolar'];
		} else {
...
// Total Sales This Month
	public function Dot_TotalEarningThisMonthDolarMarket($uid) {
		$uid = mysqli_real_escape_string($this->db, $uid);
		$CountLike = mysqli_query($this->db, "SELECT SUM(user_earning) AS earningDolar FROM dot_payments WHERE payment_type = 'pb' AND payment_status = '1' AND customer_user_id = '$uid' AND MONTH(FROM_UNIXTIME(payment_time)) = MONTH(CURRENT_DATE())  AND YEAR(FROM_UNIXTIME(payment_time)) = YEAR(CURRENT_DATE())") or die(mysqli_error($this->db));
		$row = mysqli_fetch_array($CountLike, MYSQLI_ASSOC);
		if ($row['earningDolar']) {
			return $row['earningDolar'];
		} else {
...
// Total Sales This Month
	public function Dot_TotalEarningThisMonthSumAll($uid) {
		$uid = mysqli_real_escape_string($this->db, $uid);
		$CountLike = mysqli_query($this->db, "SELECT SUM(user_earning) AS earningDolar FROM dot_payments WHERE payment_type IN ('donate','pb','liveDonate')  AND payment_status = '1' AND (customer_user_id = '$uid' OR product_owner_id = '$uid') AND calculated = '0'") or die(mysqli_error($this->db));
		//$updateNotificationUser = mysqli_query($this->db,"UPDATE dot_payments SET calculated = '1' WHERE payment_user_id = '$uid' AND payment_status = '1' AND calculated = '0'") or die(mysqli_error($this->db));
		$row = mysqli_fetch_array($CountLike, MYSQLI_ASSOC);
		if (mysqli_num_rows($CountLike) > 0) {
			$updateNotificationUser = mysqli_query($this->db, "UPDATE dot_payments SET calculated = '1' WHERE payment_type IN ('donate','pb','liveDonate') AND (customer_user_id = '$uid' OR product_owner_id = '$uid') AND payment_status = '1' AND calculated = '0'") or die(mysqli_error($this->db));
		}
		if ($row['earningDolar']) {
			return $row['earningDolar'];
		} else {
...
// Earnings Donate
	public function Dot_TotalDonateEarnings() {
		$totalBoostEarnings = mysqli_query($this->db, "SELECT SUM(admin_earning) AS totalDonate FROM dot_payments WHERE payment_type = 'donate' AND payment_status = '1'") or die(mysqli_error($this->db));
		$row = mysqli_fetch_array($totalBoostEarnings, MYSQLI_ASSOC);
		if ($row['totalDonate']) {
...
// Add User FromFavouriteList
	public function Dot_AddThisUserFromFavouriteList($uid, $favouriteID) {
		...
		$checkUserExist = mysqli_query($this->db, "SELECT user_id FROM dot_users WHERE user_id = '$uid' AND user_status = '1'") or die(mysqli_error($this->db));
		if (mysqli_num_rows($checkUserExist) == '1') {
			$saveTextFromData = mysqli_query($this->db, "INSERT INTO `dot_favourite_users`(f_user_id,f_user_id_two,time,status)VALUES('$uid','$favouriteID','$time','0')") or die(mysqli_error($this->db));
			mysqli_query($this->db, "UPDATE dot_users SET favourite_notification = '1' WHERE user_id = '$favouriteID' AND user_status = '1'") or die(mysqli_error($thisi->db));
			return true;
		} else {return false;}
	}
// Add User FromFavouriteList
...
// Insert Profile Visito If visited Profile is in pro member () visit_checked
	public function Dot_InsertVisitor($uid, $visitedUserID) {
		...
		if (mysqli_num_rows($checkVisitedProfileIsPro) == 1 && mysqli_num_rows($checkVisitChecked) == 0 && mysqli_num_rows($checkUserisInDatabase) == 1) {
			...
			mysqli_query($this->db, "UPDATE dot_users SET profile_visit_count = '1',visitor_notification = '1' WHERE  user_id = '$visitedUserID' AND user_status = '1'  AND pro_user_type = '1'") or die(mysqli_error($this->db));
			return true;
		} else {
...
// Insert Profile Visito If visited Profile is in pro member () visit_checked
	public function Dot_InsertVisitorNormal($uid, $visitedUserID) {
		...
		if (mysqli_num_rows($checkVisitChecked) == 0 && mysqli_num_rows($checkUserisInDatabase) == 1) {
			$saveTextFromData = mysqli_query($this->db, "INSERT INTO `dot_profile_visitors`(visitor_id ,visited_user_id,time)VALUES('$uid','$visitedUserID','$time')") or die(mysqli_error($this->db));
			mysqli_query($this->db, "UPDATE dot_users SET profile_visit_count = '1' , visitor_notification = '1' WHERE  user_id = '$visitedUserID' AND user_status = '1'") or die(mysqli_error($this->db));
			return true;
		} else {
			$saveTextFromData = mysqli_query($this->db, "UPDATE dot_profile_visitors SET visit_checked = '0' WHERE  visited_user_id = '$visitedUserID' AND visitor_id = '$uid'") or die(mysqli_error($this->db));
			mysqli_query($this->db, "UPDATE dot_users SET profile_visit_count = '1' , visitor_notification = '1' WHERE  user_id = '$visitedUserID' AND user_status = '1'") or die(mysqli_error($this->db));
...
/*Update Visit Notification*/
	public function Dot_UpdateVisitNotificationPlus($uid) {
		$uid = mysqli_real_escape_string($this->db, $uid);
		$checkUserisInDatabase = mysqli_query($this->db, "SELECT user_id FROM dot_users WHERE user_id = '$uid' AND user_status = '1'") or die(mysqli_error($this->db));
		if (mysqli_num_rows($checkUserisInDatabase) == 1) {
			mysqli_query($this->db, "UPDATE dot_users SET visitor_notification = '0' WHERE  user_id = '$uid' AND user_status = '1'") or die(mysqli_error($this->db));
			return true;
		} else {
			return true;
		}
	}
/*Update Favourite Notification*/
	public function Dot_UpdateFavouritesNotificationPlus($uid) {
		$uid = mysqli_real_escape_string($this->db, $uid);
		$checkUserisInDatabase = mysqli_query($this->db, "SELECT user_id FROM dot_users WHERE user_id = '$uid' AND user_status = '1'") or die(mysqli_error($this->db));
		if (mysqli_num_rows($checkUserisInDatabase) == 1) {
			mysqli_query($this->db, "UPDATE dot_users SET favourite_notification = '0' WHERE  user_id = '$uid' AND user_status = '1'") or die(mysqli_error($this->db));
			return true;
		} else {
			return true;
		}
...
// Check User have Visitor
	public function Dot_CheckUserHaveFavouriteCount($uid) {
		$uid = mysqli_real_escape_string($this->db, $uid);
		$CountLike = mysqli_query($this->db, "SELECT COUNT(*) AS favouritCounts FROM dot_favourite_users WHERE f_user_id = '$uid' AND status = '0'") or die(mysqli_error($this->db));
		$row = mysqli_fetch_array($CountLike, MYSQLI_ASSOC);
		if ($row['favouritCounts'] > 0) {
			return $row['favouritCounts'];
		} else {return false;}
	}
// Check User have Visitor
...
// Get UserName From Pages
	public function Dot_GetVisitorNotificationNumberStatus($uid) {
		$uid = mysqli_real_escape_string($this->db, $uid);
		// Username is not enouth for your user
		// If your user changed his/her name then thiw query will be start on that time
		// It will show just 25 character if you want more then you can change it easyly
		$query = mysqli_query($this->db, "SELECT visitor_notification FROM `dot_users` WHERE user_id='$uid' AND user_status='1'") or die(mysqli_error($this->db));
		$data = mysqli_fetch_array($query, MYSQLI_ASSOC);
		if ($data['visitor_notification']) {
			$name = $data['visitor_notification'];
			return $name;
		}
	}
// Get UserName From Pages
	public function Dot_GetFavouriteNotificationNumber($uid) {
		...
		// If your user changed his/her name then thiw query will be start on that time
		// It will show just 25 character if you want more then you can change it easyly
		$query = mysqli_query($this->db, "SELECT favourite_notification FROM `dot_users` WHERE user_id='$uid' AND user_status='1'") or die(mysqli_error($this->db));
		$data = mysqli_fetch_array($query, MYSQLI_ASSOC);
		if ($data['favourite_notification']) {
			$name = $data['favourite_notification'];
			return $name;
		}
	}
// Update Visit status ()
...
// Update Visit status ()
	public function Dot_UpdateUserFStatus($uid, $thisUser) {
		$uid = mysqli_real_escape_string($this->db, $uid);
		$thisUser = mysqli_real_escape_string($this->db, $thisUser);
		$checkUserExistInVisitList = mysqli_query($this->db, "SELECT status FROM dot_favourite_users WHERE status = '0' AND f_user_id = '$uid' AND f_user_id_two = '$thisUser'") or die(mysqli_error($this->db));
		if (mysqli_num_rows($checkUserExistInVisitList) == 1) {
			mysqli_query($this->db, "UPDATE dot_favourite_users SET status = '1' WHERE status = '0' AND f_user_id = '$uid' AND f_user_id_two = '$thisUser'") or die(mysqli_error($this->db));
			return true;
		} else {return false;}
	}
// Get User Favourite List
...
// Get All Cities
	public function Dot_UserCities($dataCountry, $GetUserID) {
		...
		if (mysqli_num_rows($CheckUserExist) == 1) {
			...
			$countryID = isset($GetCountryID['id']) ? $GetCountryID['id'] : NULL;
...
/*Update Show Online Offline Last seen Status*/
	public function Dot_UpdateLastSeenStatusOpenClose($uid, $status) {
		$uid = mysqli_real_escape_string($this->db, $uid);
		$status = mysqli_real_escape_string($this->db, $status);
		$checkProSysstemStatus = mysqli_query($this->db, "SELECT pro_system FROM dot_configuration WHERE configuration_id = '1'") or die(mysqli_error($this->db));
		...
		$data = mysqli_fetch_array($checkProSysstemStatus, MYSQLI_ASSOC);
		$proStatus = $data['pro_system'];
		if ($proStatus == 1) {
...
/*Get Last Announcement*/
	public function Dot_GetLastAnnouncement($uid) {
		...
		$getAID = isset($Check['an_id']) ? $Check['an_id'] : NULL;
...
/*Change Default Currency*/
	public function Dot_UpdateDefaultCurrency($uid, $crncy) {
		$uid = mysqli_real_escape_string($this->db, $uid);
		$checkUserisAdmin = mysqli_query($this->db, "SELECT user_id FROM dot_users WHERE user_id = '$uid' AND  user_type = '1'") or die(mysqli_error($this->db));
		if (mysqli_num_rows($checkUserisAdmin) == '1') {
			$UpdateDefaultCurrency = mysqli_query($this->db, "UPDATE dot_configuration SET default_currency = '$crncy' WHERE configuration_id = '1' ") or die(mysqli_error($this->db));
			return true;
		} else {return false;}
	}
...
/*Update Payment Fees*/
	public function Dot_UpdatePaymentFee($uid, $newpaymenFee, $minimumWithdrawlAmount) {
		$uid = mysqli_real_escape_string($this->db, $uid);
		$newpaymenFee = mysqli_real_escape_string($this->db, $newpaymenFee);
		$checkUserisAdmin = mysqli_query($this->db, "SELECT user_id FROM dot_users WHERE user_id = '$uid' AND user_type= '1'") or die(mysqli_error($this->db));
		if (mysqli_num_rows($checkUserisAdmin) > 0) {
			$updatePaymenFees = mysqli_query($this->db, "UPDATE dot_configuration SET payment_fee = '$newpaymenFee' , min_withdrawal_amount = '$minimumWithdrawlAmount' WHERE  configuration_id = '1'") or die(mysqli_error($this->db));
			return true;
		} else {
			return false;
		}
...
/*Update Donation Status*/
	public function Dot_UpdateDonationStatus($uid, $donationStatus) {
		$uid = mysqli_real_escape_string($this->db, $uid);
		$donationStatus = mysqli_real_escape_string($this->db, $donationStatus);
		$checkUserisAdmin = mysqli_query($this->db, "SELECT user_id FROM dot_users WHERE user_id = '$uid' AND user_type= '1'") or die(mysqli_error($this->db));
		if (mysqli_num_rows($checkUserisAdmin) > 0) {
			$updatePaymenFees = mysqli_query($this->db, "UPDATE dot_configuration SET donation_feature = '$donationStatus' WHERE  configuration_id = '1'") or die(mysqli_error($this->db));
			return true;
		} else {
			return false;
		}
	}
/*,s_facebook, s_twitter, s_tumblr,s_pinteres,s_vk, s_digg, s_linkedin ,s_reddit, s_whatsapp, s_viber, s_delicious,s_evernote, s_yahoo, s_gmail, s_copy, social_share_status*/
	public function Dot_UpdateCopyPostStatus($uid, $socialShareStatus) {
		$uid = mysqli_real_escape_string($this->db, $uid);
		$socialShareStatus = mysqli_real_escape_string($this->db, $socialShareStatus);
		$checkUserisAdmin = mysqli_query($this->db, "SELECT user_id FROM dot_users WHERE user_id = '$uid' AND user_type= '1'") or die(mysqli_error($this->db));
		if (mysqli_num_rows($checkUserisAdmin) > 0) {
			$updatePaymenFees = mysqli_query($this->db, "UPDATE dot_configuration SET s_copy = '$socialShareStatus' WHERE  configuration_id = '1'") or die(mysqli_error($this->db));
			return true;
		} else {
			return false;
		}
	}
	public function Dot_UpdateGmailSocialShareStatus($uid, $socialShareStatus) {
		$uid = mysqli_real_escape_string($this->db, $uid);
		$socialShareStatus = mysqli_real_escape_string($this->db, $socialShareStatus);
		$checkUserisAdmin = mysqli_query($this->db, "SELECT user_id FROM dot_users WHERE user_id = '$uid' AND user_type= '1'") or die(mysqli_error($this->db));
		if (mysqli_num_rows($checkUserisAdmin) > 0) {
			$updatePaymenFees = mysqli_query($this->db, "UPDATE dot_configuration SET s_gmail = '$socialShareStatus' WHERE  configuration_id = '1'") or die(mysqli_error($this->db));
			return true;
		} else {
			return false;
		}
	}
	public function Dot_UpdateYahooSocialShareStatus($uid, $socialShareStatus) {
		$uid = mysqli_real_escape_string($this->db, $uid);
		$socialShareStatus = mysqli_real_escape_string($this->db, $socialShareStatus);
		$checkUserisAdmin = mysqli_query($this->db, "SELECT user_id FROM dot_users WHERE user_id = '$uid' AND user_type= '1'") or die(mysqli_error($this->db));
		if (mysqli_num_rows($checkUserisAdmin) > 0) {
			$updatePaymenFees = mysqli_query($this->db, "UPDATE dot_configuration SET s_yahoo = '$socialShareStatus' WHERE  configuration_id = '1'") or die(mysqli_error($this->db));
			return true;
		} else {
			return false;
		}
	}
	public function Dot_UpdateEverNoteSocialShareStatus($uid, $socialShareStatus) {
		$uid = mysqli_real_escape_string($this->db, $uid);
		$socialShareStatus = mysqli_real_escape_string($this->db, $socialShareStatus);
		$checkUserisAdmin = mysqli_query($this->db, "SELECT user_id FROM dot_users WHERE user_id = '$uid' AND user_type= '1'") or die(mysqli_error($this->db));
		if (mysqli_num_rows($checkUserisAdmin) > 0) {
			$updatePaymenFees = mysqli_query($this->db, "UPDATE dot_configuration SET s_evernote = '$socialShareStatus' WHERE  configuration_id = '1'") or die(mysqli_error($this->db));
			return true;
		} else {
			return false;
		}
	}
	public function Dot_UpdateDeliciousSocialShareStatus($uid, $socialShareStatus) {
		$uid = mysqli_real_escape_string($this->db, $uid);
		$socialShareStatus = mysqli_real_escape_string($this->db, $socialShareStatus);
		$checkUserisAdmin = mysqli_query($this->db, "SELECT user_id FROM dot_users WHERE user_id = '$uid' AND user_type= '1'") or die(mysqli_error($this->db));
		if (mysqli_num_rows($checkUserisAdmin) > 0) {
			$updatePaymenFees = mysqli_query($this->db, "UPDATE dot_configuration SET s_delicious = '$socialShareStatus' WHERE  configuration_id = '1'") or die(mysqli_error($this->db));
			return true;
		} else {
			return false;
		}
	}
	public function Dot_UpdateViberSocialShareStatus($uid, $socialShareStatus) {
		$uid = mysqli_real_escape_string($this->db, $uid);
		$socialShareStatus = mysqli_real_escape_string($this->db, $socialShareStatus);
		$checkUserisAdmin = mysqli_query($this->db, "SELECT user_id FROM dot_users WHERE user_id = '$uid' AND user_type= '1'") or die(mysqli_error($this->db));
		if (mysqli_num_rows($checkUserisAdmin) > 0) {
			$updatePaymenFees = mysqli_query($this->db, "UPDATE dot_configuration SET s_viber = '$socialShareStatus' WHERE  configuration_id = '1'") or die(mysqli_error($this->db));
			return true;
		} else {
			return false;
		}
	}
	public function Dot_UpdateWhatsappSocialShareStatus($uid, $socialShareStatus) {
		$uid = mysqli_real_escape_string($this->db, $uid);
		$socialShareStatus = mysqli_real_escape_string($this->db, $socialShareStatus);
		$checkUserisAdmin = mysqli_query($this->db, "SELECT user_id FROM dot_users WHERE user_id = '$uid' AND user_type= '1'") or die(mysqli_error($this->db));
		if (mysqli_num_rows($checkUserisAdmin) > 0) {
			$updatePaymenFees = mysqli_query($this->db, "UPDATE dot_configuration SET s_whatsapp = '$socialShareStatus' WHERE  configuration_id = '1'") or die(mysqli_error($this->db));
			return true;
		} else {
			return false;
		}
	}
	public function Dot_UpdateRedditSocialShareStatus($uid, $socialShareStatus) {
		$uid = mysqli_real_escape_string($this->db, $uid);
		$socialShareStatus = mysqli_real_escape_string($this->db, $socialShareStatus);
		$checkUserisAdmin = mysqli_query($this->db, "SELECT user_id FROM dot_users WHERE user_id = '$uid' AND user_type= '1'") or die(mysqli_error($this->db));
		if (mysqli_num_rows($checkUserisAdmin) > 0) {
			$updatePaymenFees = mysqli_query($this->db, "UPDATE dot_configuration SET s_reddit = '$socialShareStatus' WHERE  configuration_id = '1'") or die(mysqli_error($this->db));
			return true;
		} else {
			return false;
		}
	}
	public function Dot_UpdateDiggSocialShareStatus($uid, $socialShareStatus) {
		$uid = mysqli_real_escape_string($this->db, $uid);
		$socialShareStatus = mysqli_real_escape_string($this->db, $socialShareStatus);
		$checkUserisAdmin = mysqli_query($this->db, "SELECT user_id FROM dot_users WHERE user_id = '$uid' AND user_type= '1'") or die(mysqli_error($this->db));
		if (mysqli_num_rows($checkUserisAdmin) > 0) {
			$updatePaymenFees = mysqli_query($this->db, "UPDATE dot_configuration SET s_digg = '$socialShareStatus' WHERE  configuration_id = '1'") or die(mysqli_error($this->db));
			return true;
		} else {
			return false;
		}
	}
	public function Dot_UpdateVContacteSocialShareStatus($uid, $socialShareStatus) {
		$uid = mysqli_real_escape_string($this->db, $uid);
		$socialShareStatus = mysqli_real_escape_string($this->db, $socialShareStatus);
		$checkUserisAdmin = mysqli_query($this->db, "SELECT user_id FROM dot_users WHERE user_id = '$uid' AND user_type= '1'") or die(mysqli_error($this->db));
		if (mysqli_num_rows($checkUserisAdmin) > 0) {
			$updatePaymenFees = mysqli_query($this->db, "UPDATE dot_configuration SET s_vk = '$socialShareStatus' WHERE  configuration_id = '1'") or die(mysqli_error($this->db));
			return true;
		} else {
			return false;
		}
	}
	public function Dot_UpdateSocialShareStatus($uid, $socialShareStatus) {
		$uid = mysqli_real_escape_string($this->db, $uid);
		$socialShareStatus = mysqli_real_escape_string($this->db, $socialShareStatus);
		$checkUserisAdmin = mysqli_query($this->db, "SELECT user_id FROM dot_users WHERE user_id = '$uid' AND user_type= '1'") or die(mysqli_error($this->db));
		if (mysqli_num_rows($checkUserisAdmin) > 0) {
			$updatePaymenFees = mysqli_query($this->db, "UPDATE dot_configuration SET social_share_status = '$socialShareStatus' WHERE  configuration_id = '1'") or die(mysqli_error($this->db));
			return true;
		} else {
			return false;
		}
	}
	public function Dot_UpdateFacebookSocialShareStatus($uid, $socialShareStatus) {
		$uid = mysqli_real_escape_string($this->db, $uid);
		$socialShareStatus = mysqli_real_escape_string($this->db, $socialShareStatus);
		$checkUserisAdmin = mysqli_query($this->db, "SELECT user_id FROM dot_users WHERE user_id = '$uid' AND user_type= '1'") or die(mysqli_error($this->db));
		if (mysqli_num_rows($checkUserisAdmin) > 0) {
			$updatePaymenFees = mysqli_query($this->db, "UPDATE dot_configuration SET s_facebook = '$socialShareStatus' WHERE  configuration_id = '1'") or die(mysqli_error($this->db));
			return true;
		} else {
			return false;
		}
	}
	public function Dot_UpdateTwitterSocialShareStatus($uid, $socialShareStatus) {
		$uid = mysqli_real_escape_string($this->db, $uid);
		$socialShareStatus = mysqli_real_escape_string($this->db, $socialShareStatus);
		$checkUserisAdmin = mysqli_query($this->db, "SELECT user_id FROM dot_users WHERE user_id = '$uid' AND user_type= '1'") or die(mysqli_error($this->db));
		if (mysqli_num_rows($checkUserisAdmin) > 0) {
			$updatePaymenFees = mysqli_query($this->db, "UPDATE dot_configuration SET s_twitter = '$socialShareStatus' WHERE  configuration_id = '1'") or die(mysqli_error($this->db));
			return true;
		} else {
			return false;
		}
	}
	public function Dot_UpdateTumblrSocialShareStatus($uid, $socialShareStatus) {
		$uid = mysqli_real_escape_string($this->db, $uid);
		$socialShareStatus = mysqli_real_escape_string($this->db, $socialShareStatus);
		$checkUserisAdmin = mysqli_query($this->db, "SELECT user_id FROM dot_users WHERE user_id = '$uid' AND user_type= '1'") or die(mysqli_error($this->db));
		if (mysqli_num_rows($checkUserisAdmin) > 0) {
			$updatePaymenFees = mysqli_query($this->db, "UPDATE dot_configuration SET s_tumblr = '$socialShareStatus' WHERE  configuration_id = '1'") or die(mysqli_error($this->db));
			return true;
		} else {
			return false;
		}
	}
	public function Dot_UpdatePinterestSocialShareStatus($uid, $socialShareStatus) {
		$uid = mysqli_real_escape_string($this->db, $uid);
		$socialShareStatus = mysqli_real_escape_string($this->db, $socialShareStatus);
		$checkUserisAdmin = mysqli_query($this->db, "SELECT user_id FROM dot_users WHERE user_id = '$uid' AND user_type= '1'") or die(mysqli_error($this->db));
		if (mysqli_num_rows($checkUserisAdmin) > 0) {
			$updatePaymenFees = mysqli_query($this->db, "UPDATE dot_configuration SET s_pinteres = '$socialShareStatus' WHERE  configuration_id = '1'") or die(mysqli_error($this->db));
			return true;
		} else {
			return false;
		}
	}
	public function Dot_UpdateLinkedinSocialShareStatus($uid, $socialShareStatus) {
		$uid = mysqli_real_escape_string($this->db, $uid);
		$socialShareStatus = mysqli_real_escape_string($this->db, $socialShareStatus);
		$checkUserisAdmin = mysqli_query($this->db, "SELECT user_id FROM dot_users WHERE user_id = '$uid' AND user_type= '1'") or die(mysqli_error($this->db));
		if (mysqli_num_rows($checkUserisAdmin) > 0) {
			$updatePaymenFees = mysqli_query($this->db, "UPDATE dot_configuration SET s_linkedin = '$socialShareStatus' WHERE  configuration_id = '1'") or die(mysqli_error($this->db));
			return true;
		} else {
			return false;
		}
...
// Get Details
	public function Dot_CheckProductAndGetDetails($uid, $ordered_id, $product_id, $buyer_id) {
		$uid = mysqli_real_escape_string($this->db, $uid);
		$ordered_id = mysqli_real_escape_string($this->db, $ordered_id);
		$product_id = mysqli_real_escape_string($this->db, $product_id);
		$buyer_id = mysqli_real_escape_string($this->db, $buyer_id);
		$checkUserisAdmin = mysqli_query($this->db, "SELECT user_id FROM dot_users WHERE user_id = '$uid' AND user_type= '1'") or die(mysqli_error($this->db));
		$checkProductOrderExist = mysqli_query($this->db, "SELECT payment_id, payment_user_id, product_owner_id, payment_post_id FROM dot_payments WHERE payment_post_id = '$product_id' AND payment_id = '$ordered_id' AND payment_user_id = '$buyer_id'") or die(mysqli_error($this->db));
		if (mysqli_num_rows($checkUserisAdmin) > 0 && mysqli_num_rows($checkProductOrderExist) > 0) {
			$query = mysqli_query($this->db, "SELECT payment_id, payment_user_id, payment_post_id, product_owner_id, order_key, payment_option, payment_time, payment_status, amount, fee, admin_earning, user_earning,crtid FROM dot_payments WHERE payment_post_id = '$product_id'") or die(mysqli_error($this->db));
			while ($row = mysqli_fetch_array($query, MYSQLI_ASSOC)) {
				// Store the result into array
				$data[] = $row;
			}
...
/*Show Clicked Post From Right Sidebar*/
	public function Dot_ShowProductPost($postID, $postOwnerId) {
		$postID = mysqli_real_escape_string($this->db, $postID);
		$postOwnerId = mysqli_real_escape_string($this->db, $postOwnerId);
		$getData = mysqli_query($this->db, "SELECT DISTINCT P.user_post_id, P.user_id_fk,P.post_type,P.comment_status,P.watermarkid,P.which_image,P.post_created_time,P.hashtag_normal,P.hashtag_diez,P.post_title_text,P.post_text_details,P.post_event_id,P.post_event_page_id,P.who_can_see_post,P.post_image_id,P.post_link_url,P.post_link_description,P.post_link_title,P.post_link_img,P.post_link_mini_url,P.post_video_id,P.post_audio_id,P.filter_name, P.gif_url,U.show_suggest_hashTags,P.user_lat ,P.user_lang,P.location_place,P.about_location,P.before_after_images,P.slug,P.post_page_type,P.shared_post,P.user_feeling,P.post_video_name,P.m_product_name,P.m_product_description,P.product_images,P.product_normal_price,P.product_category, P.product_status,P.product_discount_price,P.product_currency,P.ads_status,P.ads_display_type,U.user_name, U.user_fullname,U.verified_user,U.pro_user_type,U.style_mode,U.user_page_lang,U.user_donate_message,U.last_login,U.show_online_offline_status FROM dot_user_posts P, dot_users U  WHERE U.user_status='1' AND P.user_id_fk=U.user_id and P.user_post_id='$postID' AND P.user_id_fk = '$postOwnerId'") or die(mysqli_error($this->db));
...
/*Get Cart Details*/
	public function Dot_UserOrderCartDetails($ownerID, $cardID) {
		$ownerID = mysqli_real_escape_string($this->db, $ownerID);
		$cardID = mysqli_real_escape_string($this->db, $cardID);
		$getData = mysqli_query($this->db, "SELECT cart_id, purchase_status, product_owner_id,order_status, customer_fullname, product_id_fk,customer_address, customer_phone, customer_country, customer_city,  customer_state,customer_post_code, customer_personal_id_or_passport_id, cargo_campany, cargo_tracking_number, time, cargo_delivery_status FROM dot_user_cart  WHERE cart_id = '$cardID' AND product_owner_id = '$ownerID'") or die(mysqli_error($this->db));
		$data = mysqli_fetch_array($getData, MYSQLI_ASSOC);
		return $data;
	}
/*Delete Order ID  ()  */
	public function Dot_DeleteThisOrderID($uid, $orderID) {
		$orderID = mysqli_real_escape_string($this->db, $orderID);
		$uid = mysqli_real_escape_string($this->db, $uid);
		$checkUserisAdmin = mysqli_query($this->db, "SELECT user_id FROM dot_users WHERE user_id = '$uid' AND user_type= '1'") or die(mysqli_error($this->db));
		$checkOrderIDExist = mysqli_query($this->db, "SELECT payment_id,payment_post_id,payment_user_id,product_owner_id,calculated,crtid FROM dot_payments WHERE payment_id = '$orderID' AND payment_status = '1' AND calculated = '1'") or die(mysqli_error($this->db));
		$checkOrderIDExistNotCalculated = mysqli_query($this->db, "SELECT payment_id,payment_post_id,payment_user_id,product_owner_id,calculated,crtid FROM dot_payments WHERE payment_id = '$orderID' AND payment_status = '1' AND calculated = '0'") or die(mysqli_error($this->db));
		if (mysqli_num_rows($checkUserisAdmin) == '1' && mysqli_num_rows($checkOrderIDExist) == '1') {
			$data = mysqli_fetch_array($checkOrderIDExist, MYSQLI_ASSOC);
			$userOrderedPostID = mysqli_real_escape_string($this->db, $data['payment_post_id']);
			$userProductOwnerID = mysqli_real_escape_string($this->db, $data['product_owner_id']);
			$userProductBuyerID = mysqli_real_escape_string($this->db, $data['payment_user_id']);
			$userCartID = mysqli_real_escape_string($this->db, $data['crtid']);
			$getProductMoney = mysqli_query($this->db, "SELECT user_post_id, product_normal_price, product_discount_price FROM dot_user_posts WHERE user_post_id = '$userOrderedPostID'") or die(mysqli_error($this->db));
			$dataDet = mysqli_fetch_array($getProductMoney, MYSQLI_ASSOC);
			$discountPrice = mysqli_real_escape_string($this->db, $dataDet['product_discount_price']);
			$normalPrice = mysqli_real_escape_string($this->db, $dataDet['product_normal_price']);
			if ($discountPrice) {
				mysqli_query($this->db, "UPDATE dot_users  SET total_earnings = total_earnings - $discountPrice WHERE user_id = '$userProductOwnerID'") or die(mysqli_error($this->db));
				mysqli_query($this->db, "UPDATE dot_users  SET total_earnings = total_earnings +  $discountPrice WHERE user_id = '$userProductBuyerID'") or die(mysqli_error($this->db));
				mysqli_query($this->db, "DELETE FROM dot_payments WHERE payment_id = '$orderID' AND payment_status = '1'") or die(mysqli_error($this->db));
				mysqli_query($this->db, "UPDATE dot_user_cart  SETpurchase_status = '0' WHERE cart_id = '$userCartID'") or die(mysqli_error($this->db));
			} else {
				mysqli_query($this->db, "UPDATE dot_users  SET total_earnings = total_earnings - $normalPrice WHERE user_id = '$userProductOwnerID'") or die(mysqli_error($this->db));
				mysqli_query($this->db, "UPDATE dot_users  SET total_earnings = total_earnings +  $normalPrice WHERE user_id = '$userProductBuyerID'") or die(mysqli_error($this->db));
				mysqli_query($this->db, "DELETE FROM dot_payments WHERE payment_id = '$orderID' AND payment_status = '1'") or die(mysqli_error($this->db));
				mysqli_query($this->db, "UPDATE dot_user_cart  SETpurchase_status = '0' WHERE cart_id = '$userCartID'") or die(mysqli_error($this->db));
			}
			return true;
		} else {
			if (mysqli_num_rows($checkUserisAdmin) == '1' && mysqli_num_rows($checkOrderIDExistNotCalculated) == '1') {
				/**/
				$dataN = mysqli_fetch_array($checkOrderIDExistNotCalculated, MYSQLI_ASSOC);
				$userOrderedPostIDN = mysqli_real_escape_string($this->db, $dataN['payment_post_id']);
				$userProductOwnerIDN = mysqli_real_escape_string($this->db, $dataN['product_owner_id']);
				$userProductBuyerIDN = mysqli_real_escape_string($this->db, $dataN['payment_user_id']);
				$userCartIDN = mysqli_real_escape_string($this->db, $data['crtid']);
				$getProductMoneyN = mysqli_query($this->db, "SELECT user_post_id, product_normal_price, product_discount_price FROM dot_user_posts WHERE user_post_id = '$userOrderedPostIDN'") or die(mysqli_error($this->db));
				$dataDetN = mysqli_fetch_array($getProductMoneyN, MYSQLI_ASSOC);
				$discountPriceN = mysqli_real_escape_string($this->db, $dataDetN['product_discount_price']);
				$normalPriceN = mysqli_real_escape_string($this->db, $dataDetN['product_normal_price']);
				if ($discountPriceN) {
					mysqli_query($this->db, "UPDATE dot_users  SET total_earnings = total_earnings +  $discountPriceN WHERE user_id = '$userProductBuyerIDN'") or die(mysqli_error($this->db));
					mysqli_query($this->db, "UPDATE dot_user_cart  SETpurchase_status = '0' WHERE cart_id = '$userCartIDN'") or die(mysqli_error($this->db));
					mysqli_query($this->db, "DELETE FROM dot_payments WHERE payment_id = '$orderID' AND payment_status = '1'") or die(mysqli_error($this->db));
				} else {
					mysqli_query($this->db, "UPDATE dot_users  SET total_earnings = total_earnings +  $normalPriceN WHERE user_id = '$userProductBuyerIDN'") or die(mysqli_error($this->db));
					mysqli_query($this->db, "UPDATE dot_user_cart  SETpurchase_status = '0' WHERE cart_id = '$userCartIDN'") or die(mysqli_error($this->db));
					mysqli_query($this->db, "DELETE FROM dot_payments WHERE payment_id = '$orderID' AND payment_status = '1'") or die(mysqli_error($this->db));
				}
				/**/
...
// Get Details
	public function Dot_CheckProductAndGetDetailsByCartID($cid) {
		$uid = mysqli_real_escape_string($this->db, $uid);
		$query = mysqli_query($this->db, "SELECT payment_id, payment_user_id, payment_post_id, product_owner_id, order_key, payment_option, payment_time, payment_status, amount, fee, admin_earning, user_earning,crtid FROM dot_payments WHERE payment_post_id = '$crtid'") or die(mysqli_error($this->db));
		$data = mysqli_fetch_array($query, MYSQLI_ASSOC);
		return $data;
	}
/*Get Payment ID*/
	public function Dot_GetPaymentIDFromCartID($cartID) {
		$uid = mysqli_real_escape_string($this->db, $uid);
		$query = mysqli_query($this->db, "SELECT payment_id, payment_user_id, payment_post_id, product_owner_id, order_key, payment_option, payment_time, payment_status, amount, fee, admin_earning, user_earning,crtid FROM dot_payments WHERE crtid = '$cartID'") or die(mysqli_error($this->db));
		$data = mysqli_fetch_array($query, MYSQLI_ASSOC);
		if ($data['payment_id']) {
			$name = $data['payment_id'];
			return $name;
		}
	}
/*Delete Product From Cart*/
	public function Dot_DeleteProductFromCartData($uid, $cartID) {
		$cartID = mysqli_real_escape_string($this->db, $cartID);
		$uid = mysqli_real_escape_string($this->db, $uid);
		$checkUserisAdmin = mysqli_query($this->db, "SELECT user_id FROM dot_users WHERE user_id = '$uid' AND user_type= '1'") or die(mysqli_error($this->db));
		$checkCartIDFromData = mysqli_query($this->db, "SELECT cart_id FROM dot_user_cart WHERE cart_id = '$cartID'") or die(mysqli_error($this->db));
		if (mysqli_num_rows($checkUserisAdmin) == 1 && mysqli_num_rows($checkCartIDFromData) == 1) {
			mysqli_query($this->db, "DELETE FROM dot_user_cart WHERE cart_id = '$cartID'") or die(mysqli_error($this->db));
			return true;
		} else {return false;}
	}
/*Send Email */
	public function Dot_SendEmailToList($uid, $sendTo) {
		$sendTo = mysqli_real_escape_string($this->db, $sendTo);
		$uid = mysqli_real_escape_string($this->db, $uid);
		$checkUserisAdmin = mysqli_query($this->db, "SELECT user_id FROM dot_users WHERE user_id = '$uid' AND user_type= '1'") or die(mysqli_error($this->db));
		if (mysqli_num_rows($checkUserisAdmin) == 1) {
			$query = mysqli_query($this->db, "SELECT user_id, user_name,user_email, last_login FROM dot_users WHERE FROM_UNIXTIME(last_login) > DATE_SUB(CURRENT_TIMESTAMP, INTERVAL $sendTo)") or die(mysqli_error($this->db));
			while ($row = mysqli_fetch_array($query, MYSQLI_ASSOC)) {
				// Store the result into array
				$data[] = $row;
			}
	...
	public function Dot_SendEmailToOnlineUsersList($uid) {
		$uid = mysqli_real_escape_string($this->db, $uid);
		$checkUserisAdmin = mysqli_query($this->db, "SELECT user_id FROM dot_users WHERE user_id = '$uid' AND user_type= '1'") or die(mysqli_error($this->db));
		if (mysqli_num_rows($checkUserisAdmin) == 1) {
			$query = mysqli_query($this->db, "SELECT user_id, user_name,user_email, last_login FROM dot_users WHERE FROM_UNIXTIME(last_login) > DATE_SUB(CURRENT_TIMESTAMP, INTERVAL 3 minute)") or die(mysqli_error($this->db));
			while ($row = mysqli_fetch_array($query, MYSQLI_ASSOC)) {
				// Store the result into array
				$data[] = $row;
			}
	...
	public function Dot_SendEmailToOfflineUsersList($uid) {
		$uid = mysqli_real_escape_string($this->db, $uid);
		$checkUserisAdmin = mysqli_query($this->db, "SELECT user_id FROM dot_users WHERE user_id = '$uid' AND user_type= '1'") or die(mysqli_error($this->db));
		if (mysqli_num_rows($checkUserisAdmin) == 1) {
			$query = mysqli_query($this->db, "SELECT user_id, user_name,user_email, last_login FROM dot_users WHERE FROM_UNIXTIME(last_login) < DATE_SUB(CURRENT_TIMESTAMP, INTERVAL 3 minute)") or die(mysqli_error($this->db));
	...
	public function Dot_SendEmailToNormalUsersList($uid) {
		$uid = mysqli_real_escape_string($this->db, $uid);
		$checkUserisAdmin = mysqli_query($this->db, "SELECT user_id FROM dot_users WHERE user_id = '$uid' AND user_type= '1'") or die(mysqli_error($this->db));
		if (mysqli_num_rows($checkUserisAdmin) == 1) {
			$query = mysqli_query($this->db, "SELECT user_id, user_name,user_email, last_login FROM dot_users WHERE user_status = '1' AND pro_user_type = '0'") or die(mysqli_error($this->db));
			while ($row = mysqli_fetch_array($query, MYSQLI_ASSOC)) {
				// Store the result into array
				$data[] = $row;
			}
	...
	public function Dot_SendEmailToProUsersList($uid) {
		$uid = mysqli_real_escape_string($this->db, $uid);
		$checkUserisAdmin = mysqli_query($this->db, "SELECT user_id FROM dot_users WHERE user_id = '$uid' AND user_type= '1'") or die(mysqli_error($this->db));
		if (mysqli_num_rows($checkUserisAdmin) == 1) {
			$query = mysqli_query($this->db, "SELECT user_id, user_name,user_email, last_login FROM dot_users WHERE user_status = '1' AND pro_user_type = '1'") or die(mysqli_error($this->db));
			while ($row = mysqli_fetch_array($query, MYSQLI_ASSOC)) {
				// Store the result into array
				$data[] = $row;
			}
	...
	public function Dot_SendEmailToAllUsersList($uid) {
		$uid = mysqli_real_escape_string($this->db, $uid);
		$checkUserisAdmin = mysqli_query($this->db, "SELECT user_id FROM dot_users WHERE user_id = '$uid' AND user_type= '1'") or die(mysqli_error($this->db));
		if (mysqli_num_rows($checkUserisAdmin) == 1) {
			$query = mysqli_query($this->db, "SELECT user_id, user_name,user_email, last_login FROM dot_users") or die(mysqli_error($this->db));
			while ($row = mysqli_fetch_array($query, MYSQLI_ASSOC)) {
				// Store the result into array
				$data[] = $row;
			}
	...
	public function Dot_SendEmailToInactiveUsersList($uid) {
		$uid = mysqli_real_escape_string($this->db, $uid);
		$checkUserisAdmin = mysqli_query($this->db, "SELECT user_id FROM dot_users WHERE user_id = '$uid' AND user_type= '1'") or die(mysqli_error($this->db));
		if (mysqli_num_rows($checkUserisAdmin) == 1) {
			$query = mysqli_query($this->db, "SELECT user_id, user_name,user_email, last_login FROM dot_users WHERE user_status = '1' AND (email_verification != '1' OR email_verification IS NOT NULL) ") or die(mysqli_error($this->db));
			while ($row = mysqli_fetch_array($query, MYSQLI_ASSOC)) {
				// Store the result into array
				$data[] = $row;
			}
	...
	public function Dot_SendEmailToActiveUsersList($uid) {
		$uid = mysqli_real_escape_string($this->db, $uid);
		$checkUserisAdmin = mysqli_query($this->db, "SELECT user_id FROM dot_users WHERE user_id = '$uid' AND user_type= '1'") or die(mysqli_error($this->db));
		if (mysqli_num_rows($checkUserisAdmin) == 1) {
			$query = mysqli_query($this->db, "SELECT user_id, user_name,user_email, last_login FROM dot_users WHERE user_status = '1'  AND (email_verification = '1' OR email_verification IS NULL)") or die(mysqli_error($this->db));
			while ($row = mysqli_fetch_array($query, MYSQLI_ASSOC)) {
				// Store the result into array
				$data[] = $row;
			}
	...
	public function Dot_UpdateUserCanShareText($uid, $updateThisUserDetail, $updateUserVerify) {
		$userid = mysqli_real_escape_string($this->db, $uid);
		$checkUserAdmin = mysqli_query($this->db, "SELECT user_id FROM dot_users WHERE user_id = '$userid' AND user_type = '1'") or die(mysqli_error($this->db));
		$checkUserExist = mysqli_query($this->db, "SELECT user_id FROM dot_users WHERE user_id = '$updateThisUserDetail'") or die(mysqli_error($this->db));
		if (mysqli_num_rows($checkUserAdmin) == 1 && mysqli_num_rows($checkUserExist) == 1) {
			$update = mysqli_query($this->db, "UPDATE dot_users SET can_post_text = '$updateUserVerify' WHERE user_id = '$updateThisUserDetail'") or die(mysqli_error($this->db));
			return true;
		} else {return false;}
	}
	public function Dot_UpdateUserCanShareImagePost($uid, $updateThisUserDetail, $updateUserVerify) {
		$userid = mysqli_real_escape_string($this->db, $uid);
		$checkUserAdmin = mysqli_query($this->db, "SELECT user_id FROM dot_users WHERE user_id = '$userid' AND user_type = '1'") or die(mysqli_error($this->db));
		$checkUserExist = mysqli_query($this->db, "SELECT user_id FROM dot_users WHERE user_id = '$updateThisUserDetail'") or die(mysqli_error($this->db));
		if (mysqli_num_rows($checkUserAdmin) == 1 && mysqli_num_rows($checkUserExist) == 1) {
			$update = mysqli_query($this->db, "UPDATE dot_users SET can_post_image = '$updateUserVerify' WHERE user_id = '$updateThisUserDetail'") or die(mysqli_error($this->db));
			return true;
		} else {return false;}
	}
	public function Dot_UpdateUserCanShareVideoPost($uid, $updateThisUserDetail, $updateUserVerify) {
		$userid = mysqli_real_escape_string($this->db, $uid);
		$checkUserAdmin = mysqli_query($this->db, "SELECT user_id FROM dot_users WHERE user_id = '$userid' AND user_type = '1'") or die(mysqli_error($this->db));
		$checkUserExist = mysqli_query($this->db, "SELECT user_id FROM dot_users WHERE user_id = '$updateThisUserDetail'") or die(mysqli_error($this->db));
		if (mysqli_num_rows($checkUserAdmin) == 1 && mysqli_num_rows($checkUserExist) == 1) {
			$update = mysqli_query($this->db, "UPDATE dot_users SET can_post_video = '$updateUserVerify' WHERE user_id = '$updateThisUserDetail'") or die(mysqli_error($this->db));
			return true;
		} else {return false;}
	}
	public function Dot_UpdateUserCanShareAudioPost($uid, $updateThisUserDetail, $updateUserVerify) {
		$userid = mysqli_real_escape_string($this->db, $uid);
		$checkUserAdmin = mysqli_query($this->db, "SELECT user_id FROM dot_users WHERE user_id = '$userid' AND user_type = '1'") or die(mysqli_error($this->db));
		$checkUserExist = mysqli_query($this->db, "SELECT user_id FROM dot_users WHERE user_id = '$updateThisUserDetail'") or die(mysqli_error($this->db));
		if (mysqli_num_rows($checkUserAdmin) == 1 && mysqli_num_rows($checkUserExist) == 1) {
			$update = mysqli_query($this->db, "UPDATE dot_users SET can_post_audio = '$updateUserVerify' WHERE user_id = '$updateThisUserDetail'") or die(mysqli_error($this->db));
			return true;
		} else {return false;}
	}
	public function Dot_UpdateUserCanShareLinkPost($uid, $updateThisUserDetail, $updateUserVerify) {
		$userid = mysqli_real_escape_string($this->db, $uid);
		$checkUserAdmin = mysqli_query($this->db, "SELECT user_id FROM dot_users WHERE user_id = '$userid' AND user_type = '1'") or die(mysqli_error($this->db));
		$checkUserExist = mysqli_query($this->db, "SELECT user_id FROM dot_users WHERE user_id = '$updateThisUserDetail'") or die(mysqli_error($this->db));
		if (mysqli_num_rows($checkUserAdmin) == 1 && mysqli_num_rows($checkUserExist) == 1) {
			$update = mysqli_query($this->db, "UPDATE dot_users SET can_post_link = '$updateUserVerify' WHERE user_id = '$updateThisUserDetail'") or die(mysqli_error($this->db));
			return true;
		} else {return false;}
	}
	public function Dot_UpdateUserCanShareFiltersPost($uid, $updateThisUserDetail, $updateUserVerify) {
		$userid = mysqli_real_escape_string($this->db, $uid);
		$checkUserAdmin = mysqli_query($this->db, "SELECT user_id FROM dot_users WHERE user_id = '$userid' AND user_type = '1'") or die(mysqli_error($this->db));
		$checkUserExist = mysqli_query($this->db, "SELECT user_id FROM dot_users WHERE user_id = '$updateThisUserDetail'") or die(mysqli_error($this->db));
		if (mysqli_num_rows($checkUserAdmin) == 1 && mysqli_num_rows($checkUserExist) == 1) {
			$update = mysqli_query($this->db, "UPDATE dot_users SET can_post_filters = '$updateUserVerify' WHERE user_id = '$updateThisUserDetail'") or die(mysqli_error($this->db));
			return true;
		} else {return false;}
	}
	public function Dot_UpdateUserCanShareGiphyPost($uid, $updateThisUserDetail, $updateUserVerify) {
		$userid = mysqli_real_escape_string($this->db, $uid);
		$checkUserAdmin = mysqli_query($this->db, "SELECT user_id FROM dot_users WHERE user_id = '$userid' AND user_type = '1'") or die(mysqli_error($this->db));
		$checkUserExist = mysqli_query($this->db, "SELECT user_id FROM dot_users WHERE user_id = '$updateThisUserDetail'") or die(mysqli_error($this->db));
		if (mysqli_num_rows($checkUserAdmin) == 1 && mysqli_num_rows($checkUserExist) == 1) {
			$update = mysqli_query($this->db, "UPDATE dot_users SET can_post_giphy = '$updateUserVerify' WHERE user_id = '$updateThisUserDetail'") or die(mysqli_error($this->db));
			return true;
		} else {return false;}
	}
	public function Dot_UpdateUserCanShareWhichOnePost($uid, $updateThisUserDetail, $updateUserVerify) {
		$userid = mysqli_real_escape_string($this->db, $uid);
		$checkUserAdmin = mysqli_query($this->db, "SELECT user_id FROM dot_users WHERE user_id = '$userid' AND user_type = '1'") or die(mysqli_error($this->db));
		$checkUserExist = mysqli_query($this->db, "SELECT user_id FROM dot_users WHERE user_id = '$updateThisUserDetail'") or die(mysqli_error($this->db));
		if (mysqli_num_rows($checkUserAdmin) == 1 && mysqli_num_rows($checkUserExist) == 1) {
			$update = mysqli_query($this->db, "UPDATE dot_users SET can_post_which = '$updateUserVerify' WHERE user_id = '$updateThisUserDetail'") or die(mysqli_error($this->db));
			return true;
		} else {return false;}
	}
	public function Dot_UpdateUserCanShareLocationPost($uid, $updateThisUserDetail, $updateUserVerify) {
		$userid = mysqli_real_escape_string($this->db, $uid);
		$checkUserAdmin = mysqli_query($this->db, "SELECT user_id FROM dot_users WHERE user_id = '$userid' AND user_type = '1'") or die(mysqli_error($this->db));
		$checkUserExist = mysqli_query($this->db, "SELECT user_id FROM dot_users WHERE user_id = '$updateThisUserDetail'") or die(mysqli_error($this->db));
		if (mysqli_num_rows($checkUserAdmin) == 1 && mysqli_num_rows($checkUserExist) == 1) {
			$update = mysqli_query($this->db, "UPDATE dot_users SET can_post_location = '$updateUserVerify' WHERE user_id = '$updateThisUserDetail'") or die(mysqli_error($this->db));
			return true;
		} else {return false;}
	}
	public function Dot_UpdateUserCanShareBeforeAfterPost($uid, $updateThisUserDetail, $updateUserVerify) {
		$userid = mysqli_real_escape_string($this->db, $uid);
		$checkUserAdmin = mysqli_query($this->db, "SELECT user_id FROM dot_users WHERE user_id = '$userid' AND user_type = '1'") or die(mysqli_error($this->db));
		$checkUserExist = mysqli_query($this->db, "SELECT user_id FROM dot_users WHERE user_id = '$updateThisUserDetail'") or die(mysqli_error($this->db));
		if (mysqli_num_rows($checkUserAdmin) == 1 && mysqli_num_rows($checkUserExist) == 1) {
			$update = mysqli_query($this->db, "UPDATE dot_users SET can_post_bfaf = '$updateUserVerify' WHERE user_id = '$updateThisUserDetail'") or die(mysqli_error($this->db));
			return true;
		} else {return false;}
	}
	public function Dot_UpdateUserCanShareProductPost($uid, $updateThisUserDetail, $updateUserVerify) {
		$userid = mysqli_real_escape_string($this->db, $uid);
		$checkUserAdmin = mysqli_query($this->db, "SELECT user_id FROM dot_users WHERE user_id = '$userid' AND user_type = '1'") or die(mysqli_error($this->db));
		$checkUserExist = mysqli_query($this->db, "SELECT user_id FROM dot_users WHERE user_id = '$updateThisUserDetail'") or die(mysqli_error($this->db));
		if (mysqli_num_rows($checkUserAdmin) == 1 && mysqli_num_rows($checkUserExist) == 1) {
			$update = mysqli_query($this->db, "UPDATE dot_users SET can_post_product = '$updateUserVerify' WHERE user_id = '$updateThisUserDetail'") or die(mysqli_error($this->db));
			return true;
		} else {return false;}
	}
	public function Dot_UpdateUserCanShareWatermarkedPost($uid, $updateThisUserDetail, $updateUserVerify) {
		$userid = mysqli_real_escape_string($this->db, $uid);
		$checkUserAdmin = mysqli_query($this->db, "SELECT user_id FROM dot_users WHERE user_id = '$userid' AND user_type = '1'") or die(mysqli_error($this->db));
		$checkUserExist = mysqli_query($this->db, "SELECT user_id FROM dot_users WHERE user_id = '$updateThisUserDetail'") or die(mysqli_error($this->db));
		if (mysqli_num_rows($checkUserAdmin) == 1 && mysqli_num_rows($checkUserExist) == 1) {
			$update = mysqli_query($this->db, "UPDATE dot_users SET can_post_watermark = '$updateUserVerify' WHERE user_id = '$updateThisUserDetail'") or die(mysqli_error($this->db));
			return true;
		} else {return false;}
	}
	public function Dot_CreateNeweStreaming($uid, $streamingName) {
		$userid = mysqli_real_escape_string($this->db, $uid);
		$streamingName = mysqli_real_escape_string($this->db, $streamingName);
		$time = time();
		$checkThisUserCreatedLiveStreamingBefore = mysqli_query($this->db, "SELECT live_uid_fk,live_status FROM dot_live WHERE live_uid_fk = '$uid' AND live_status IN('0','1')") or die(mysqli_error($this->db));
		if (mysqli_num_rows($checkThisUserCreatedLiveStreamingBefore) == 1) {
			mysqli_query($this->db, "DELETE FROM dot_live WHERE live_uid_fk = '$uid'") or die(mysqli_error($this->db));
			$query = mysqli_query($this->db, "INSERT INTO `dot_live`(live_uid_fk,live_channel,live_time,live_status,started_at)VALUES('$uid','$streamingName','$time','1','$time')") or die(mysqli_error($this->db));
		} else {
			$query = mysqli_query($this->db, "INSERT INTO `dot_live`(live_uid_fk,live_channel,live_time,live_status,started_at)VALUES('$uid','$streamingName','$time','1','$time')") or die(mysqli_error($this->db));
		}
		$getLive = mysqli_query($this->db, "SELECT live_id,live_uid_fk,live_channel,live_time,live_status FROM dot_live WHERE live_uid_fk = '$uid' AND live_status = '1' ORDER BY live_id DESC LIMIT 1") or die(mysqli_error($this->db));
		$row = mysqli_fetch_array($getLive, MYSQLI_ASSOC);
		$liveID = $row['live_id'];
		$time = time();
		$query = mysqli_query($this->db, "INSERT INTO `dot_live_video_comments`(live_comment_uid_fk,start_live,live_video_comment_time,live_video_id)VALUES('$uid','1','$time','$liveID')") or die(mysqli_error($this->db));
		return $row;
	}
/*Get Live Video Details*/
	public function Dot_GetLivingVideo($uid) {
		$userid = mysqli_real_escape_string($this->db, $uid);
		$getLive = mysqli_query($this->db, "SELECT live_id,live_uid_fk,live_channel,live_time,live_status FROM dot_live WHERE live_uid_fk = '$uid' AND live_status = '1' ORDER BY live_id DESC LIMIT 1") or die(mysqli_error($this->db));
		$row = mysqli_fetch_array($getLive, MYSQLI_ASSOC);
		return $row;
	}
	public function Dot_CheckNewLiveVideoCommentLatest($liveVideoID) {
		$liveVideoID = mysqli_real_escape_string($this->db, $liveVideoID);

		$DataComments = mysqli_query($this->db, "SELECT  V.live_comment_id,V.live_comment_uid_fk,V.live_video_comment,V.start_live, V.live_video_comment_time, V.live_video_id,V.live_donate_type , U.user_name, U.user_fullname,U.verified_user,U.pro_user_type FROM dot_live_video_comments V, dot_users U WHERE  U.user_status = '1' AND V.live_comment_uid_fk = U.user_id AND V.live_video_id = '$liveVideoID' ORDER BY V.live_comment_id DESC LIMIT 1") or die(mysqli_error($this->db));
		$row = mysqli_fetch_array($DataComments, MYSQLI_ASSOC);
		return $row;
	}
/*Get The Last message From Real Time*/
	public function Dot_CheckNewLiveVideoComment($liveVideoID, $lastCommendID) {
		$liveVideoID = mysqli_real_escape_string($this->db, $liveVideoID);
		$lastCommendID = mysqli_real_escape_string($this->db, $lastCommendID);
		mysqli_query($this->db, "SET character_set_client=utf8mb4") or die(mysqli_error($this->db));
		mysqli_query($this->db, "SET character_set_connection=utf8mb4") or die(mysqli_error($this->db));
		$lasComment = '';
		if ($lastCommendID) {
			$lasComment = "AND V.live_comment_id > '" . $lastCommendID . "'";
		}
		$query = mysqli_query($this->db, "SELECT V.live_comment_id,V.live_comment_uid_fk,V.live_video_comment,V.start_live, V.live_video_comment_time, V.live_video_id,V.live_donate_type , U.user_name, U.user_fullname,U.verified_user,U.pro_user_type FROM dot_live_video_comments V, dot_users U WHERE U.user_status = '1' AND V.live_comment_uid_fk = U.user_id AND V.live_video_id = '$liveVideoID' AND V.start_live = '0' $lasComment ORDER BY V.live_comment_id ") or die(mysqli_error($this->db));
		$data = mysqli_fetch_array($query, MYSQLI_ASSOC);
		return $data;
	}
/*Insert a New Live Video Comment*/
	public function Dot_InsertiveComment($uid, $liveID, $liveComment) {
		$uid = mysqli_real_escape_string($this->db, $uid);
		$liveID = mysqli_real_escape_string($this->db, $liveID);
		$checkLiveVideo = mysqli_query($this->db, "SELECT live_id FROM dot_live WHERE live_id = '$liveID' AND comment_status = '1'") or die(mysqli_error($this->db));
		$checkUserExist = mysqli_query($this->db, "SELECT user_id FROM dot_users WHERE user_id = '$uid'") or die(mysqli_error($this->db));
		mysqli_query($this->db, "SET character_set_client=utf8mb4") or die(mysqli_error($this->db));
		mysqli_query($this->db, "SET character_set_connection=utf8mb4") or die(mysqli_error($this->db));
		if (mysqli_num_rows($checkLiveVideo) == 1 && mysqli_num_rows($checkUserExist) == 1) {
			$time = time();
			$insertQuery = mysqli_query($this->db, "INSERT INTO `dot_live_video_comments`(live_comment_uid_fk,live_video_comment,live_video_comment_time,live_video_id)VALUES('$uid','$liveComment','$time','$liveID')") or die(mysqli_error($this->db));
			$query = mysqli_query($this->db, "SELECT V.live_comment_id,V.live_comment_uid_fk,V.live_video_comment,V.start_live, V.live_video_comment_time, V.live_video_id , U.user_name, U.user_fullname,U.verified_user,U.pro_user_type FROM dot_live_video_comments V, dot_users U WHERE U.user_status = '1' AND V.live_comment_uid_fk = U.user_id AND V.live_video_id = '$liveID' AND V.start_live = '0' AND V.live_comment_uid_fk = '$uid' ORDER BY V.live_comment_id DESC LIMIT 1") or die(mysqli_error($this->db));
			$data = mysqli_fetch_array($query, MYSQLI_ASSOC);
			return $data;
		}
	}
/*Insert Live Time*/
	public function Dot_InsertMyOnlineStatus($uid, $liveID) {
		$uid = mysqli_real_escape_string($this->db, $uid);
		$liveID = mysqli_real_escape_string($this->db, $liveID);
		$checkLiveVideo = mysqli_query($this->db, "SELECT live_id FROM dot_live WHERE live_id = '$liveID'") or die(mysqli_error($this->db));
		$checkUserExist = mysqli_query($this->db, "SELECT user_id FROM dot_users WHERE user_id = '$uid'") or die(mysqli_error($this->db));
		if (mysqli_num_rows($checkLiveVideo) == 1 && mysqli_num_rows($checkUserExist) == 1) {
			$time = time();
			$checkUserExistInLive = mysqli_query($this->db, "SELECT live_user_uid_fk,live_video_id FROM dot_live_video_users WHERE live_video_id = '$liveID' AND live_user_uid_fk = '$uid'") or die(mysqli_error($this->db));
			if (mysqli_num_rows($checkUserExistInLive) == 1) {
				mysqli_query($this->db, "UPDATE dot_live_video_users SET live_time = '$time' WHERE  live_video_id = '$liveID' AND live_user_uid_fk = '$uid'") or die(mysqli_error($this->db));
			} else {
				mysqli_query($this->db, "INSERT INTO `dot_live_video_users`(live_user_uid_fk,live_time,live_video_id)VALUES('$uid','$time','$liveID')") or die(mysqli_error($this->db));
			}
		}
	}

// Total Online Users
	public function Dot_OnlineLiveVideoUserCount($uid, $liveID) {
		$uid = mysqli_real_escape_string($this->db, $uid);
		$liveID = mysqli_real_escape_string($this->db, $liveID);
		$checkLiveVideo = mysqli_query($this->db, "SELECT live_id FROM dot_live WHERE live_id = '$liveID'") or die(mysqli_error($this->db));
		$checkUserExist = mysqli_query($this->db, "SELECT user_id FROM dot_users WHERE user_id = '$uid'") or die(mysqli_error($this->db));
		...
		$query = mysqli_query($this->db, "SELECT COUNT(*) AS countLiveVideoUsers FROM dot_live_video_users WHERE live_video_id = '$liveID' AND FROM_UNIXTIME(live_time) > DATE_SUB(CURRENT_TIMESTAMP, INTERVAL 3 minute)") or die(mysqli_error($this->db));
		$row = mysqli_fetch_array($query, MYSQLI_ASSOC);
		if ($row) {
			return $row['countLiveVideoUsers'];
		} else {return 0;}
	}
/*Count Live Video Like in 1 minutes*/
	public function Dot_CountLikeLiveVideo($liveID) {
		$liveID = mysqli_real_escape_string($this->db, $liveID);
		$query = mysqli_query($this->db, "SELECT COUNT(*) AS countLiveVideoLikes FROM dot_live_video_likes WHERE live_video_id = '$liveID' AND FROM_UNIXTIME(liked_time) > DATE_SUB(CURRENT_TIMESTAMP, INTERVAL 1 minute)") or die(mysqli_error($this->db));
		$row = mysqli_fetch_array($query, MYSQLI_ASSOC);
		if ($row) {
			return $row['countLiveVideoLikes'];
		} else {return 0;}
	}
/*Insert Live Video Likes*/
	public function Dot_InserLiveLikeVideo($uid, $liveID) {
		$uid = mysqli_real_escape_string($this->db, $uid);
		$liveID = mysqli_real_escape_string($this->db, $liveID);
		$checkLiveVideo = mysqli_query($this->db, "SELECT live_id FROM dot_live WHERE live_id = '$liveID'") or die(mysqli_error($this->db));
		$checkUserExist = mysqli_query($this->db, "SELECT user_id FROM dot_users WHERE user_id = '$uid'") or die(mysqli_error($this->db));
		$checkUserLikedThisLiveVideoBefore = mysqli_query($this->db, "SELECT liked_uid_fk FROM dot_live_video_likes WHERE liked_uid_fk = '$uid'") or die(mysqli_error($this->db));
		if (mysqli_num_rows($checkLiveVideo) == 1 && mysqli_num_rows($checkUserExist) == 1 && mysqli_num_rows($checkUserLikedThisLiveVideoBefore) == 0) {
			$time = time();
			mysqli_query($this->db, "INSERT INTO `dot_live_video_likes`(liked_uid_fk,live_video_id,liked_time)VALUES('$uid','$liveID','$time')") or die(mysqli_error($this->db));
		} else {return false;}
	}
	public function Dot_UpdateLiveStatus($uid, $liveID) {
		mysqli_query($this->db, "UPDATE dot_live SET live_status = '0' WHERE  live_id = '$liveID' AND live_uid_fk = '$uid'") or die(mysqli_error($this->db));
	}
	public function Dot_UpdateLiveStatusIfExist($uid) {
		$checkLiveVideo = mysqli_query($this->db, "SELECT live_time FROM dot_live WHERE live_uid_fk = '$uid' AND live_status = '1'") or die(mysqli_error($this->db));
		if (mysqli_num_rows($checkLiveVideo) > 0) {
			mysqli_query($this->db, "UPDATE dot_live SET live_status = '0' WHERE live_uid_fk = '$uid' AND FROM_UNIXTIME(live_time) < DATE_SUB(CURRENT_TIMESTAMP, INTERVAL 1 minute)") or die(mysqli_error($this->db));
		}
	}
	public function Dot_UpdateLiveTime($uid, $liveID) {
		$checkLiveVideo = mysqli_query($this->db, "SELECT live_id FROM dot_live WHERE live_id = '$liveID' AND live_uid_fk = '$uid'") or die(mysqli_error($this->db));
		$checkUserExist = mysqli_query($this->db, "SELECT user_id FROM dot_users WHERE user_id = '$uid'") or die(mysqli_error($this->db));
		if (mysqli_num_rows($checkLiveVideo) == 1 && mysqli_num_rows($checkUserExist) == 1) {
			$time = time();
			mysqli_query($this->db, "UPDATE dot_live SET live_time = '$time' WHERE  live_id = '$liveID' AND live_uid_fk = '$uid' AND live_status = '1'") or die(mysqli_error($this->db));
		} else {return false;}
	}
	public function Dot_GetProductAmounts($productID, $uidfk) {
		$checkPostExist = mysqli_query($this->db, "SELECT user_post_id,user_id_fk FROM dot_user_posts WHERE user_post_id = '$productID' AND user_id_fk = '$uidfk'") or die(mysqli_error($this->db));
		$checkUserExist = mysqli_query($this->db, "SELECT user_id FROM dot_users WHERE user_id = '$uidfk'") or die(mysqli_error($this->db));
		if (mysqli_num_rows($checkPostExist) == 1 && mysqli_num_rows($checkUserExist) == 1) {
			$getQuery = mysqli_query($this->db, "SELECT product_normal_price,product_discount_price FROM dot_user_posts WHERE  user_post_id = '$productID' AND user_id_fk = '$uidfk'") or die(mysqli_error($this->db));
			$data = mysqli_fetch_array($getQuery, MYSQLI_ASSOC);
			return $data;
		} else {return false;}
	}
	public function Dot_FriendsLiveVideoStreams($uid) {
		$uid = mysqli_real_escape_string($this->db, $uid);
		$allFriendsQuery = mysqli_query($this->db, "SELECT DISTINCT F.friend_id, F.user_one, F.role, F.user_two, U.user_id,U.user_name,U.user_fullname, V.live_uid_fk,V.live_status,V.live_id,V.live_time FROM dot_users U INNER JOIN dot_friends F ON F.user_one = U.user_id INNER JOIN dot_live V ON F.user_two = V.live_uid_fk WHERE U.user_status = '1' AND F.user_one='$uid' AND F.role IN('flwr','fri')
		AND FROM_UNIXTIME(V.live_time) > (CURRENT_DATE - INTERVAL 24 HOUR)
	    AND FROM_UNIXTIME(V.live_time) < (CURRENT_DATE + INTERVAL 24 HOUR)
	    ORDER BY V.live_id DESC LIMIT " . $this->perpage) or die(mysqli_error($this->db));
		while ($row = mysqli_fetch_array($allFriendsQuery, MYSQLI_ASSOC)) {
			// Store the result into array
			$data[] = $row;
		}
		if (!empty($data)) {
	...
	public function Dot_UpdateAgora($uid, $agoraapp_id, $agora_customer_id, $agora_certificate) {
		$uid = mysqli_real_escape_string($this->db, $uid);
		$agoraapp_id = mysqli_real_escape_string($this->db, $agoraapp_id);
		$agora_customer_id = mysqli_real_escape_string($this->db, $agora_customer_id);
		$agora_certificate = mysqli_real_escape_string($this->db, $agora_certificate);

		$checkuserisAdmin = mysqli_query($this->db, "SELECT user_id FROM dot_users WHERE user_id = '$uid' AND user_type='1'") or die(mysqli_error($this->db));
		if (mysqli_num_rows($checkuserisAdmin) == 1) {
			$updateCargoInfos = mysqli_query($this->db, "UPDATE dot_configuration SET agora_app_id = '$agoraapp_id',agora_customer_id = '$agora_customer_id',agora_certificate = '$agora_certificate' WHERE  configuration_id = '1'") or die(mysqli_error($this->db));
			return true;
		} else {return false;}
	}
/*Update Maintenance Mode*/
	public function Dot_AgoraModeUpdate($uid, $AgoraModde) {
		$uid = mysqli_real_escape_string($this->db, $uid);
		$AgoraModde = mysqli_real_escape_string($this->db, $AgoraModde);
		mysqli_query($this->db, "SET character_set_client=utf8") or die(mysqli_error($this->db));
		mysqli_query($this->db, "SET character_set_connection=utf8") or die(mysqli_error($this->db));
		$checkUserisAdmin = mysqli_query($this->db, "SELECT user_id,user_status,user_type FROM dot_users WHERE user_id = '$uid' AND user_status = '1' AND user_type = '1'") or die(mysqli_error($this->db));
		if (mysqli_num_rows($checkUserisAdmin) == 1) {
			$updateSiteSetting = mysqli_query($this->db, "UPDATE dot_configuration SET agora_status = '$AgoraModde' WHERE configuration_id = '1'") or die(mysqli_error($this->db));
			return true;
		} else {
			return false;
		}
	}
	public function Dot_DeleteEventRequest($uid, $evi) {
		$uid = mysqli_real_escape_string($this->db, $uid);
		$evi = mysqli_real_escape_string($this->db, $evi);
		$checkEventInviteExist = mysqli_query($this->db, "SELECT invited_user_id,invited_event_id FROM dot_event_invites WHERE invited_user_id = '$uid' AND invited_event_id = '$evi'") or die(mysqli_error($this->db));
		$checkUserExist = mysqli_query($this->db, "SELECT user_id FROM dot_users WHERE user_id = '$uid'") or die(mysqli_error($this->db));
		if (mysqli_num_rows($checkEventInviteExist) == 1 && mysqli_num_rows($checkUserExist) == 1) {
			mysqli_query($this->db, "DELETE FROM dot_event_invites WHERE  invited_user_id = '$uid' AND invited_event_id = '$evi'") or die(mysqli_error($this->db));
			return true;
		} else {
			return false;
		}
	}
	public function Dot_DeleteLiveStreamingFromData($uid, $liveStreamingID) {
		$uid = mysqli_real_escape_string($this->db, $uid);
		$liveStreamingID = mysqli_real_escape_string($this->db, $liveStreamingID);
		$checkLiveStreamingExist = mysqli_query($this->db, "SELECT live_id FROM dot_live WHERE live_id = '$liveStreamingID'") or die(mysqli_error($this->db));
		$checkuserisAdmin = mysqli_query($this->db, "SELECT user_id FROM dot_users WHERE user_id = '$uid' AND user_type='1'") or die(mysqli_error($this->db));
		if (mysqli_num_rows($checkLiveStreamingExist) == 1 && mysqli_num_rows($checkuserisAdmin) == 1) {
			mysqli_query($this->db, "DELETE FROM dot_live WHERE  live_id = '$liveStreamingID'") or die(mysqli_error($this->db));
			return true;
		} else {
			return false;
		}
	}
	public function Dot_StartCloudRecording($vendor, $region, $bucket, $accessKey, $secretKey, $cname, $uid, $post_id, $agoraAppID, $agoraCustomerID, $agoraCertificate) {
		$ch = curl_init();
    ...
    "clientRequest":{
		...
		if (!empty($data->sid) && !empty($resourceId)) {
			mysqli_query($this->db, "UPDATE dot_live SET a_resource_id = '" . $resourceId . "', a_sid = '" . $data->sid . "' WHERE live_id = '$post_id'") or die(mysqli_error($this->db));
		}
		return true;
	}
	public function Dot_GetCommentStatus($liveID) {
		$liveID = mysqli_real_escape_string($this->db, $liveID);
		$checkLiveVideo = mysqli_query($this->db, "SELECT live_id FROM dot_live WHERE live_id = '$liveID'") or die(mysqli_error($this->db));
		if (mysqli_num_rows($checkLiveVideo) == 1) {
			$getLiveVideoCommentStatus = mysqli_query($this->db, "SELECT comment_status FROM dot_live WHERE live_id = '$liveID'") or die(mysqli_error($this->db));
			$data = mysqli_fetch_array($getLiveVideoCommentStatus, MYSQLI_ASSOC);
			return $data['comment_status'];
		}
	}
/*Update Live Video Mode*/
	public function Dot_LiveVideoMode($uid, $LiveModde) {
		$uid = mysqli_real_escape_string($this->db, $uid);
		$LiveModdev = mysqli_real_escape_string($this->db, $LiveModde);
		$checkUserisAdmin = mysqli_query($this->db, "SELECT user_id,user_status,user_type FROM dot_users WHERE user_id = '$uid' AND user_status = '1' AND user_type = '1'") or die(mysqli_error($this->db));
		if (mysqli_num_rows($checkUserisAdmin) == 1) {
			$updateSiteSetting = mysqli_query($this->db, "UPDATE dot_configuration SET live_video_status = '$LiveModde' WHERE configuration_id = '1'") or die(mysqli_error($this->db));
			return true;
		} else {
			return false;
		}
...
/*Save Live Videos*/
	public function Dot_SaveLiveVideosMode($uid, $sLiveModde) {
		$uid = mysqli_real_escape_string($this->db, $uid);
		$sLiveModde = mysqli_real_escape_string($this->db, $sLiveModde);
		$checkUserisAdmin = mysqli_query($this->db, "SELECT user_id,user_status,user_type FROM dot_users WHERE user_id = '$uid' AND user_status = '1' AND user_type = '1'") or die(mysqli_error($this->db));
		if (mysqli_num_rows($checkUserisAdmin) == 1) {
			$updateSiteSetting = mysqli_query($this->db, "UPDATE dot_configuration SET save_live_videos = '$sLiveModde' WHERE configuration_id = '1'") or die(mysqli_error($this->db));
			return true;
		} else {
			return false;
		}
	}
/*Save Live Videos from Amazon*/
	public function Dot_SaveLiveVideosToAmazonMode($uid, $sALiveModde) {
		$uid = mysqli_real_escape_string($this->db, $uid);
...
/*Save Live Videos from Amazon*/
	public function Dot_SaveLiveVideosToAmazons3Mode($uid, $sALiveModde) {
		$uid = mysqli_real_escape_string($this->db, $uid);
		$sALiveModde = mysqli_real_escape_string($this->db, $sALiveModde);
		$checkUserisAdmin = mysqli_query($this->db, "SELECT user_id,user_status,user_type FROM dot_users WHERE user_id = '$uid' AND user_status = '1' AND user_type = '1'") or die(mysqli_error($this->db));
		if (mysqli_num_rows($checkUserisAdmin) == 1) {
			$updateSiteSetting = mysqli_query($this->db, "UPDATE dot_configuration SET s3_status = '$sALiveModde' WHERE configuration_id = '1'") or die(mysqli_error($this->db));
			return true;
		} else {
			return false;
		}
	}
/*Update Amazon S3 Details*/
	public function Dot_AmazonS3Update($uid, $theNewBucketName, $theNewAmazonKey, $theNewAmazonSecretKey, $theNewAmazonRegion) {
		$uid = mysqli_real_escape_string($this->db, $uid);
		$checkUserisAdmin = mysqli_query($this->db, "SELECT user_id,user_status,user_type FROM dot_users WHERE user_id = '$uid' AND user_status = '1' AND user_type = '1'") or die(mysqli_error($this->db));
		if (mysqli_num_rows($checkUserisAdmin) == 1) {
			$updateSiteSetting = mysqli_query($this->db, "UPDATE dot_configuration SET bucket_name_2 = '$theNewBucketName', amazone_s3_key_2 = '$theNewAmazonKey', amazone_s3_s_key_2 = '$theNewAmazonSecretKey', region_2 = '$theNewAmazonRegion' WHERE configuration_id = '1'") or die(mysqli_error($this->db));
			return true;
		} else {
			return false;
		}
	}
	public function Dot_UpdateLiveStreamingVideoWidth($uid, $updateLiveStreamingVideoWidth) {
		$uid = mysqli_real_escape_string($this->db, $uid);
		$updateLiveStreamingVideoWidth = mysqli_real_escape_string($this->db, $updateLiveStreamingVideoWidth);
		$checkUserisAdmin = mysqli_query($this->db, "SELECT user_id,user_status,user_type FROM dot_users WHERE user_id = '$uid' AND user_status = '1' AND user_type = '1'") or die(mysqli_error($this->db));
		if (mysqli_num_rows($checkUserisAdmin) == 1) {
			$updateSiteSetting = mysqli_query($this->db, "UPDATE dot_configuration SET streaming_video_with = '$updateLiveStreamingVideoWidth' WHERE configuration_id = '1'") or die(mysqli_error($this->db));
			return true;
		} else {
			return false;
		}
...
/*Check All is Ok*/
	public function Dot_CheckAllisOkForLive($uid, $donateThisLive, $donateThisUser) {
		$uid = mysqli_real_escape_string($this->db, $uid);
		$donateThisLive = mysqli_real_escape_string($this->db, $donateThisLive);
		$donateThisUser = mysqli_real_escape_string($this->db, $donateThisUser);
		$CheckUserExist = mysqli_query($this->db, "SELECT user_id FROM dot_users WHERE user_id= '$uid' AND user_status = '1'") or die(mysqli_error($this->db));
		$CheckLiveStreamExist = mysqli_query($this->db, "SELECT live_id FROM dot_live WHERE live_id= '$donateThisLive' AND live_uid_fk = '$donateThisUser'") or die(mysqli_error($this->db));
		if (mysqli_num_rows($CheckUserExist) == 1 && mysqli_num_rows($CheckLiveStreamExist) == 1) {
			return true;
		} else {
			return false;
		}
	}
	public function Dot_GetLikeLiveStatus($liveID) {
		$liveID = mysqli_real_escape_string($this->db, $liveID);
		$checkLiveVideo = mysqli_query($this->db, "SELECT live_id FROM dot_live WHERE live_id = '$liveID'") or die(mysqli_error($this->db));
		if (mysqli_num_rows($checkLiveVideo) == 1) {
			$getLiveVideoCommentStatus = mysqli_query($this->db, "SELECT like_status FROM dot_live WHERE live_id = '$liveID'") or die(mysqli_error($this->db));
			$data = mysqli_fetch_array($getLiveVideoCommentStatus, MYSQLI_ASSOC);
			return $data['like_status'];
		}
	}
	public function Dot_GetDonatetLiveStatus($liveID) {
		$liveID = mysqli_real_escape_string($this->db, $liveID);
		$checkLiveVideo = mysqli_query($this->db, "SELECT live_id FROM dot_live WHERE live_id = '$liveID'") or die(mysqli_error($this->db));
		if (mysqli_num_rows($checkLiveVideo) == 1) {
			$getLiveVideoCommentStatus = mysqli_query($this->db, "SELECT donate_status FROM dot_live WHERE live_id = '$liveID'") or die(mysqli_error($this->db));
			$data = mysqli_fetch_array($getLiveVideoCommentStatus, MYSQLI_ASSOC);
			return $data['donate_status'];
		}
	}
	public function Dot_UpdateLiveStreamingCommentStatus($uid, $liveStramID, $status) {
		$uid = mysqli_real_escape_string($this->db, $uid);
		$liveStramID = mysqli_real_escape_string($this->db, $liveStramID);
		$status = mysqli_real_escape_string($this->db, $status);
		$checkLiveVideo = mysqli_query($this->db, "SELECT live_id FROM dot_live WHERE live_id = '$liveStramID' AND live_uid_fk = '$uid'") or die(mysqli_error($this->db));
		$checkUserExist = mysqli_query($this->db, "SELECT user_id FROM dot_users WHERE user_id = '$uid'") or die(mysqli_error($this->db));
		if (mysqli_num_rows($checkLiveVideo) == 1 && mysqli_num_rows($checkUserExist) == 1) {
			$updateSiteSetting = mysqli_query($this->db, "UPDATE dot_live SET comment_status = '$status' WHERE  live_id = '$liveStramID' AND live_uid_fk = '$uid'") or die(mysqli_error($this->db));
			return true;
		} else {return false;}
	}
	public function Dot_UpdateLiveStreamingLikeStatus($uid, $liveStramID, $status) {
		$uid = mysqli_real_escape_string($this->db, $uid);
		$liveStramID = mysqli_real_escape_string($this->db, $liveStramID);
		$status = mysqli_real_escape_string($this->db, $status);
		$checkLiveVideo = mysqli_query($this->db, "SELECT live_id FROM dot_live WHERE live_id = '$liveStramID' AND live_uid_fk = '$uid'") or die(mysqli_error($this->db));
		$checkUserExist = mysqli_query($this->db, "SELECT user_id FROM dot_users WHERE user_id = '$uid'") or die(mysqli_error($this->db));
		if (mysqli_num_rows($checkLiveVideo) == 1 && mysqli_num_rows($checkUserExist) == 1) {
			$updateSiteSetting = mysqli_query($this->db, "UPDATE dot_live SET like_status = '$status' WHERE  live_id = '$liveStramID' AND live_uid_fk = '$uid'") or die(mysqli_error($this->db));
			return true;
		} else {return false;}
	}
	public function Dot_UpdateLiveStreamingDonationStatus($uid, $liveStramID, $status) {
		$uid = mysqli_real_escape_string($this->db, $uid);
		$liveStramID = mysqli_real_escape_string($this->db, $liveStramID);
		$status = mysqli_real_escape_string($this->db, $status);
		$checkLiveVideo = mysqli_query($this->db, "SELECT live_id FROM dot_live WHERE live_id = '$liveStramID' AND live_uid_fk = '$uid'") or die(mysqli_error($this->db));
		$checkUserExist = mysqli_query($this->db, "SELECT user_id FROM dot_users WHERE user_id = '$uid'") or die(mysqli_error($this->db));
		if (mysqli_num_rows($checkLiveVideo) == 1 && mysqli_num_rows($checkUserExist) == 1) {
			$updateSiteSetting = mysqli_query($this->db, "UPDATE dot_live SET donate_status = '$status' WHERE  live_id = '$liveStramID' AND live_uid_fk = '$uid'") or die(mysqli_error($this->db));
			return true;
		} else {return false;}
	}
	public function Dot_Filters() {
		$query = mysqli_query($this->db, "SELECT id, fkey FROM dot_filters") or die(mysqli_error($this->db));
		while ($row = mysqli_fetch_array($query, MYSQLI_ASSOC)) {
			// Store the result into array
			$data[] = $row;
		}
	...
	public function Dot_UpdateLiveVideoFilter($uid, $status, $liveStramID) {
		$uid = mysqli_real_escape_string($this->db, $uid);
		$liveStramID = mysqli_real_escape_string($this->db, $liveStramID);
		$status = mysqli_real_escape_string($this->db, $status);
		$checkLiveVideo = mysqli_query($this->db, "SELECT live_id FROM dot_live WHERE live_id = '$liveStramID' AND live_uid_fk = '$uid'") or die(mysqli_error($this->db));
		$checkUserExist = mysqli_query($this->db, "SELECT user_id FROM dot_users WHERE user_id = '$uid'") or die(mysqli_error($this->db));
		if (mysqli_num_rows($checkLiveVideo) == 1 && mysqli_num_rows($checkUserExist) == 1) {
			$updateSiteSetting = mysqli_query($this->db, "UPDATE dot_live SET live_video_filter = '$status' WHERE  live_id = '$liveStramID' AND live_uid_fk = '$uid'") or die(mysqli_error($this->db));
			return true;
		} else {return false;}
	}
	public function Dot_GetFilterLiveStatus($liveID) {
		$liveID = mysqli_real_escape_string($this->db, $liveID);
		$checkLiveVideo = mysqli_query($this->db, "SELECT live_id FROM dot_live WHERE live_id = '$liveID'") or die(mysqli_error($this->db));
		if (mysqli_num_rows($checkLiveVideo) == 1) {
			$getLiveVideoCommentStatus = mysqli_query($this->db, "SELECT live_video_filter FROM dot_live WHERE live_id = '$liveID'") or die(mysqli_error($this->db));
			$data = mysqli_fetch_array($getLiveVideoCommentStatus, MYSQLI_ASSOC);
			return $data['live_video_filter'];
		}
	}
	public function Dot_UpdateUserTypeAdminorNormalUser($uid, $updateThisUserDetail, $updateUserVerify) {
		$userid = mysqli_real_escape_string($this->db, $uid);
		$checkUserAdmin = mysqli_query($this->db, "SELECT user_id FROM dot_users WHERE user_id = '$userid' AND user_type = '1'") or die(mysqli_error($this->db));
		$checkUserExist = mysqli_query($this->db, "SELECT user_id FROM dot_users WHERE user_id = '$updateThisUserDetail'") or die(mysqli_error($this->db));
		if (mysqli_num_rows($checkUserAdmin) == 1 && mysqli_num_rows($checkUserExist) == 1) {
			$update = mysqli_query($this->db, "UPDATE dot_users SET user_type = '$updateUserVerify' WHERE user_id = '$updateThisUserDetail'") or die(mysqli_error($this->db));
			return true;
		} else {return false;}
	}
	public function Dot_UpdateUserTypeProOrNormalUser($uid, $updateThisUserDetail, $updateUserVerify) {
		$userid = mysqli_real_escape_string($this->db, $uid);
		$checkUserAdmin = mysqli_query($this->db, "SELECT user_id FROM dot_users WHERE user_id = '$userid' AND user_type = '1'") or die(mysqli_error($this->db));
		$checkUserExist = mysqli_query($this->db, "SELECT user_id FROM dot_users WHERE user_id = '$updateThisUserDetail'") or die(mysqli_error($this->db));
		if (mysqli_num_rows($checkUserAdmin) == 1 && mysqli_num_rows($checkUserExist) == 1) {
			$update = mysqli_query($this->db, "UPDATE dot_users SET pro_user_type = '$updateUserVerify' WHERE user_id = '$updateThisUserDetail'") or die(mysqli_error($this->db));
			return true;
		} else {return false;}
	}
/*Check Payment is Pro*/
	public function Dot_CheckPaymentIsLiveBroadCasting($puid) {
		$puid = mysqli_real_escape_string($this->db, $puid);
		$getLatestPayment = mysqli_query($this->db, "SELECT live_video_id,product_owner_id,payment_user_id,payment_type FROM dot_payments WHERE payment_user_id = '$puid' AND payment_type='liveDonate' AND FROM_UNIXTIME(payment_time) > DATE_SUB(CURRENT_TIMESTAMP, INTERVAL 2 minute) ORDER BY payment_id DESC LIMIT 1") or die(mysqli_error($this->db));
		$dataLastPayment = mysqli_fetch_array($getLatestPayment, MYSQLI_ASSOC);
		if ($dataLastPayment) {return $dataLastPayment;} else {return false;}
	}
/*Insert Live Video Payment from Comment*/
	public function Dot_InsertPaymentFromLiveVideo($data_uid, $liveVideoOwner, $liveVideoID) {
		$checkUserExist = mysqli_query($this->db, "SELECT user_id FROM dot_users WHERE user_id = '$data_uid' AND user_status = '1'") or die(mysqli_error($this->db));
		if (mysqli_num_rows($checkUserExist) == 1) {
			$time = time();
			mysqli_query($this->db, "INSERT INTO `dot_live_video_comments`(live_comment_uid_fk,live_video_id,live_video_comment_time,live_donate_type)VALUES('$data_uid','$liveVideoID','$time','1')") or die(mysqli_error($this->db));
			return true;
		} else {return false;}
	}
/*Update Watermark Status*/
	public function Dot_UpdateWatermarkStatus($uid, $status) {
		$uid = mysqli_real_escape_string($this->db, $uid);
		$status = mysqli_real_escape_string($this->db, $status);
		$checkUserAdmin = mysqli_query($this->db, "SELECT user_id FROM dot_users WHERE user_id = '$uid' AND user_type = '1'") or die(mysqli_error($this->db));
		if (mysqli_num_rows($checkUserAdmin) == 1) {
			mysqli_query($this->db, "UPDATE dot_configuration SET watermark_status = '$status' WHERE  configuration_id = '1'") or die(mysqli_error($this->db));
			return true;
		} else {return false;}
	}
	public function Dot_AddStoryPoint($uid, $pointType, $pointPost, $votePostID) {
		$uid = mysqli_real_escape_string($this->db, $uid);
		$pointType = mysqli_real_escape_string($this->db, $pointType);
		$votePostID = mysqli_real_escape_string($this->db, $votePostID);
		$pointPost = mysqli_real_escape_string($this->db, $pointPost);
	...
	public function Dot_RemoveStoryPoint($uid, $storyID) {
		$query = mysqli_query($this->db, "SELECT user_id FROM dot_users WHERE user_id = '$uid' AND user_status = '1'") or die(mysqli_error($this->db));
		$checkStoryBefore = mysqli_query($this->db, "SELECT poninted_post_id,poninted_user_id FROM dot_user_point_earnings WHERE poninted_post_id = '$storyID' AND poninted_user_id = '$uid' AND pointed_type = 'story'") or die(mysqli_error($this->db));
		if (mysqli_num_rows($query) == 1 && mysqli_num_rows($checkStoryBefore) == '1') {
			mysqli_query($this->db, "DELETE FROM dot_user_point_earnings WHERE poninted_post_id='$storyID' AND poninted_user_id='$uid' AND pointed_type = 'story'") or die(mysqli_error($this->db));
			return true;
		} else {return false;}
	}
	// Enable Disable Profile Sexuality Feature
	public function Dot_EnableDisableProfileProfileSexuality($uid, $showHide) {
		$uid = mysqli_real_escape_string($this->db, $uid);
		$showHide = mysqli_real_escape_string($this->db, $showHide);
		$checkUserisAdmin = mysqli_query($this->db, "SELECT user_id FROM dot_users WHERE user_id = '$uid' AND user_type= '1'") or die(mysqli_error($this->db));
		if (mysqli_num_rows($checkUserisAdmin) > 0) {
			$UpdateMode = mysqli_query($this->db, "UPDATE dot_configuration SET ed_sexuality = '$showHide' WHERE  configuration_id = '1'") or die(mysqli_error($this->db));
			return true;
		} else {
			return false;
		}
	}
	// Enable Disable User Height Feature
	public function Dot_EnableDisableUserHeight($uid, $showHide) {
		$uid = mysqli_real_escape_string($this->db, $uid);
		$showHide = mysqli_real_escape_string($this->db, $showHide);
		$checkUserisAdmin = mysqli_query($this->db, "SELECT user_id FROM dot_users WHERE user_id = '$uid' AND user_type= '1'") or die(mysqli_error($this->db));
		if (mysqli_num_rows($checkUserisAdmin) > 0) {
			$UpdateMode = mysqli_query($this->db, "UPDATE dot_configuration SET ed_height = '$showHide' WHERE  configuration_id = '1'") or die(mysqli_error($this->db));
			return true;
		} else {
			return false;
		}
	}
	// Enable Disable User Weight Feature
	public function Dot_EnableDisableUserWeight($uid, $showHide) {
		$uid = mysqli_real_escape_string($this->db, $uid);
		$showHide = mysqli_real_escape_string($this->db, $showHide);
		$checkUserisAdmin = mysqli_query($this->db, "SELECT user_id FROM dot_users WHERE user_id = '$uid' AND user_type= '1'") or die(mysqli_error($this->db));
		if (mysqli_num_rows($checkUserisAdmin) > 0) {
			$UpdateMode = mysqli_query($this->db, "UPDATE dot_configuration SET ed_weight = '$showHide' WHERE  configuration_id = '1'") or die(mysqli_error($this->db));
			return true;
		} else {
			return false;
		}
	}
	// Enable Disable User Life Style Feature
	public function Dot_EnableDisableUserLifeStyle($uid, $showHide) {
		$uid = mysqli_real_escape_string($this->db, $uid);
		$showHide = mysqli_real_escape_string($this->db, $showHide);
		$checkUserisAdmin = mysqli_query($this->db, "SELECT user_id FROM dot_users WHERE user_id = '$uid' AND user_type= '1'") or die(mysqli_error($this->db));
		if (mysqli_num_rows($checkUserisAdmin) > 0) {
			$UpdateMode = mysqli_query($this->db, "UPDATE dot_configuration SET ed_lifestyle = '$showHide' WHERE  configuration_id = '1'") or die(mysqli_error($this->db));
			return true;
		} else {
			return false;
		}
	}
	// Enable Disable User Children Feature
	public function Dot_EnableDisableUserChildren($uid, $showHide) {
		$uid = mysqli_real_escape_string($this->db, $uid);
		$showHide = mysqli_real_escape_string($this->db, $showHide);
		$checkUserisAdmin = mysqli_query($this->db, "SELECT user_id FROM dot_users WHERE user_id = '$uid' AND user_type= '1'") or die(mysqli_error($this->db));
		if (mysqli_num_rows($checkUserisAdmin) > 0) {
			$UpdateMode = mysqli_query($this->db, "UPDATE dot_configuration SET ed_children = '$showHide' WHERE  configuration_id = '1'") or die(mysqli_error($this->db));
			return true;
		} else {
			return false;
		}
	}
	// Enable Disable User Smoking Feature
	public function Dot_EnableDisableUserSmoking($uid, $showHide) {
		$uid = mysqli_real_escape_string($this->db, $uid);
		$showHide = mysqli_real_escape_string($this->db, $showHide);
		$checkUserisAdmin = mysqli_query($this->db, "SELECT user_id FROM dot_users WHERE user_id = '$uid' AND user_type= '1'") or die(mysqli_error($this->db));
		if (mysqli_num_rows($checkUserisAdmin) > 0) {
			$UpdateMode = mysqli_query($this->db, "UPDATE dot_configuration SET ed_smoking = '$showHide' WHERE  configuration_id = '1'") or die(mysqli_error($this->db));
			return true;
		} else {
			return false;
		}
	}
	// Enable Disable User Drinking Feature
	public function Dot_EnableDisableUserDrinking($uid, $showHide) {
		$uid = mysqli_real_escape_string($this->db, $uid);
		$showHide = mysqli_real_escape_string($this->db, $showHide);
		$checkUserisAdmin = mysqli_query($this->db, "SELECT user_id FROM dot_users WHERE user_id = '$uid' AND user_type= '1'") or die(mysqli_error($this->db));
		if (mysqli_num_rows($checkUserisAdmin) > 0) {
			$UpdateMode = mysqli_query($this->db, "UPDATE dot_configuration SET ed_drinking = '$showHide' WHERE  configuration_id = '1'") or die(mysqli_error($this->db));
			return true;
		} else {
			return false;
		}
	}
	// Enable Disable User Body Type Feature
	public function Dot_EnableDisableUserBodyType($uid, $showHide) {
		$uid = mysqli_real_escape_string($this->db, $uid);
		$showHide = mysqli_real_escape_string($this->db, $showHide);
		$checkUserisAdmin = mysqli_query($this->db, "SELECT user_id FROM dot_users WHERE user_id = '$uid' AND user_type= '1'") or die(mysqli_error($this->db));
		if (mysqli_num_rows($checkUserisAdmin) > 0) {
			$UpdateMode = mysqli_query($this->db, "UPDATE dot_configuration SET ed_body_type = '$showHide' WHERE  configuration_id = '1'") or die(mysqli_error($this->db));
			return true;
		} else {
			return false;
		}
	}
	// Enable Disable User Hair Color Feature
	public function Dot_EnableDisableUserHairColor($uid, $showHide) {
		$uid = mysqli_real_escape_string($this->db, $uid);
		$showHide = mysqli_real_escape_string($this->db, $showHide);
		$checkUserisAdmin = mysqli_query($this->db, "SELECT user_id FROM dot_users WHERE user_id = '$uid' AND user_type= '1'") or die(mysqli_error($this->db));
		if (mysqli_num_rows($checkUserisAdmin) > 0) {
			$UpdateMode = mysqli_query($this->db, "UPDATE dot_configuration SET ed_hair_color = '$showHide' WHERE  configuration_id = '1'") or die(mysqli_error($this->db));
			return true;
		} else {
			return false;
		}
	}
	// Enable Disable User Eye Color Feature
	public function Dot_EnableDisableUserEyeColor($uid, $showHide) {
		$uid = mysqli_real_escape_string($this->db, $uid);
		$showHide = mysqli_real_escape_string($this->db, $showHide);
		$checkUserisAdmin = mysqli_query($this->db, "SELECT user_id FROM dot_users WHERE user_id = '$uid' AND user_type= '1'") or die(mysqli_error($this->db));
		if (mysqli_num_rows($checkUserisAdmin) > 0) {
			$UpdateMode = mysqli_query($this->db, "UPDATE dot_configuration SET ed_eye_color = '$showHide' WHERE  configuration_id = '1'") or die(mysqli_error($this->db));
			return true;
		} else {
			return false;
		}
...
// Update User Country
	public function Dot_UpdateUserCountry($uid, $ucCode, $uip, $userLatitude, $userLongitude) {
		$uid = mysqli_real_escape_string($this->db, $uid);
		$ucCode = mysqli_real_escape_string($this->db, $ucCode);
		$uip = mysqli_real_escape_string($this->db, $uip);
		$checkUserisExist = mysqli_query($this->db, "SELECT user_id FROM dot_users WHERE user_id = '$uid'") or die(mysqli_error($this->db));
		if (mysqli_num_rows($checkUserisExist) == 1) {
			mysqli_query($this->db, "UPDATE dot_users SET country = '$ucCode' , user_ip = '$uip', ulat = '$userLatitude', ulong = '$userLongitude' WHERE  user_id = '$uid'") or die(mysqli_error($this->db));
			return true;
		} else {
			return false;
		}
	}
/*Get Last Shared Status*/
	public function Dot_GetLastSharedStatus($uid) {
		$uid = mysqli_real_escape_string($this->db, $uid);
		$getLastStory = mysqli_query($this->db, "SELECT uid_fk, stories_img FROM dot_user_stories WHERE uid_fk = '$uid' ORDER BY s_id DESC LIMIT 1") or die(mysqli_error($this->db));
		$row = mysqli_fetch_array($getLastStory, MYSQLI_ASSOC);
		$lastStoryImg = $row['stories_img'];
...
/*Update Product Details*/
	public function Dot_UpdateMyProductDetails($uid, $newProductTitle, $newProductDetail, $newProductCategory, $newProductPrice, $newProductPlace, $newProducttumbs, $newProductCurrency, $productID, $slug) {
		$uid = mysqli_real_escape_string($this->db, $uid);
		$productID = mysqli_real_escape_string($this->db, $productID);
		mysqli_query($this->db, "SET character_set_client=utf8mb4") or die(mysqli_error($this->db));
		mysqli_query($this->db, "SET character_set_connection=utf8mb4") or die(mysqli_error($this->db));
		$checkPostExist = mysqli_query($this->db, "SELECT user_post_id, user_id_fk FROM dot_user_posts WHERE user_post_id = '$productID' AND user_id_fk = '$uid'") or die(mysqli_error($this->db));
		$checkUserExist = mysqli_query($this->db, "SELECT user_id FROM dot_users WHERE user_id = '$uid'") or die(mysqli_error($this->db));
		if (mysqli_num_rows($checkPostExist) == 1 && mysqli_num_rows($checkUserExist) == 1) {
			$updateProductDetails = mysqli_query($this->db, "UPDATE dot_user_posts SET
			slug = '$slug',
			m_product_name = '$newProductTitle',
			m_product_description = '$newProductDetail',
			product_images = '$newProducttumbs',
			product_category = '$newProductCategory',
			product_normal_price = '$newProductPrice',
			product_currency = '$newProductCurrency'
	   WHERE user_post_id = '$productID' AND user_id_fk = '$uid'") or die(mysqli_error($this->db));
			return true;
		} else {
			return false;
		}
...
/*Get Live Payment Details*/
	public function Dot_GetLiveDonationDetails($uid) {
		$getLatestPayment = mysqli_query($this->db, "SELECT payment_post_id,customer_user_id,payment_user_id,product_owner_id FROM dot_payments WHERE payment_user_id = '$uid' AND payment_type = 'liveDonate' AND payment_post_id IS NULL AND FROM_UNIXTIME(payment_time) > DATE_SUB(CURRENT_TIMESTAMP, INTERVAL 2 minute) ORDER BY payment_id DESC LIMIT 1") or die(mysqli_error($this->db));
		$dataLastPayment = mysqli_fetch_array($getLatestPayment, MYSQLI_ASSOC);
		if (mysqli_num_rows($getLatestPayment) == 1) {
			$paymentOwnerID = $dataLastPayment['product_owner_id'];
			$query = mysqli_query($this->db, "SELECT user_name FROM `dot_users` WHERE user_id='$paymentOwnerID' AND user_status='1'") or die(mysqli_error($this->db));
			$data = mysqli_fetch_array($query, MYSQLI_ASSOC);
			if ($data['user_name']) {
				$name = $data['user_name'];
				return $name;
...
/*Add New Country*/
	public function Dot_SaveNewCountry($uid, $countryName, $countryShortName, $countryPhoneCode) {
		$uid = mysqli_real_escape_string($this->db, $uid);
		$checkUserisAdmin = mysqli_query($this->db, "SELECT user_id FROM dot_users WHERE user_id = '$uid' AND user_type= '1'") or die(mysqli_error($this->db));
		$checkCountryShoryCodeExist = mysqli_query($this->db, "SELECT sortname FROM dot_countries WHERE sortname = '$countryShortName'") or die(mysqli_error($this->db));
		mysqli_query($this->db, "SET character_set_client=utf8") or die(mysqli_error($this->db));
		mysqli_query($this->db, "SET character_set_connection=utf8") or die(mysqli_error($this->db));
		if (mysqli_num_rows($checkUserisAdmin) == 1 && mysqli_num_rows($checkCountryShoryCodeExist) == 0) {
			mysqli_query($this->db, "INSERT INTO dot_countries(sortname, phonecode, name)VALUES('$countryShortName','$countryPhoneCode','$countryName')") or die(mysqli_error($this->db));
			return true;
		} else {
			return false;
		}
	}
/*Add New City from Country*/
	public function Dot_SaveNewCity($uid, $cityCountry, $cityName) {
		$checkUserisAdmin = mysqli_query($this->db, "SELECT user_id FROM dot_users WHERE user_id = '$uid' AND user_type= '1'") or die(mysqli_error($this->db));
		$checkCountryShoryCodeExist = mysqli_query($this->db, "SELECT id FROM dot_countries WHERE id = '$cityCountry'") or die(mysqli_error($this->db));
		mysqli_query($this->db, "SET character_set_client=utf8") or die(mysqli_error($this->db));
		mysqli_query($this->db, "SET character_set_connection=utf8") or die(mysqli_error($this->db));
		if (mysqli_num_rows($checkUserisAdmin) == 1 && mysqli_num_rows($checkCountryShoryCodeExist) == 1) {
			mysqli_query($this->db, "INSERT INTO dot_states(name, country_id)VALUES('$cityName','$cityCountry')") or die(mysqli_error($this->db));
			return true;
		} else {
			return false;
		}
	}
/*Add New State from City*/
	public function Dot_SaveNewState($uid, $cityID, $stateName) {
		$checkUserisAdmin = mysqli_query($this->db, "SELECT user_id FROM dot_users WHERE user_id = '$uid' AND user_type= '1'") or die(mysqli_error($this->db));
		$checkCitiesIDExist = mysqli_query($this->db, "SELECT id FROM dot_states WHERE id = '$cityID'") or die(mysqli_error($this->db));
		mysqli_query($this->db, "SET character_set_client=utf8") or die(mysqli_error($this->db));
		mysqli_query($this->db, "SET character_set_connection=utf8") or die(mysqli_error($this->db));
		if (mysqli_num_rows($checkUserisAdmin) == 1 && mysqli_num_rows($checkCitiesIDExist) == 1) {
			mysqli_query($this->db, "INSERT INTO dot_cities(name, state_id)VALUES('$stateName','$cityID')") or die(mysqli_error($this->db));
			return true;
		} else {
			return false;
		}
...
/*List All City*/
	public function Dot_GetCityStatesAll() {

		$allcities = mysqli_query($this->db, "SELECT id, name, country_id FROM dot_states") or die(mysqli_error($this->db));
		while ($row = mysqli_fetch_array($allcities)) {
			$data[] = $row;
		}
		if (!empty($data)) {
...
/*Get HashTags*/
	public function Dot_GetHashTagsSearch($hashTag) {
		...
		mysqli_query($this->db, "SET character_set_client=utf8mb4") or die(mysqli_error($this->db));
		mysqli_query($this->db, "SET character_set_connection=utf8mb4") or die(mysqli_error($this->db));
		$getData = mysqli_query($this->db, "SELECT P.user_post_id, P.user_id_fk,P.post_type,P.post_created_time,P.comment_status,P.watermarkid,P.which_image,P.which_image,P.hashtag_normal,P.hashtag_diez,P.post_title_text,P.post_text_details,P.user_lat, P.user_lang,P.who_can_see_post,P.post_image_id,P.post_link_url,P.post_link_description,P.post_link_title,P.post_link_img,P.post_link_mini_url,P.post_video_id,P.post_audio_id,P.filter_name,P.gif_url,P.slug,P.post_event_page_id,P.post_video_name ,P.post_page_type,P.post_event_id, U.user_name, U.user_fullname,U.user_page_lang FROM dot_users U  INNER JOIN dot_user_posts P  ON U.user_id =  P.user_id_fk WHERE ($hashtag_query) ORDER BY P.user_post_id DESC LIMIT " . $this->perpage) or die(mysqli_error($this->db));
		while ($row = mysqli_fetch_array($getData, MYSQLI_ASSOC)) {
			// Store the result into array
			$data[] = $row;
		}
	...
	public function Dot_CaltulateHashFromDatabase($hashTag = "") {
		$hashTag = mysqli_real_escape_string($this->db, strip_tags(trim($hashTag)));
		// Now if it has commas, you have to explode() it to an array
		$hashtags_list = explode(',', $hashTag);
		// A variable to hold all the hashtag LIKE conditions
		...
		$Calculate = mysqli_query($this->db, "SELECT COUNT(*) AS totalHashTag FROM dot_user_posts WHERE ($hashtag_query)") or die(mysqli_error($this->db));
		$row = mysqli_fetch_array($Calculate, MYSQLI_ASSOC);
		return $row['totalHashTag'];
	}
	public function Dot_CheckImageExistIn($url) {
		$curl = curl_init($url);
	...
	public function Dot_AmazonS3sUpdate($uid, $theNewBucketName, $theNewAmazonKey, $theNewAmazonSecretKey, $theNewAmazonRegion) {
		$uid = mysqli_real_escape_string($this->db, $uid);
		$checkUserisAdmin = mysqli_query($this->db, "SELECT user_id,user_status,user_type FROM dot_users WHERE user_id = '$uid' AND user_status = '1' AND user_type = '1'") or die(mysqli_error($this->db));
		if (mysqli_num_rows($checkUserisAdmin) == 1) {
			$updateSiteSetting = mysqli_query($this->db, "UPDATE dot_configuration SET s3_bucket = '$theNewBucketName', s3_key = '$theNewAmazonKey', s3_secret_key = '$theNewAmazonSecretKey', s3_region = '$theNewAmazonRegion' WHERE configuration_id = '1'") or die(mysqli_error($this->db));
			return true;
		} else {
			return false;
		}
